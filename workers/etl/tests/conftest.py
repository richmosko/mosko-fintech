...
# The failure policy lives in a sibling module so the CALL SITE is a testable
# unit — see fixture_policy.py's header for why that distinction is the whole
# point (the predicate was pinned; that the fixtures called it was not, and QA
...
def backend():
    """A single PFinBackend for all integration tests.

    Failure policy lives in fixture_policy.construct_or_skip — skips ONLY when
    no database is reachable, and not even then when the run is
    role-constrained. Every other failure propagates.
    """
    import pfin_back_etl as pfbe

    pfb = construct_or_skip("PFinBackend", pfbe.PFinBackend)
    report_login_identity(pfb.engine, "PFinBackend")
    return pfb
...
def nav_worker():
    ...
    migration 054 (pfin.nav_daily) applied in the fixture DB.

    Same failure policy as `backend`.
    """
    import pfin_back_etl as pfbe

    worker = construct_or_skip("NavDailyWorker", pfbe.NavDailyWorker)
    report_login_identity(worker._system_tbc.engine, "NavDailyWorker")
    return worker
