...
- **Reconfirmed a FIFTH time — run 28, 2026-09-23 (`--only dns --confirm-cutover`, main `391954b4`, post-PR #882):** `realrun28-dns.clean.log:61-82` -- the `docker_compose_domains` PATCH read-back again exact-matched; the app-level fqdn again read `<empty>` on arrival, so the `TINKER-WRITE-ALLOW-09` "Checking the app-level sslip fqdn on arrival" step correctly printed SKIPPED (no clear needed, second live confirmation after run 26). The new container (`a55185b1ab30`) again carried the expected env values (`COOLIFY_FQDN=pfindash.com,www.pfindash.com COOLIFY_URL=https://pfindash.com,https://www.pfindash.com SERVICE_FQDN_APP=pfindash.com`); cert poll ok for both apex and www (final http 200, effective `.../login`, over a verified TLS chain), same shape as runs 22/24/26. `realrun28-cutover.clean.log` (same session): `cutover` also VERIFIED, both preflight and apply -- exactly one server, `pfin-prod-1`. **Evidence qualifiers on this run, stated because they cannot be re-derived later:** (a) the box's own provenance banner read `tree: DIRTY` for this run -- team-lead separately measured, in-session, `git status --short` (excluding `.claude/agent-memory`) empty and `git diff --stat -- scripts` = 0, establishing the EXECUTED `scripts/` tree was byte-identical to `391954b4` despite the DIRTY flag (see PR #884's banner-detail fix, booked from this exact ambiguity); (b) `TINKER-WRITE-ALLOW-09`'s arrival-FINDING → probe → clear sequence has still never fired live across any of runs 26/28 (fqdn was empty going in both times) -- its only assurance remains the offline fence scenarios in `fence-assign-app-domain-strikes.sh`, not a live run.
- **Still NOT measured:** whether any OTHER `docker_compose_domains`-consuming endpoint or Coolify version returns the array read-shape instead of the measured object-string shape (COOLIFY-FACT-15's own scope). The `TINKER-WRITE-ALLOW-09` clear path itself (as opposed to its no-op/SKIPPED path, now measured twice) remains live-unmeasured -- see BACKLOG's low-priority live-exercise booking.
