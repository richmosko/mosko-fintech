...
| Container | `migrator` |
| Command | ```sh -c 'echo "PFIN-BUILD-SHA=$(cat /workspace/.build-sha)"; supabase db push --yes --db-url "$PROD_DB_URL" --workdir /workspace; rc=$?; echo "PFIN-LEDGER-TOP=$(psql "$PROD_DB_URL" -tAc "select max(version) from supabase_migrations.schema_migrations")"; echo "PFIN-NEWEST-FILE=$(ls /workspace/supabase/migrations | sort -V | tail -1 | cut -d_ -f1)"; exit $rc'``` — ⚠ **ADR-072 Amendment 7 (2026-09-17, draft):** this is a FOURTH hand-maintained copy of the same literal (the other three: Coolify's own stored Scheduled Task, `infra/supabase/docker-compose.yml`'s `migrator` service comment, and `MIGRATOR_TASK_COMMAND` in `/etc/pfin/migrator-trigger.conf`, written by `scripts/provision-vps.sh`). **This Command row is the literal's home (Sec FLAG B fix, #802) — edit it first.** `docs/deployment-runbook.md` §6.5 is the ONE place that documents the propagation procedure (not a second place to edit the literal itself) — after changing this row, follow §6.5 to propagate the identical literal to the other three sites and re-run `provision-vps.sh --apply`. `migrator-orchestrate.sh` fails closed (exit 10) if Coolify's live task ever drifts from `MIGRATOR_TASK_COMMAND`, so a stale copy here is caught, not silent. **`sort -V` (Sec FLAG A on #802, not a bare `sort`):** file selection must not re-introduce the same width trap the ledger-vs-file comparison already guards against. `sort` is lexicographic — with today's 3-digit versions it happens to agree with numeric order, but the moment a 4-digit migration lands, plain `sort` orders `1000_…` before `999_…` and `tail -1` picks the wrong "newest" file even though `migrator-orchestrate.sh`'s own comparison is numeric-safe. `sort -V` (version sort) orders by numeric value regardless of digit-width. Shape: emits `PFIN-BUILD-SHA=` before the apply (the container's own baked sha, `/workspace/.build-sha` — see the Dockerfile), runs the same `--yes`-guarded `db push` Amendment 6 already validated (rationale below, unchanged), then emits `PFIN-LEDGER-TOP=` (the ledger's own top row, read via `psql` against the same `$PROD_DB_URL`) and `PFIN-NEWEST-FILE=` (the newest migration file baked into this image) before exiting with the apply's own real exit code (`rc`, captured before either `echo`, so a later `psql`/`ls` failure never masks a `db push` failure). `migrator-orchestrate.sh` parses these three tagged lines out of the execution's own `message` field (Coolify's own captured stdout+stderr) instead of `docker exec`ing anything itself — `ci-migrate` has no route to the docker socket at all (measured, ADR-072 Amendment 7). `--yes` rationale (unchanged from Amendment 6, PR #791): measured against the exact pinned CLI binary (v2.107.0, not just its source), under `docker exec` with no pseudo-tty (this Scheduled Task's own invocation shape) the confirmation prompt does NOT hang and does NOT silently decline — a non-TTY read times out after 100ms and falls through to the prompt's own hardcoded default, which for `db push` is "yes" (`internal/utils/console.go`'s `ReadLine`/`PromptYesNo`, `internal/db/push/push.go`'s three `PromptYesNo(ctx, msg, true)` call sites). So this was already safe before `--yes` was added — the flag is added so correctness stops depending on an unversioned upstream default this repo does not control. Empirically confirmed `--yes` works on the real pinned binary (auto-answers immediately, no prompt wait), and that an actual decline (only reachable by piping literal `n` — not something this Scheduled Task's own command line ever does) exits non-zero with the ledger correctly unchanged, never a silent skip — measured against a disposable rig running the exact release binary, four cases (with/without `--yes`, closed stdin vs. a real decline), ADR-072 Amendment 6. `psql` availability: `infra/supabase/migrator/Dockerfile` installs `postgresql-client-15` (Amendment 7) specifically so this `psql` call resolves inside the container; `migrator` authenticates with its own `MIGRATOR_DB_USER`/`MIGRATOR_DB_PASSWORD` (already in its env per `docker-compose.yml`) and owns `supabase_migrations.schema_migrations` per Amendment 5's design, so no new grant is needed for the `select max(version)` read. |
