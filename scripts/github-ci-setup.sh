...
#      reviewer).
#   1  REFUSED -- a real finding: the environment already exists WITHOUT
#      a reviewer rule (refuses to silently add one over an
#      operator-removed gate -- a human decision, not this script's to
#      make unilaterally) and readback still shows no reviewer, or a
#      readback value disagrees with what was just set.
...
SECRET_LIST="$(gh secret list --json name,updatedAt 2>/dev/null || echo '[]')"
...
CURRENT_VAR="$(gh variable get PROD_SSH_HOST 2>/dev/null || true)"
...
if [[ "$ENV_EXISTS" == "1" && "$ENV_HAS_REVIEWER" == "1" && "$ENV_REVIEWER_LOGIN" != "$REVIEWER_LOGIN" ]]; then
  info "environment '$ENVIRONMENT_NAME' already has a DIFFERENT required reviewer ($ENV_REVIEWER_LOGIN) -- this script will ADD/confirm $REVIEWER_LOGIN as well via the same PUT (GitHub's reviewers list is not exclusive to one user); it never REMOVES an existing reviewer."
fi

read -r -d '' ENV_BODY_PY <<PY || true
import json
print(json.dumps({"reviewers": [{"type": "User", "id": $REVIEWER_ID}], "deployment_branch_policy": None}))
PY
ENV_BODY="$(python3 -c "$ENV_BODY_PY")"
