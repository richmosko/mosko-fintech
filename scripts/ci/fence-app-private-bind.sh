...
#     never a second attached network, never a non-'default' key, never
...
# trusting whichever line a `head -1` happened to find first. Same fix as
# scripts/ci/fence-worker-private-bind.sh's own vector 4 -- kept as two
# separate scripts (this fence has no network-var argument to parametrise
# with), not merged.
