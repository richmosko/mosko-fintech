...
- **Zod `.strict()` server-side input validation** at every input boundary (request body, query params, form data, Plaid webhook payload). `.passthrough()` is forbidden on user-facing inputs ([Lock 14](../DECISIONS.md#adr-011) V1-SHIP-BLOCK Sec **mod #1** — `.strict()` typed-input validation + mass-assignment prevention are ONE mod, with `users_id` never read from the body; per [SECURITY §4.5 RT-23](../docs/SECURITY/index.html#rt-23). The "#1 typed-input + #2 mass-assignment" two-mod split that stood here was a mis-numbering, caught by Sec at the SELF-242 joint review 2026-08-20). Numeric inputs go through the shared sanitization battery (NaN/Inf/currency-string/scientific-notation rejection — that battery is **mod #2**).
...
- **Same-transaction audit-log**: every state-changing write to an **audit-class** surface ([ADR-011](../DECISIONS.md#adr-011) Decision 2 — financial-correctness and compliance-attestation data) emits its audit-log row in the same transaction (TypeScript-side analogue of the worker discipline). No retry queues, no fire-and-forget. ⚠ **Settings-store tables are NOT audit-class** ([ADR-011](../DECISIONS.md#adr-011) Decision 18 / Lock 14 — `planning_target` · `tax_bracket_schedule` · `tax_bracket_row` · `owner_identification`): UPSERT-in-place, no edit-history rows, and DELETE is a granted operation so "unset" can remove the row — a discipline an append-only audit surface would forbid. Settings endpoints emit no audit-log row **by design**; do not add one. Any surface not in that enumeration is audit-class until Sec says otherwise.
