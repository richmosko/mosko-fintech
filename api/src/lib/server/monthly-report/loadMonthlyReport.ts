...
export async function loadMonthlyReportForRender(
	...
	// report's OWN `data_as_of` (Lock 15), never today's date. `storedAsOf` re-brands an
	// already-real stored date (`row.data_as_of` — 108's own column type) — the factory for a
	// DB-derived date (V1.5 aal2/asOf close-out follow-up, Sec self360-sec-review.md shape (a)).
	// `userSuppliedAsOf` stays reserved for an ACTUAL client-supplied as-of, which this is not —
	// see asOf.ts's own module header for why the two factories must not be conflated.
	let cashflowRowStaleness: CashflowRowStalenessMap = EMPTY_CASHFLOW_ROW_STALENESS;
	if (staleAccountIds !== null) {
		try {
			const contributors = await loadCashflowContributors(supabase, storedAsOf(row.data_as_of));
