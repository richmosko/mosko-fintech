// navHistory.test.ts — unit coverage for the §2.1.2.c NAV-over-time chart read
...
// 067's NULL-not-zero contract), the fail-soft degrade-to-[] on an RPC error
// and on a non-array payload, and resolveNavHistoryWindow's default-window
// derivation (each half independently optional, relative-to-resolved-end).

import { describe, it, expect, vi } from 'vitest';
import type { SupabaseClient } from '@supabase/supabase-js';
import { loadNavHistory, resolveNavHistoryWindow } from './navHistory';
...
describe('loadNavHistory', () => {
	it('happy path: rows pass through with numeric coercion + correct RPC args', async () => {
		...
		const rows = await loadNavHistory(client, 'monthly', '2020-01-01', '2020-12-31');
	...
	it('empty series (no rows) passes through as []', async () => {
		const { client } = makeSupabase({ data: [] });
		const rows = await loadNavHistory(client, 'monthly', '2020-01-01', '2020-12-31');
		expect(rows).toEqual([]);
	});

	it('RPC error → [] (fail-soft; dashboard must survive)', async () => {
		const { client } = makeSupabase({ error: { message: 'permission denied' } });
		const rows = await loadNavHistory(client, 'monthly', '2020-01-01', '2020-12-31');
		expect(rows).toEqual([]);
	});

	it('non-array payload (unexpected) → [] (degrade, do not assert a shape)', async () => {
		const { client } = makeSupabase({ data: { unexpected: 'shape' } });
		const rows = await loadNavHistory(client, 'monthly', '2020-01-01', '2020-12-31');
		expect(rows).toEqual([]);
	});

	it('null payload (unexpected) → [] (degrade, do not assert a shape)', async () => {
...
describe('resolveNavHistoryWindow', () => {
	it('both absent: end defaults to today, start defaults to 60 months before end', () => {
		...
		const { start, end } = resolveNavHistoryWindow(undefined, undefined);
	...
	it('end supplied, start absent: start defaults to 60 months before the SUPPLIED end, not today', () => {
		const { start, end } = resolveNavHistoryWindow(undefined, '2020-06-30');
		expect(end).toBe('2020-06-30');
		expect(start).toBe('2015-06-30');
	});

	it('both supplied: returned unchanged', () => {
		const { start, end } = resolveNavHistoryWindow('2019-01-01', '2020-01-01');
	...
	it('start supplied, end absent: end still defaults to today (unsafeAsOfForTest sanity: distinct from a fixed past date)', () => {
		...
		const { start, end } = resolveNavHistoryWindow('2019-01-01', undefined);
		expect(start).toBe('2019-01-01');
