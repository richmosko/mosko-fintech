...
describe('AC3 drill-down identity — 238 collapsed row vs 240 total row', () => {
	it('exact equality against a mixed portfolio (US-equity holdings + non-US-equity Non-RE holdings)', () => {
		const taxonomy: TaxonomySubCatRow[] = [
			...
			...US_EQUITY_SUB_CATS.map((sub_cat, i) => ({ id: 10 + i, cat: 'Equity', sub_cat, display_order: 100 + i * 10 }))
		];
		const marketValueRows: SubcatMarketValueRow[] = [
			...
			{ sub_cat_id: 10, cat: 'Equity', sub_cat: 'US-01-Basic_Materials', market_value: 400 },
			{ sub_cat_id: 15, cat: 'Equity', sub_cat: 'US-06-Financials', market_value: 900 },
			{ sub_cat_id: 21, cat: 'Equity', sub_cat: 'US-Growth-Non_Sector', market_value: 300 }
		];
		...
		const equity238 = allocation238.groups.find((g) => g.cat === 'Equity')!;
		const collapsed238 = equity238.rows.find((r) => r.kind === 'us_sector_diversified')!;

		const usEquityTaxonomy = taxonomy.filter((t) => t.cat === 'Equity').map((t) => ({ id: t.id, sub_cat: t.sub_cat }));
