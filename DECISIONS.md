...
**Five V1 instances locked** (four at ADR authoring + instance #5 added by the 2026-07-05 enumeration pass — see the count-grain annotation below):
- **#1 — Lock 9 mod #1:** `pfin.reconciliation_event_trans (event_id, account_trans_id)` — WITH CHECK matching `reconciliation_event.account_id` to `account_trans.account_id`.
- **#2 — Lock 10 mod #2:** `pfin.account_trans.replaces_trans_id` self-FK — WITH CHECK matching target's `account_id` to row's `account_id`. (Realized @ migration `004`.)
- **#3 — Lock 11 mod #9:** `pfin.monthly_report.included_reconciliation_event_ids INTEGER[]` — BEFORE INSERT/UPDATE trigger validating every array element's `reconciliation_event.users_id` equals row's `users_id`.
- **#4 — Lock 12 Architect-spec + mod #2:** `pfin.monthly_report_account_snapshot.account_id` — matched-tenant validation trigger + parent immutability extension fencing `monthly_report.users_id` UPDATE post-creation.
- **#5 — SELF-201 / migration `012` ([ADR-025](#adr-025)):** `pfin.account.sub_cat_id → pfin.user_taxonomy(id)` — BEFORE INSERT OR UPDATE trigger (`fn_account_matched_sub_cat`, SECURITY INVOKER, NULL-safe fail-closed) matching `user_taxonomy.users_id` to the account's `users_id`; both sides per-user. Added by the deferred enumeration pass (F/CTO-ratified 2026-07-05; Sec numbering sign-off at joint-review). **Supersedes** the contaminated operational "7 → 8" downstream references.
