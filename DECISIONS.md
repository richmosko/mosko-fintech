...
- **#13 — `029` (M2.5) / `account_trans_split`:** `pfin.account_trans_split.sub_cat_id` → `pfin.user_taxonomy(id)` · migration `029` · **CR** matched-tenant, chain-resolved (`fn_account_trans_split_matched_sub_cat`, BEFORE INSERT OR UPDATE WHEN `sub_cat_id IS NOT NULL`, INVOKER, NULL-safe; the split child has no own `users_id` — resolved via `trans_id → account_trans → account.users_id`, the `023` #10 shape) · split-child line categorization; realized **AHEAD of #12** (M2.5 ships first) — Sec-pinned at the M2.5 joint-review (took `#13` before `#12` realized, hence the non-contiguity).
