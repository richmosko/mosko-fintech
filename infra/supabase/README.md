...
- **Docker Compose Location:** `infra/supabase/docker-compose.yml`
...
3. **Every relative bind-mount source is repo-root-relative (`./infra/supabase/volumes/...`), not compose-file-relative (`./volumes/...`) as upstream writes them.** This matters because of *how* this compose gets deployed: Coolify's `dockercompose` build pack (`POST /applications/public` with `docker_compose_location` set to this file's path) reads the compose file out of a git checkout — it is not a person running `docker compose up` by hand from inside `infra/supabase/`, which is the scenario upstream's own relative paths assume. Multiple Coolify GitHub issues (#1996, #2444, #3294, #3375, #4468, #6056, #8107 — spanning a wide version range, none confirmed fixed) report that a Docker Compose build pack resolves relative volume sources against the application's checkout **root**, not the compose file's own directory, when the compose file lives in a subdirectory. Written the upstream way, every mount below would point at a nonexistent path at the repo root — and Docker auto-creates a bind mount's missing source as an **empty directory** rather than erroring, so Envoy would boot with no config and the DB init scripts would silently not run: both "comes up healthy, serves the wrong config" failures, not loud ones.

   **⚠ Not verified empirically against this box's Coolify 4.3.18 specifically** — this is the best call from the external evidence available, not a live-tested fact for this install. **Verify at first deploy**, before trusting the stack with anything: check `docker logs supabase-envoy` for an actual config load (not just "container running"), and check `docker logs supabase-db` during first boot for each init-script filename actually executing.
