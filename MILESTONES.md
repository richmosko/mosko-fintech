...
| **Next deliverable** | **Step 1 of the ruled order: the SELF-263 tax-value inventory session** — F/CTO time, a sitting not a dispatch (R5: BOTH default tables; the four decisions the log enumerates), then its seed-delta migration + `tax_relevant` comment-on-column (R10). **In parallel: SELF-259 (bracket tables — Seam A ruled (C) at R4, the D3 label allocates AT the migration) and SELF-267 (YTD-Paid overlay + the Gate A/B ADR fold-in)**, each Sec joint-review MANDATORY. Then 260 → 262 (keystone, with its named residual per R6) → 265 → 264 ∥ 266 → 268 (R3 A′, one-way door, four-layer single walk) → 269. Also on deck: Sec's re-scoped D19 doc PR (F/CTO amendment-vs-silent ruling OPEN) · the ADR-025 CHECK-vs-WITH-CHECK correction (F/CTO correct-vs-annotate ruling OPEN) · the restore/bulk-load runbook (DevOps; MUST precede the incumbent-import item) · the ADR-063-citation sweep (§7.31). |
