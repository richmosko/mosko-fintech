...
| **Next deliverable** | **SELF-223** — §2.1.4 NAV at three reference dates (team-lead recommendation; F/CTO confirms at pickup). SELF-222 is DONE (PRs #452 + #453): the panel consumes `fn_nav_delta_panel`'s now-ELEVEN-column return — migration `072` added `delta_inflation_adjusted_percent` (parity-load-bearing per PRD §3.3 clause (ii); the ratified base is the deflated prior-Year-End anchor, spec'd in `072`'s header + catalog comment; a terse-ADR home is a pending F/CTO ruling). Remaining V1.1 set: SELF-223 / SELF-229 (staleness-framework ramp) / SELF-228 (close-gate, stays LAST). ⚠ **The local dev DB's DATA was wiped 2026-08-14** (QA ran `supabase db reset --db-url <scratch>`; the flag does NOT scope the reset — the command is now banned outright for all agents; schema intact, at `072` post-merge). **Recovery is F/CTO-gated**: assets preserved + dual-verified at `~/Projects/mosko-fintech-recovery/nav-history-20260814/` — ⚠ the RUN LOG is the recovery source, the CSV ends 2025-09-30 and is NOT the run's input. Re-seed path: `supabase/seed.sql` (users/taxonomy) + CPI re-pull + `run_nav_backfill.py` re-run per ADR-053. No live demo of any NAV surface until then. |
...
| **Issue state** | Read it from Linear, not from here. No known Linear-vs-reality divergence as of 2026-08-14 (SELF-222 closed Done on PR #452 merge, liaison-verified with merge comment). The SELF-217 seeding run's DATA was lost in the 2026-08-14 wipe (run record survives at [`docs/records/self217-nav-seeding-run.md`](docs/records/self217-nav-seeding-run.md); re-run is F/CTO-gated per the Next-deliverable cell). |
