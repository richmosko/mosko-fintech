...
- Last updated: 2026-05-29 (**Phase 2 Steps 4–9 complete** — wireframes + UX→Visual handoff + design-system foundation/spec/two-tier-tokens, consolidated in [ADR-014](DECISIONS.md#adr-014); committed home [`docs/DESIGN/`](docs/DESIGN/) established; only Step 10 tokens-as-code remains, gated on the Phase-3 framework choice; PR: `meta/phase-2-design-landing`. Prior: Steps 1–3 + ADR-013, PR `meta/phase-2-step-3-adr`).
...
| **Coupling touchpoint** | Architect's frontend framework choice (Phase 3) ↔ Visual Designer's design tokens format (Phase 2). Single hard coupling; Visual locks abstract taxonomy framework-agnostic; tokens-as-code finalization waits on framework gate OR emits intermediate format (Style Dictionary / W3C design-tokens JSON) per ADR-012 Flag 8 disposition. Other Phase 2 ↔ Phase 3 coordination is case-by-case (Architect flags inline when a flow materially constrains the API surface). |
...
| Status | _Phase 1 closed 2026-05-26; Phase 2 + Phase 3 entered 2026-05-27 in parallel per ADR-012. Detailed steps subsections landed at this PR. Active surfaces: Phase 2 flows + wireframes + design system + tokens (UX + Visual co-lead); Phase 3 ARCH HTML drafting consuming ADR-011 + locks log + 13 carry-over tasks (Architect lead; Sec mandatory joint-review). Coupling touchpoint at Architect's frontend-framework choice ↔ Visual's design-tokens-format._ |
