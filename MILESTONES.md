...
| **Next deliverable** | **SELF-222** — §2.1.3.b multi-horizon NAV-delta panel UI (team-lead recommendation; F/CTO confirms at pickup). Its backend (SELF-221) is Done with a fully-specified consumer contract in the issue's AC5(i): ten-column return incl. the panel's OWN current-side staleness signal (`current_checkpoint_date` — must not borrow the chart's boundary signal) and the annual Jan/Feb structural carried-basis rendering via §2.4.4's ruled shapes ("the first January in production must read as the calendar, not as an incident"). Remaining V1.1 set: SELF-222 / SELF-223 (§2.1.4 three reference dates) / SELF-229 (staleness-framework ramp) / SELF-228 (close-gate, stays LAST). |
