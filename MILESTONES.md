...
| **Next deliverable** | **SELF-218** — the CPI-U inflation-adjusted overlay backend primitive, and the first consumer of the helper `063`–`066` were built for. ⚠ The substrate is complete and measured; the consumer has never been started — that asymmetry is why it is next. |
...
| **Issue state** | Read it from Linear, not from here. ⚠ One known divergence: **SELF-217 reads Backlog in Linear but is PARKED** — see [`BACKLOG.md`](BACKLOG.md) §7.12. |
...
- **2026-08-11 (Phase 6 — five-agent shutdown carry-forward; the notes found five NEW gaps, two self-inflicted; v1.191)** — asking every agent for a handoff **before** teardown surfaced a **revert gap** (`066`'s DROP already destroyed the comment `065` set, so a revert must re-issue it), an **`app_id` blind spot created by today's own Option-B registration**, an unowned **`053` first-of-month grain hole with four downstream outputs**, a sweep bucket that will rot into decoration, and a rejected ADR option recorded nowhere. ⚠ **Every one was found by an agent writing up what it knew, not by any check** — which is the argument for notes-before-teardown, and the reason this entry exists.
- **2026-08-11 (Phase 6 — the ADR-049 chain MEASURED, not just landed; PRs #414–#417, `main` `3f209de`, v1.190)** — `063`–`066` were applied in a committed state for the first time anywhere and **all four first-execution assumptions were proven under a real production login**, the last residual (the writer's own INSERT branch) was closed by a **required** `pgtest` check rather than a one-off, and §7.6 was retitled as a stream with §7.7's placement rationale rebuilt on grounds a retitle cannot reach. ⚠ **The recurring lesson was that a count is not a no-write assertion** — the guard that protected 137 live rows was a teammate's restraint, not the check that had been set.
- **2026-08-11 (Phase 6 — S21 CLOSED and the ADR-049 helper reaches eight columns; PRs #409–#413, `main` `31bbc3f`, v1.189)** — the CPI-U transport drop was lifted and gated by a **three-term** reconciliation (the ruled two-term form could not balance and would have killed the nightly ingest on an `M13`), `066` added `period_was_due` + `coverage_through` so a consumer stops re-deriving the gap policy, **branch protection went 15 → 17** with the ETL lanes finally able to block a merge, and ⚠ **every green on the `063`–`066` stack so far was a rolled-back-transaction green — the writer has never executed against the table it writes.**
