...
| **Next deliverable** | **The amendment batch RAN 2026-09-04** — Linear carries the re-derived ACs on all 19 issues (SELF-345–362, 365), the Platform project's own V1.5 milestone on A1–A8, SELF-350 re-scoped to the RT-22 manifest fence, SELF-366 created for the on-demand write path; the re-derived ACs file merged at PR #626; the PRD §2.6 amendment batch + §2.5.3 fold merged at PR #625 `8e51d6e` (F/CTO read + merge 2026-09-05). Nothing of the batch is outstanding. **Next: the first dispatch — A8 → P7 (SELF-352 → SELF-359)** per `sitting-log.md` R13, with A9 (SELF-353) in parallel; the A3 latency probe (BACKLOG §7.34 item 1) before the A1+A2+A3 design unit. Also on deck: the CA-2026 schedule seed when FTB publishes (SELF-363) · PM's PRD §2.5.3 amendments (SELF-364) · the `fn_gl_entries`/`fn_holdings_as_of` volatility pin (SELF-326 — Done in Linear but NOT on the tree; F/CTO to reopen or record the descoping) · a latency probe on `fn_compute_tax_liability` behind every §2.1 read · Sec's re-scoped D19 doc PR (F/CTO ruling OPEN) · the ADR-025 CHECK-vs-WITH-CHECK correction (F/CTO ruling OPEN) · the restore/bulk-load runbook (DevOps) · the ADR-063-citation sweep (§7.31). |
