...
- **2026-08-16 (Phase 6 — V1.2 reconciliation ratified AND SELF-235 shipped; PRs #468 + #469, `main` `60394a2`)** — the reconciliation-first pass on the rotated milestone found SELF-234 wholly superseded (FOURTH schema-impossible instance; the drafted design was the exact thing `022` rejected, premise traced to the incumbent ETL's `pfin.asset_cat` shape) and F/CTO ratified Option C: cancel-superseded + residuals split to SELF-324 (%Target substrate, Lock-14 one-way door) and SELF-325 (manual-asset add flow, scoped by the F/CTO multi-asset ruling: accounts are pure holding structures, manual holdings are first-class `pfin.asset` rows with `manual_valuation` price history, cost basis account-side, classification per-asset) + a DDL-anchored AC-amendment pass across 7 issues + the collapsed "US - Sector Diversified" drill-down row + BACKLOG §7.19 (the F/CTO-directed PRD/ARCH post-GL recalibration pass — the generator behind the pattern, now with a standing AC-3 minimum at rotation/parity boundaries). SELF-235 then shipped as the first V1.2 build item (PR #469: full symbols list + cascading reassignment; 4-commit sequential-pen build; ADR-013 H1 domain pre-validation is the system's ONLY domain enforcement per `022`'s own note, now fenced by a pgTAP KNOWN-GAP tripwire; PM caught-and-rewrote a false-product-claim lede; stated limit: no authenticated live click-through — rides §7.18).
