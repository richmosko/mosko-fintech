...
| **Next deliverable** | **SELF-228 — the V1.1 close-gate (§2.1.7 multi-function cross-tenant RLS battery) is the ONLY remaining V1.1 item.** SELF-229 is DONE (PR #461, merged 2026-08-15): staleness badge on all four V1.1 NW surfaces + composition per-leaf staleness (server-side RLS-scoped join, tri-state) + the F/CTO-ruled framework-root fail-closed rework (`is_stale: boolean|null`, `UNKNOWN_STALENESS` distinct from empty, REQUIRED props with the compiler as the watcher) + the January-family copy set (PM-reviewed) — through a three-round Sec joint-review (AMBER→GREEN→GREEN, final no-findings; record on the PR). The SELF-229 Linear AC1 signature was corrected post-hoc (zero-arg 046, liaison-applied) — second schema-impossible `(p_users_id, p_scope)` AC instance; PM adopted a copied-DDL-or-marked-provisional drafting rule. Pending F/CTO rulings queued: `db reset` mechanical guard (Sec recommends a permission deny rule) · terse-ADR home for the `072` real-percent base. The 2026-08-14 data wipe remains RECOVERED (closure in [`docs/records/2026-08-14-db-reset-incident.md`](docs/records/2026-08-14-db-reset-incident.md)); `account`/`linked_source`/`user_settings` stay empty by design (BACKLOG §7.17); `supabase db reset` remains banned outright for all agents. |
