...
| Field | Value |
|---|---|
| **Current build** | **V1.4 — Estimated taxes full (§2.5), rotated in 2026-09-03** per `/milestone-rotation` + [ADR-017](DECISIONS.md#adr-017) Decision 2 (**V1.3 ✅ COMPLETE 2026-09-03** — 15 issues all Done, verified against live Linear; close-gate SELF-257 discharged at PR #596 with Sec GREEN as its own AC11; final merge `1601366`). V1.4 holds **8 issues in Linear, all Backlog** (SELF-263/265/266/267/268/269 + GL follow-ups SELF-302/303); close-gate **SELF-269** (§2.5.5 RLS battery) LAST per the standing pattern. **THE FIRST REAL §7→LINEAR PROMOTION FIRED at this rotation**: V1.5 (Monthly report) promoted from BACKLOG §7.1+§7.2 as **SELF-345–362** (18 issues: A1–A9 → Platform/Cross-cutting incl. A9's cutover-clock capture substrate; P2–P10 → Monthly report / V1.5 milestone; P11 stays in §7 for the V1.4-completes rotation per current+next discipline) — descriptions verbatim-with-links verified per-issue against the file, §7 entries annotated. ⚠ **V1.4's issues PREDATE the GL rework** (drafted before the recalibration baseline, same as V1.3's were — and V1.3's pre-flight measured 0 of 14 buildable as drafted, with three MORE AC-text defects surfacing even after that pass at SELF-257): **a V1.4 pre-flight recalibration sitting (the V1.3-precedent pass, ADR-063 protocol 1) is the recommended FIRST act of the milestone — dispatch nothing against the drafted ACs until it runs.** |
| **Next deliverable** | **The V1.4 pre-flight recalibration sitting (F/CTO + PM + Architect + Sec; the ADR-063 protocol-1 pass)** — re-derive all 8 issues' ACs against the current tree (baseline: read `main` live at the sitting), produce the ruled dispatch order + Sec map, land the record per the [`docs/records/v13-preflight/`](docs/records/v13-preflight/) precedent. Justification is measured, not cautionary: V1.3's equivalent pass found 0 of 14 drafted ACs buildable, and SELF-257 still surfaced three more schema-false AC claims post-pass (the 4th–6th PM-draft-vs-schema instances, clarified on the issue). After the sitting: first build dispatch per ITS ruled order. Also on deck before/alongside: Sec's re-scoped D19 doc PR (all THREE HTML-escaped sites + the false Sweep sentence; F/CTO amendment-vs-silent ruling OPEN) · the ADR-025 CHECK-vs-WITH-CHECK correction (F/CTO correct-vs-annotate ruling OPEN) · the restore/bulk-load runbook (DevOps; MUST precede the incumbent-import item; Sec's catch criterion incl. the VALIDATE-CONSTRAINT-no-op trap is on record at the #596 review thread) · the ADR-063-citation sweep (§7.31). |
