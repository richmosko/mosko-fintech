...
| **Next deliverable** | **SELF-220** — the §2.1.2.d NAV-over-time chart UI (nominal + inflation-adjusted dual-line). Every upstream is Done as of 2026-08-12 (SELF-219 series, SELF-218 overlay, SELF-217 backfill machinery). ⚠ Two riders travel with it: the ADR-053 Decision 7 staleness-rendering ruling (PM+UX — how the monthly-grain imported decade renders at daily/weekly granularity) and Sec's standing joint-review request on rendering `nav_inflation_adjusted` (SUM/AVG over the nullable column silently skips un-deflatable points) — both already recorded as comments on the Linear issue. |
...
| **Issue state** | Read it from Linear, not from here. No known Linear-vs-reality divergence as of 2026-08-12 close (SELF-217 + SELF-218 both Done via merged PRs). ⚠ One PENDING OPERATION, not an issue: the SELF-217 production seeding run is F/CTO-gated — preconditions and the tracked-safe run-record obligation are recorded as the final comment on the Done issue. |
