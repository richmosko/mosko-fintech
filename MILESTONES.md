...
| Field | Value |
|---|---|
| **Current build** | **V1.2 — Asset allocation full (§2.2), rotated in 2026-08-16** per `/milestone-rotation` + [ADR-017](DECISIONS.md#adr-017) Decision 2 (V1.1 ✅ COMPLETE 2026-08-16 — all 13 issues Done, close-gate PR #464). V1.2 = 11 issues in Linear (SELF-234–SELF-244; 1 pre-existing Done — but see Issue state ⚠) + 4 V1.x cross-cutting upstream (SELF-230–SELF-233; 2 Done). **Next milestone: V1.3 — Cash flow full (§2.3)** (already booked in Linear per the V1.0–V1.4 carve-out; the FIRST BACKLOG §7→Linear promotion fires when V1.3 completes, so no promotion happened this rotation and the free-issue limit did not bite). |
| **Next deliverable** | **First V1.2 item — sequencing awaits F/CTO ratify.** Team-lead's standing recommendation: SELF-234 (asset denormalized-cols migration) first — schema substrate before the UI chain — with the SELF-236-vs-`048` verification (see Issue state ⚠) as a pre-step, and PM/Architect to propose the full V1.2 sequence. Still-queued F/CTO rulings: terse-ADR home for the `072` real-percent base · SELF-217 clarifying comment · pg_dump pre-flight snapshot (Sec rec C) · disposition of the 4 local-vs-CI battery failures. The db-reset mechanical guard is **ACTIVE** (fresh-session four-probe proof on PR #463); `supabase db reset` remains banned outright for all agents; `account`/`linked_source`/`user_settings` stay empty by design (BACKLOG §7.17). |
...
| **Issue state** | Read it from Linear, not from here. ⚠ Two known divergences, surfaced NOT reconciled: (1) **SELF-217 reads Done (auto-completed 2026-08-13) but the record says PARKED-foreclosed** per ADR-040 — do not read its Done as "built"; awaiting F/CTO's call on a clarifying comment. (2) **SELF-236 ("Per-manual-account Sub-Cat UI") is V1.2's one pre-existing Done, but migration `048` later DROPPED `account_sub_cat`** — its Done predates the substrate's removal and must be schema-reality-checked (PM/Architect) before V1.2 planning builds on it. The SELF-217 seeding DATA remains restored (128 rows byte-identical to the preserved run log; [`docs/records/self217-nav-seeding-run.md`](docs/records/self217-nav-seeding-run.md)). |
...
- **2026-08-16 (Phase 6 — milestone-rotation V1.1 → V1.2)** — V1.2 — Asset allocation full (§2.2) rotated to current (11 Linear issues, close-gate SELF-244 last; next = V1.3 Cash flow, already booked, so no §7 promotion fired), with two Linear-vs-reality divergences surfaced en route (SELF-217 Done-vs-parked; SELF-236 Done predating `048`'s substrate drop — see Issue state).
