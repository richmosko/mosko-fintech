...
| Field | Value |
|---|---|
| **Current build** | **V1.5 — Monthly report full (§2.6), rotated in 2026-09-04** per `/milestone-rotation` + [ADR-017](DECISIONS.md#adr-017) Decision 2 (**V1.4 ✅ COMPLETE 2026-09-04** — 10 issues all Done, verified against live Linear; close-gate SELF-269 discharged at PR #619). V1.5's set after the 2026-09-04 pre-flight sitting (R8 / R9): **A1–A8 (SELF-345–352) + A10 (SELF-366, on-demand generation write path) + P2–P10 (SELF-354–362)**; A9 (SELF-353) stays unmilestoned in Platform and is dispatched early on its own clock; close-gate **P10 / SELF-362** LAST (§2.6.6 RLS verification battery — no V1.5 issue closes until it passes). **V1.final promoted from BACKLOG §7 P11 → SELF-365** (V1.final milestone, 0→1 issues) — this rotation's promotion event per ADR-017 D2. |
| **Next deliverable** | **V1.5 is mid-wave under F/CTO delegation (2026-09-05 → 06; log [`docs/records/v15-execution/log.md`](docs/records/v15-execution/log.md), on `meta/v15-execution-log` until close-out).** The whole substrate is on `main`: A1–A9 (SELF-345–353), migrations `106`–`115` with their batteries, the PDF worker + render client, the monthly-report cron, P9 (SELF-361); one Sec veto (the `111` audit helper's `'cron'` forgery channel) raised and cleared on conditions C1–C4. **Held at the browser-walk gate, built and rebased onto `main`:** P7 (SELF-359), P2 (SELF-354, PR #643 CI-green), P3 (355), P5 (357), P8 (360), P4 (356), P6 (358, with build-time CSS extraction + ADR-069). P10 (SELF-362) is authored — six new legs over the shipped batteries plus the render-budget measurement — and its close-gate VERDICT is the last act. A10 (SELF-366) closes with P5 by design. **The one open decision: forward-applying `106`–`115` to the shared local dev DB for the walks** (the team-lead does not mutate that DB without an explicit yes). Then: walks → Sec reads (P3 / P4 / P6 mandatory) → merges in stack order → P10 verdict → close-out (this ledger, BACKLOG §7.32 item 6, agent-memory batch 3, the log PR). Still on deck: SELF-363 · SELF-326's descoping (Done in Linear, absent on the tree) · SELF-375–378 as booked · the `fence-set-config` promotion into required contexts + its RT catalog row · Sec's re-scoped D19 doc PR · the ADR-025 CHECK-vs-WITH-CHECK correction · the restore/bulk-load runbook · the ADR-063-citation sweep (§7.31). |
| **Milestone close-gate** | **SELF-362** — P10, the §2.6.6 RLS verification battery. It gates the WHOLE V1.5 milestone, so it is the LAST item (the SELF-244 / SELF-228 / SELF-257 / SELF-269 pattern). V1.final is already promoted to Linear as of this rotation (SELF-365); its own N=2-consecutive-month close-gate runs per PRD §3.4, not a further §7 promotion. |
...
- **2026-09-04 (Phase 6 — V1.5 PRE-FLIGHT SITTING CLOSED + AMENDMENT BATCH RAN; sitting log PR #624 `1417337`, re-derived ACs PR #626 `0c5bd25`, PRD amendment PR #625 open)** — 14 of 14 rulings recorded one per turn (frozen rendered payload · app pushes finished HTML, worker is a PDF printer · impersonation binding for the cron · one monotone status transition, no DEFINER · dormant array fence · the RT-22 issue re-scoped to the manifest fence · the reserved general audit helper authored · A1–A8 into the milestone · A10 opened · PRD batch authorized · commentary column renamed · V1.final month-of-operation defined · dispatch order · consolidated ADR on A1's PR). Linear now carries every ruled AC; read-back verified by normalized diff. First dispatch next: A8 → P7.
- **2026-09-04 (Phase 6 — THE V1.5 PRE-FLIGHT SITTING RAN; findings PR #623 `d1e5e3d`, sitting log on `meta/v15-sitting`)** — Architect / PM / Sec audited the 18 V1.5 issues at `b90b846` over two cross-citation rounds each: buildable-as-drafted 2 / 3 / 4 of 18 over three stated predicates; one one-way door (frozen rendered artifact vs read-time recomposition — ruled FROZEN); the PDF direction inverted in three drafts (ruled: app pushes finished HTML, worker is a PDF printer); SELF-350 already built with two fence-weakening AC clauses (Sec veto; re-scoped to the dependency-manifest fence); two Decision-3 labels re-allocated by drafts that already hold them (struck). 14 rulings R1–R14 recorded with losing sides; A1–A8 join the milestone, A10 opens for the on-demand write path, the consolidated Gates A/B/F + Lock 11/12/13 ADR rides A1's PR. First dispatch A8 → P7.
- **2026-09-04 (Phase 6 — V1.4 → V1.5 MILESTONE ROTATION; close-gate PR #619 `a1d5dc0`, ledger PR #620)** — all ten V1.4 issues verified Done against live Linear, `/milestone-rotation` executed: **V1.5 (Monthly report) is CURRENT, and V1.final (§3.4 close mechanism) promoted from BACKLOG §7 P11 as SELF-365** — the sole remaining §7.2 entry, verbatim-with-links, verified against the live V1.final milestone (0→1 issues); recommended first act of V1.5: the pre-flight recalibration sitting, on the V1.3/V1.4 measured precedent (0 of N buildable as drafted, both times).
