...
| **Next deliverable** | **⚠ FIRST: the L1 cash-granularity F/CTO decision is PENDING IN DEPTH** — the substrate supports ONE raw-cash classification per user per currency vs four seeded cash Sub-Cats; PM brief recommends (a) instrument-routed cash (CD/T-Bill fill per-asset under the multi-asset ruling; FDIC/SPIC = the residual per-account gap) with an incumbent-p.4 inspection hook; fallback (c) render-time split by account type; (b) per-account classification = V2. Full trace: `076`'s header L1/L2 blocks + the PM brief in the 2026-08-17 session record + the handoff memory. THEN: **SELF-325** (manual-asset add flow, design ruled) and the ratified sequence 238∥240 → 242 → 239/241 → 243 → close-gate 244 LAST — **gated by the routed follow-ups: the kernel price-tiebreak cross-copy fix + kernel-identity CI fence (Architect+DevOps, Sec-ruled BEFORE 238/240 ship) and SELF-233 (settings hardening) before the allocation backends; the STABLE volatility pin (3 financial functions incl. both sides of foot-to-NAV) queued with its own joint review; the Lock-14 app-layer fence half OWED at 238/240/242.** **The small-items queue is CLEARED (2026-08-16, PRs #471–#474):** `072` base → ADR-055 · SELF-217 annotated · battery disposition landed (scratch-DB venue doctrine; h14 verified-in-CI) · pg_dump snapshot mechanism merged — ⚠ **merged ≠ ACTIVE: the NEXT fresh session must verify the SessionStart snapshot hook actually fired (check `/tmp/db-snapshot-hook.log` + the snapshot dir) and record the proof on PR #473 before any text calls it active.** The db-reset mechanical guard is **ACTIVE** (PR #463 probes); `supabase db reset` remains banned outright; `account`/`linked_source`/`user_settings` stay empty by design (BACKLOG §7.17). |
...
- **2026-08-16 late (Phase 6 — the small-items queue cleared; PRs #471–#474, `main` `9e2ba59`)** — agent-memory housekeeping landed (#471, 45 files); **ADR-055** gave the `072` real-percent formula base its canonical home (#472); the **pg_dump pre-flight snapshot mechanism** merged (#473, Sec AMBER→AMBER→GREEN — round-2 residual was Sec's own supplied threshold, unfixable by tuning on a schema-dominated dump, remediated by construction with the never-prune-the-largest guard; SessionStart wiring team-lead-applied under explicit F/CTO authorization after the classifier correctly denied the DevOps edit; **fresh-session hook proof still owed on the PR before "active" may be claimed**); the **local-battery disposition** landed (#474, Sec-consult-driven after QA refused two originally-ruled fixes and was sustained on both — h14's credential fence preserved with a half-specific annotation, `062`'s corrupt-the-control blindness documented + a two-sided raw-table companion leg added, the limit-1 displacement class fact recorded, and QA's cluster-level-role finding folded back as a correction to Sec's own venue clearance: h14 is VERIFIED-IN-CI, expected-different locally). SELF-217's Done clarified in Linear (outcome-via-seeding-run, not implemented-as-drafted).
