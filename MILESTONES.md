...
| **Next deliverable** | **Step 7 of the ruled order: SELF-264 ∥ SELF-266 (the two read surfaces over `fn_compute_tax_liability`), in flight on `origin/main` @ `346d204`+** — one Backend (shared `taxLiability.ts` query + both loaders) and two Frontends, each Sec joint-review MANDATORY (money rendering, ADR-066 D1 (b)), walk gate before Sec. Then **SELF-268** (R3 A′, the one-way NAV flip — headline read source moves, Sec re-reads R3, four-layer single walk) → **SELF-269** close-gate battery LAST → `/milestone-rotation` (V1.final promotes from §7 P11). **Steps 1–6 LANDED 2026-09-04** (263 `e89cc9a` · 267 `a9d4347` · 259 `e4eae27` · 260 `957b6fc` · 262 `346d204` · 265 `26b72c3`), all under the F/CTO delegation recorded at the head of [`docs/records/v14-execution/log.md`](docs/records/v14-execution/log.md) (E1–E38, every ruling with its losing side; reversal window open until the close-gate). Also on deck: the CA-2026 schedule seed when FTB publishes · the `fn_gl_entries`/`fn_holdings_as_of` volatility pin (role:migration) · PM's PRD §2.5.3 amendments (installment-count definition, rollover copy) · Sec's re-scoped D19 doc PR (F/CTO ruling OPEN) · the ADR-025 CHECK-vs-WITH-CHECK correction (F/CTO ruling OPEN) · the restore/bulk-load runbook (DevOps) · the ADR-063-citation sweep (§7.31). |
