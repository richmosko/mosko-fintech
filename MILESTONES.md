...
| **Next deliverable** | **SELF-324 — the %Target substrate options pass** (Architect + mandatory Sec joint-review; the ratified-sequence next step and the blocker for SELF-238/240/242), with **SELF-325** (manual-asset add flow, F/CTO-ruled design on record) available in parallel. Ratified sequence remainder: 237 → 238∥240 → 242 → 239/241 → 243 → close-gate 244 LAST; SELF-232/233 (V1.x substrate) must land before the allocation backends. Still-queued F/CTO rulings: terse-ADR home for the `072` real-percent base · SELF-217 clarifying comment · pg_dump pre-flight snapshot (Sec rec C) · disposition of the 4 local-vs-CI battery failures. The db-reset mechanical guard is **ACTIVE** (PR #463 probes); `supabase db reset` remains banned outright; `account`/`linked_source`/`user_settings` stay empty by design (BACKLOG §7.17). |
...
| **Issue state** | Read it from Linear, not from here. ⚠ One divergence still open: **SELF-217 reads Done (auto-completed 2026-08-13) but the record says PARKED-foreclosed** per ADR-040 — do not read its Done as "built"; awaiting F/CTO's call on a clarifying comment. (The SELF-236 divergence is RESOLVED: annotated built-then-retired-at-`048` in Linear per the 2026-08-16 ratify; successor = SELF-325.) The SELF-217 seeding DATA remains restored (128 rows byte-identical to the preserved run log; [`docs/records/self217-nav-seeding-run.md`](docs/records/self217-nav-seeding-run.md)). |
