...
| Field | Value |
|---|---|
| **Current build** | **V1.5 — Monthly report full (§2.6), rotated in 2026-09-04** per `/milestone-rotation` + [ADR-017](DECISIONS.md#adr-017) Decision 2 (**V1.4 ✅ COMPLETE 2026-09-04** — 10 issues all Done, verified against live Linear; close-gate SELF-269 discharged at PR #619). V1.5's set: A1–A9 (SELF-345–353, Platform/Cross-cutting substrate) + P2–P10 (SELF-354–362, Monthly report project); close-gate **P10 / SELF-362** LAST (§2.6.6 RLS verification battery — no V1.5 issue closes until it passes). **V1.final promoted from BACKLOG §7 P11 → SELF-365** (V1.final milestone, 0→1 issues) — this rotation's promotion event per ADR-017 D2. |
| **Next deliverable** | First act of V1.5: the **pre-flight recalibration sitting**, on the same precedent as V1.3 and V1.4 (both measured 0 of N issues buildable as drafted before their sittings ran). Also on deck: the CA-2026 schedule seed when FTB publishes (SELF-363) · PM's PRD §2.5.3 amendments (SELF-364) · the `fn_gl_entries`/`fn_holdings_as_of` volatility pin (SELF-326 — Done in Linear but NOT on the tree; F/CTO to reopen or record the descoping) · a latency probe on `fn_compute_tax_liability` behind every §2.1 read · Sec's re-scoped D19 doc PR (F/CTO ruling OPEN) · the ADR-025 CHECK-vs-WITH-CHECK correction (F/CTO ruling OPEN) · the restore/bulk-load runbook (DevOps) · the ADR-063-citation sweep (§7.31). |
