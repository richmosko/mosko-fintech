...
3. **§2.1.5 tax lines.** `105` subtracts two envelopes from `104`. **The ruling** is ADR-067 Decision 3: *"**Option C** (backfill the series) carries a recorded **Sec VETO**."* **The rationale wording** is migration `105`'s `comment on function`: *"the tax state for a past date is not recoverable, so a back-fill would be a fabrication with the shape of a measurement (Sec veto, recorded at ruling R3)."* **Recomposing a past month's tax lines from today's designations reaches exactly that vetoed artifact by another route.**
   - ⚠ **This entry originally attributed the rationale sentence to ADR-067 D3 itself. That was wrong and it is the reusable lesson:** `grep -c 'fabrication with the shape of a measurement' DECISIONS.md` returns **0** — the string lives in `105` and in `docs/records/v14-*`, never in canon. Right ruling, wrong pointer ([[feedback_false_composite_citation]]). Caught by Sec at the V1.5 pre-flight, 2026-09-04. **A quotable sentence in a migration comment or a records file reads exactly like an ADR quotation once it is repeated; grep `DECISIONS.md` before writing "ADR-NNN says" around quote marks.**
