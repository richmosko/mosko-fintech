...
- [My review measurements become quoted sources](feedback_my_review_measurements_become_quoted_sources.md) — re-run in the SAME turn as the number; name what ONE UNIT of a count is. ⚠⚠ THREE malformed instruments in one wave, all returning the reassuring EMPTY answer — never build a zsh pathspec from `$var` (no word-split); give every scoped command a POSITIVE CONTROL; a too-clean answer is a prompt to re-measure. A wrong QUESTION + a wrong instrument = a confident wrong answer; DELTAS reviewed ≠ TREE reviewed.
...
- [Supplied text ending mid-sentence orphans the tail](feedback_supplied_text_ending_midsentence_orphans_the_tail.md) — name the splice boundary on BOTH sides.
