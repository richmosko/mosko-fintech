...
> ⚠ **Not only DDL — any guard I REQUIRE can invalidate fixtures inside the same PR.** At PR #828 I required a `UUID_RE='^[a-z0-9]{20,32}$'` shape-guard in `scripts/db-shell.sh` before it interpolates an `.env` value into a root `ssh` command. That same PR's new CI job seeded `stackuuid456` (12 chars) and `notthestack` (11) as its fixture values — **both barred by the guard I was asking for**, so the remediation as stated would have turned a passing fence red at merge. The blast radius of a narrowing constraint runs through the fixtures of the very change under review, not just through merged batteries elsewhere. **Before handing over any tightening, grep the PR's own fixtures for values the new predicate would refuse, and ship the fixture change and the third (negative-control) strike leg as part of the same condition** — otherwise the guard lands unwatched or the lane lands broken.
