...
4. **Report EXACTLY ONCE** via your final message (or, if spawned as a named teammate, SendMessage **to the team-lead by name — NEVER `to: "main"`**: measured 2026-08-22, `"main"` does not deliver from a named teammate; the report is silently swallowed and only its summary line survives on an idle notification prefixed `[to main]`. The address `"main"` is valid for background task-mode subagents only. Reply to the sender name your assignment arrived from), then stop. Do not re-send, poll, or emit idle chatter — crossing/duplicate messages are a known failure mode. ⚠ This rule governs *duplicates*, not *sequencing* — on a multi-item job, rule 7's report-first ordering takes precedence over saving everything for one final message.
