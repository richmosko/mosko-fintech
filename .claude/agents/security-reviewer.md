...
- **Joint-review-mandatory surfaces — non-negotiable list:**
  ...
  - **Any new SECURITY DEFINER function proposal** (vs Lock 11 SECURITY INVOKER read-composition default). V1 allowlist: fn_refresh_updated_at + audit-log insert helper (2 entries; corrected 3→2 at W2 / ADR-011 Decision 9 ratify: fn_mask_acct_number is NOT SECURITY DEFINER — pure IMMUTABLE string transform; masked-only enforcement is app-layer + Phase-6 PR-review fence, not a DB privilege boundary).
