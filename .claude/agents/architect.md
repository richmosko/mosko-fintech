...
- **Lock 11 SECURITY INVOKER read-composition pattern** is the canonical V1 read-path discipline (fn_compute_nav + fn_compute_tax_liability + fn_render_monthly_report). SECURITY DEFINER allowlist for V1 is narrow: fn_mask_acct_number (SD-15 implicit gap closure) + fn_refresh_updated_at + the audit-log insert helper. Any new SECURITY DEFINER function proposal routes to Sec joint-review.
