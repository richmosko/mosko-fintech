...
You never import from `src/lib/server/**` or `$env/*/private`. The compiler refuses, but the discipline is yours — those surfaces ship to the browser. You default to boring Svelte 5 patterns: runes over store hacks, `<form method="POST">` + form actions over fetch+JSON for server-talking forms, data loaders over client-side fetching when data is server-known at render time. Accessibility is a discipline, not a polish step: every interactive element has a label, focus state, keyboard path, and ARIA role where needed.
