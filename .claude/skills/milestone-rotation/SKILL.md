...
**[ADR-017](../../../DECISIONS.md#adr-017) Decision 1** (MILESTONES.md compact-ledger convention): last 5 entries; 1 sentence per entry naming deliverable + key F/CTO ratify + headline metric; each entry ends with `Detail: [CHANGELOG vN.NN](CHANGELOG.md#vNNN--YYYY-MM-DD)`. Oldest entry rotates out when at 5.
...
## Step 3 — update MILESTONES.md and CHANGELOG.md

9. Update MILESTONES.md "Recent activity" section:
   - Add a new 1-sentence entry at the top of the last-5 list: names the milestone that completed + headline count (e.g., "N Linear issues closed; PR #NN") + `Detail: [CHANGELOG vX.YY](CHANGELOG.md#vXYY--YYYY-MM-DD)` (root-relative — the link is written into MILESTONES.md, a repo-root file).
   - If the list is at 5 entries, drop the oldest.
   - 1 sentence only. Detail belongs in CHANGELOG.md.
10. Add a CHANGELOG `vX.YY` entry (increment from the prior version): milestone name + completion date + list of closed issues + promotion count (how many §7 entries promoted + the SELF-N range assigned). This is where the dense narrative lives. Follow the structure of prior Wave-close entries.
...
| **MILESTONES.md entry > 1 sentence** | Decision 1 is a hard compact-ledger constraint. Dense narrative goes in CHANGELOG only; violation re-inflates session-context bloat (the original ADR-017 context: 6000-word auto-loaded section across Waves 1–4). |
...
- Composes with `/brief-drift-catch` (run Discipline 1 over any BACKLOG.md spec count or SELF-N range before forwarding to F/CTO ratify) and `/finish-doc-update` (to land the MILESTONES.md + CHANGELOG.md edits).
