...
> ⚠ **Do not infer the merge style from `main`'s recent history — read it from THIS line.** Fifty PRs (#704–#749, 2026-09-10 → 2026-09-13) were squash-merged by drift: the team-lead session resumed from a context summary, merged the next PR with `--squash` for no stated reason, and every later merge copied the one before it. Nothing forced it — branch protection has `required_linear_history=false` and all three styles are allowed. F/CTO caught it on 2026-09-13 and ruled merge-commit stands; those fifty single-parent `… (#NNN)` commits are left as they are. **A run of single-parent `(#NNN)` commits at the top of `main` is evidence of drift, not of house style.** After any compaction or resume, re-read this step before the first merge.
