...
mosko-fintech adaptation of `richmosko/project_template`'s `/serve-docs` per [ADR-010](../../../DECISIONS.md#adr-010). Differences from template:

- **No `/open-doc` reference.** Template's serve-docs skill cross-references its `/open-doc` skill for the Chrome→Safari browser-open fallback; mosko doesn't ship `/open-doc`, so the same fallback is inlined directly here.
- **No `/merge-pr` reference** in the suggested workflow. mosko merges via the GitHub UI or `gh pr merge --squash <pr#>`.
...
/serve-docs ARCH         # same for ARCH (scaffold exists; content drafts in Phase 3)
...
(Chrome first; falls back to Safari if Chrome isn't installed or fails to open.)
...
- **`scripts/serve-docs.sh` is missing or non-executable**: surface the error from the Bash launch; suggest checking the repo bootstrap (the script ships with this PR).
...
- **Pairs with `/refine-doc`.** Typical session: `/serve-docs PRD` → review in browser + add comments via the widget → `/refine-doc PRD` → `/start-doc-update` → `/finish-doc-update` → merge via GitHub UI or `gh pr merge --squash <pr#>`.
- **ARCH content note.** `docs/ARCH/index.html` is scaffolded (per PR #40) but the substantive content lands in Phase 3 (Architect lead). The widget activates on the scaffold; meaningful section-level comments will be more useful once Phase 3 drafting completes.
