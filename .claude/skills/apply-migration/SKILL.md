...
>   - the **`comment on role` / `create role` statement dispositions** on **`117`, `118`, `119`** (**PR #775**, **PR #784**, and Amendment 5 **Decision G4** + its SUPPLEMENT) — changes statements, not header prose. ✅ **WIDENED by F/CTO assent 2026-09-16 (second re-ratify of the day; same expiry, same anti-precedent clause).** *Requested earlier the same day and kept as dated history: this entry named `118`/`119`; `117` was edited and appeared NOWHERE in this block, and `119` had been trimmed from the Amendment 5 row hours before. The `001`–`118` sweep entry does not cover them — those are statement-level dispositions, not the sweep — so widening the Scope was F/CTO's act, and until assent landed the three edits were NOT covered.* ⚠ **The assent widens the NAMED LIST ONLY: the expiry is unchanged (when the re-bootstrap lands and a deployment survives) and the anti-precedent clause is unchanged. Second re-ratify in one day — a third should prompt asking whether this still describes one bounded window.**
...
>   - **`007`/`015`: the WHOLE VIEW UNIT behind an applier guard** — create + comment + three REVOKEs + grant, applied whole or skipped whole — **plus ownership transfer by a supervised POST-step**, under vault disposition **(iv‴), F/CTO-ratified 2026-09-16** (superseding (iv′), ratified and then falsified by measurement the same day). `security_invoker = true` is **retained as a COMPONENT** of that shape, not as the shape itself. Also the header corrections riding the same edit: `015`'s *"owner-semantics required"* mechanism sentence, and both files' *"never the raw whole-vault surface"* claim (enforcement for `anon`/`authenticated`, convention for `service_role`). *This entry previously named only the `security_invoker` change under disposition (iii), whose ratified basis was measured FALSE — kept as the dated record; the Scope was **re-ratified 2026-09-16** to name the unit guard.* The unit is **executable DDL**; the headers ride it because separating them would leave a corrected view on a falsified rationale.
