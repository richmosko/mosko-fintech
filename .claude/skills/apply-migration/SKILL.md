...
>   - the **`comment on role` / `create role` statement dispositions** on **`117`, `118`, `119`** (**PR #775**, **PR #784**, and Amendment 5 **Decision G4** + its SUPPLEMENT) — changes statements, not header prose. ⚠ **WIDENING REQUESTED 2026-09-16 — NOT YET ASSENTED: this entry named `118`/`119`; `117` is edited and appeared NOWHERE in this block, and `119` had been trimmed from the Amendment 5 row earlier the same day. The `001`–`118` sweep entry does not cover them — those are statement-level dispositions, not the sweep. Widening the Scope is F/CTO's act; until assent lands these three edits are NOT covered by this exception.**
