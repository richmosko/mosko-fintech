...
begin
  ...
  if v_admin then
    ...
    raise warning 'G4-SKIP: comment on role migrator NOT re-issued — % holds neither superuser nor ADMIN OPTION on the role, and Postgres refuses to grant any role ADMIN OPTION on itself, so no grant closes this. The comment is landed by the SUPERVISED pre-step, which runs this same file as the image''s true superuser (docs/deployment-runbook.md §6.3). This is the ADR-072 Decision-4 tripwire behaving as designed. If this fires and the pre-step did NOT run 118, the role carries a STALE or ABSENT comment — verify with shobj_description.', current_user;
  end if;
end
$g4$;
