...
--     (6) ONE UPDATE writes the payload, its version, the freeze instant, the owner
...
--   way.
--   ⚠ **THE ALTERNATIVE, COSTED, SO IT CAN BE TAKEN DELIBERATELY:** add
--   `monthly_report_finalization` to the vocabulary in this migration and emit
--   through `111`. It buys one thing the row cannot give — WHICH PATH finalized
--   (cron vs on-demand). It costs the one-way door above, and it **requires the
--   V1.final measurement's predicate to gain `surface_name =
--   'monthly_report_generation'` IN THE SAME PR**, or N is overcounted from the first
--   finalize. Routed to Sec joint-review and F/CTO; not taken unilaterally here.
--
-- ----------------------------------------------------------------------------
...
--   WRITES, in ONE transaction: `commentary_disposition`, then `rendered_payload` +
...
begin
  ...
  -- (6) FREEZE. One statement, same transaction: payload, its version (taken FROM the
