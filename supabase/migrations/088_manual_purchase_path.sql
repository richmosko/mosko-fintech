...
comment on function pfin.fn_create_manual_purchase(bigint, date, numeric, numeric, bigint, text, text, text, bigint, text, text) is
  ...
  'UNPRICED-BUT-LOUD: a global asset no provider has priced cannot be priced by this caller, so the position values at zero until one does. That state ships rather than blocking the purchase, on the condition that it is not silent — hence the composite return, which forces a caller wanting only the trans_id to project the flag away, a step visible in a diff (ADR-049 Decision 4, composite-return-over-documented-obligation). ⚠ THIS BODY NO LONGER DEFINES `priced` — it CALLS pfin.fn_asset_priced_flags (089), as does the account-detail read path, and 089 is where the predicate, its one named imprecision and the argument against inverting it are stated. That delegation is load-bearing, not tidiness: this body computed the flag inline while the read path reimplemented it in TypeScript, both claimed the predicate was ''reused verbatim'', and Sec found at the SELF-325 joint review that they had diverged — the read side picked the first row at the max date with no tiebreak, which at a same-date tie was NONDETERMINISTIC rather than merely different. Verbatim reuse across a SQL/TypeScript boundary is impossible by construction, so the claim could never have been true; 089 replaces an asserted agreement with a structural one. ⚠ If this call is ever replaced by an inline copy, 089 stops delivering its own premise and the divergence class returns silently. It is an indicator for the rendering layer, NOT a valuation primitive — nothing downstream may compute money from it. '
