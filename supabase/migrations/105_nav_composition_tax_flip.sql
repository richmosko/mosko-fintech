...
--   Before this migration `debt` was the ladder's ONLY sign flip: 051 emits a
--   positive magnitude and the consumer negates it. That stays true, and the two
--   tax rows JOIN it rather than change it.
--     • This function emits `debt` as a POSITIVE MAGNITUDE and the two tax
--       `amount`s with 104's SIGN UNCHANGED (no abs, no re-clamp, no negation).
--     • `nav` SUBTRACTS all three.
--     • The CONSUMER's ladder renders all three as subtractions — ONE negation,
--       in ONE place, for all three rows. There is no flip in this file.
...
as $$
  ...
  -- UNCHANGED; `nav` SUBTRACTS all three. The consumer's ladder renders each as a
  -- subtraction - exactly one negation, in exactly one place, for all three rows.
  -- A SECOND FLIP ANYWHERE RENDERS A CORRECT VALUE WITH THE WRONG SIGN, which is the
  -- double-negation route Sec M-3 names. Frontend adds no flip of its own for tax.
