...
begin
  ...
  if v_admin then
    execute pg_catalog.format('comment on role migrator is %L', v_text);
    raise notice '119: comment on role migrator re-issued as %.', current_user;
    return;
  end if;
  ...
  if v_live is null then
    raise exception using errcode = '42501',
      message = pg_catalog.format('migration 119 cannot be applied by %I AND the supervised pre-step did not land the comment: pg_shdescription carries NOTHING for role migrator.', current_user),
      detail  = 'COMMENT ON ROLE migrator requires superuser or the ADMIN option on the role, and Postgres refuses to grant any role the ADMIN option on ITSELF (GRANT migrator TO migrator errors "role migrator is a member of role migrator"), so no grant closes this. Skipping here would leave the role permanently undocumented with nothing observing it — the "does nothing, forever" failure ADR-072 Amendment 5 G4 refuses.',
      hint    = 'Run the pre-step step 2b: psql -U supabase_admin -f supabase/migrations/119_migrator_role_comment_amendment3_recitation.sql, then re-run the apply. Do NOT widen the migrator role to get past this.';
  elsif v_live is distinct from v_text then
    raise exception using errcode = '42501',
      message = pg_catalog.format('migration 119 cannot be applied by %I AND the comment on role migrator is STALE — present, but not the text this migration carries.', current_user),
      detail  = 'The supervised pre-step ran an OLDER revision of this file, or something else re-issued the comment afterwards. A stale catalog comment on a standing DDL credential is read at \du+ by an operator with no repo, so it is refused rather than skipped.',
      hint    = 'Re-run the pre-step step 2b with THIS revision of the file, then re-run the apply.';
  end if;

  raise warning 'G4-SKIP: comment on role migrator NOT re-issued by % — it holds neither superuser nor ADMIN OPTION on the role, and no grant can close that. SKIP IS VERIFIED, NOT ASSUMED: pg_shdescription was read and carries exactly this migration''s text, so the supervised pre-step demonstrably ran. This is the ADR-072 Decision-4 tripwire behaving as designed.', current_user;
end
$g4$;
