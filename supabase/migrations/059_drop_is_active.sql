...
--
--     (b) THE GRANULARITIES DIFFER, and this one is visible. `closed_at` is
--         timestamptz and `p_as_of` is a date, so `closed_at > p_as_of` compares
--         against MIDNIGHT: an account closed at 14:00 on p_as_of is INCLUDED,
--         one closed at 00:00 is EXCLUDED. The GATE, by contrast, proves its
--         legs at DATE granularity (`new.closed_at::date`). So the read is
--         timestamp-granular over a proof that is date-granular. On VALUE this
--         is still free — the account holds zero across the whole of that date
--         by the gate — but it IS visible in 049's ROW SET, where a same-date
--         afternoon closure yields a zero-valued card and a midnight one yields
--         no card at all. Recorded rather than changed: altering the predicate
--         is a behaviour change on a Sec-reviewed financial function and belongs
--         to this re-point's own review, not to a comment.
-- ----------------------------------------------------------------------------
