...
--       (acc.closed_at is null or acc.closed_at::date > p_as_of)
...
--
--   ⚑ THE `>` BOUNDARY IS INVARIANT-DEPENDENT, NOT ARBITRARY — and it is NOT
--     free in the way it first appears. Two separate points, and conflating
--     them is what makes this look like a style choice:
--
--     (a) `>` vs `>=` (exclude vs include an account closed AS OF exactly
--         p_as_of) is FREE ON VALUE, and free ONLY because of the standing
--         invariant: the gate proved zero holdings and zero cash as of
--         closed_at, so the account contributes zero on that date under either
--         boundary. Weaken any leg of the gate and the two boundaries stop
--         agreeing. Do not "tidy" this to `>=` for symmetry with some other
--         predicate — the equality is a consequence of a fence in another file.
--
--     (b) THE GRANULARITY IS RULED HERE, NOT INHERITED — `::date`, and it is a
--         REGRESSION FIX rather than a preference. An earlier revision of this
--         very comment recorded the mismatch and declined to change it. That was
--         wrong, and the measurement is why.
--
--         `closed_at` is timestamptz and `p_as_of` is a date, so a bare
--         `closed_at::date > p_as_of` promotes the date to MIDNIGHT. MEASURED against
--         what `is_active` said BEFORE this migration:
--
...
--
--         >> THE BARE FORM SILENTLY CHANGES CURRENT-STATE BEHAVIOUR AT THIS
--            MIGRATION. << `is_active` went false the instant `closed_at` was
--            set, so an account closed this afternoon left the §2.1.1 headline
--            IMMEDIATELY. Under the bare form it STAYS UNTIL MIDNIGHT.
--         And it is the UNIVERSAL case, not an edge: `fn_close_account` defaults
--         `p_closed_at` to `now()`, so EVERY app-closed account carries a
--         non-midnight time. A user closes an account and it is still on the
--         dashboard — which reads as "the close silently failed", on the one
--         control whose entire job is to be believed.
--         NAV VALUE is unchanged either way (the gate proved zero as of that
--         date), and THAT is what makes it dangerous: the regression is
--         invisible to every value assertion and surfaces only in 049's ROW SET
--         and in account COUNTS.
--
--         `::date` also puts the read at the SAME GRANULARITY AS EVERYTHING IT
--         READS: fn_holdings_as_of(date), fn_account_cash_as_of(date),
--         account_trans.transaction_date, both checkpoints' as_of_date, and the
--         gate's own `new.closed_at::date` legs. The time-of-day component of
--         closed_at is a precision NO OTHER SURFACE ACTS ON — letting it decide
--         row-set membership here would make an otherwise-ignored field
--         load-bearing in exactly one place.
--
--         REJECTED — `closed_at::date >= p_as_of` (an account closed ON p_as_of
--         stays visible for that day). Defensible: it existed that day, and leg 3
--         permits activity dated ON the closure date, so it may have transacted.
--         But it reintroduces the same-day "close didn't work" lag for EVERY
--         closure including midnight ones, and it disagrees with the retired
--         boolean it replaces.
--
--         ⚠ DO NOT "OPTIMIZE" THE CAST AWAY. `closed_at::date > p_as_of` is not
--           sargable where `closed_at >= (p_as_of + 1)` would be. That rewrite is
--           behaviourally EQUIVALENT and therefore safe — but the tempting
--           NEARBY form, simply dropping the cast, is the regression above.
--           pfin.account holds a handful of rows per tenant and carries no index
--           on closed_at: nothing to win, a silent behaviour change to lose.
-- ----------------------------------------------------------------------------
...
--
--   ⚠ REVERSAL, RECORDED RATHER THAN QUIETLY PERFORMED (Architect, 2026-08-03;
--     F/CTO-instructed). An earlier revision of the fn_compute_nav comment below
--     KEPT the stale "sound only at p_as_of = current_date" clause on purpose,
--     arguing: the strike is a FILED DECISION (the ADR-039 amendment, booked in
--     BACKLOG §7.7 as its own comment-only migration), and *"059 does not get to
--     absorb work booked elsewhere. 059 corrects only what 059 falsifies."*
--
--     THAT RULE IS RIGHT AND IT WAS APPLIED TO THE WRONG SIDE OF ITS OWN TEST.
--     059 RE-POINTS THIS FUNCTION'S BODY from a current-state boolean to an
--     as-of predicate. The stale clause asserts unsoundness BECAUSE the filter
--     was current-state. So 059 does not merely coincide with the strike —
--     >> 059 IS THE FALSIFIER. << By the file's own rule, correcting it here is
--     mandatory rather than optional.
--
--     HOW IT LOOKED LIKE SOMEONE ELSE'S WORK: the same correction was ALSO
--     booked, on a different basis (as an ADR-039 amendment). Two true
--     descriptions — "a decision filed elsewhere" and "a claim my own change
--     falsifies" — and only the second is controlling here. **A booking does not
--     transfer ownership of a falsification.** The scoping guard it invoked
--     (§7.6 S3) fences EDITS TO MERGED MIGRATIONS; it says nothing about a
--     comment on a function this very file is rewriting.
--
--     Consequence for the booked item: BACKLOG §7.7's F5 now records that this
--     comment rides 059, and the 052-shape comment-only migration keeps its
--     OTHER riders (046:167's stale counts, the six §10-count strips). It does
--     not disappear — it loses one rider.
--
...
--
--   ⚠ fn_compute_nav's comment is now corrected IN FULL — this paragraph
--     previously said "ONLY IN PART, deliberately … YOU MAY FIX HALF A COMMENT"
--     and is reversed above, for the reason given there: 059 IS the falsifier of
--     the sound-only clause, because 059 is what re-points the predicate the
--     clause reasons from. The half-fix was the file applying its own rule to
--     the wrong side of the test.
--
--     ⚑ THE SWEEP THAT FOUND THIS ONE IS WORTH KEEPING. After rewriting the
--       comment I grepped for the struck clause and got ZERO hits, which read as
--       done. A SECOND pattern over the same file found THIS paragraph — a
--       PROSE restatement of the same stale claim, in a rationale block, phrased
--       differently enough to escape the first pattern. Checkpoint 7, exactly:
--       the clean result was believed for as long as it took to run one more
--       query. >> A comment and its rationale are TWO sites, and fixing the
--       normative one while leaving the explanatory one produces a file that
--       argues against its own behaviour. <<
--
--   Both regenerated from obj_description(), diff-proved. Residual `is_active`
--   in fn_compute_nav is now EXPECTED-AND-SMALLER: historical back-references
--   only, since the preserved-ADR-039-clause exemption no longer exists.
-- ----------------------------------------------------------------------------
comment on function pfin.fn_account_unrealized_gl(date) is
  'SECURITY INVOKER per-account unrealized-G/L aggregation primitive (V1.1 "Net worth full"; PRD §2.1.5.a / SELF-224; shared with §2.2 asset-allocation V1.2; Lock 11 read-composition — clones the fn_compute_nav / fn_gl_entries posture). One row per account OPEN AS OF p_as_of visible to the caller (was: non-inactive; the boolean was RETIRED at 059 per ADR-042). DESIGN C (account-total, symmetric; ADR-038, F/CTO 2026-08-02). current_market_value (ALL types, uniform) = securities MV [Σ(fn_holdings_as_of qty × best-available eod_price[D-first LOCF] × fx→USD)] + cash [roll-forward balance × fx→USD] = the account''s fn_compute_nav contribution → Σ over ACTIVE accounts = fn_compute_nav OVER ACTIVE ACCOUNTS ONLY (049 filters by the as-of open/closed predicate per PRD §2.4.2 — closed_at is null or closed_at::date > p_as_of; fn_compute_nav/019 does not — they diverge only on a value-bearing inactive account; that 019 scope gap is tracked separately, and 049''s open/closed filter is correct). (securities & cash disjoint per the 017 CHECK — counted once). INVESTMENT-class (account_type ∈ investment/retirement/crypto, 003 CHECK): cost_basis = securities carried book [Σ(fn_gl_entries `trade_position` × fx→USD) — acquisition + basis_adjust − FIFO/specific-lot matched-sell removal; single basis truth == GL == tax == reports; unmatched sells leave basis on the books per 037 Suspense floor] + the SAME roll-forward cash term as mv (the shared cash figure → cancels; NOT fn_gl_entries asset_liability, a pure-ledger Σ that diverges when a checkpoint exists). REDEFINITION: cost_basis is the ACCOUNT-TOTAL BOOK (securities book + cash face), not AC#2-literal securities-only. unrealized_gl = current_market_value − cost_basis = securities MV − securities book (pure securities G/L; cash cancels exactly). NON-INVESTMENT (depository/manual_other/real_estate/liability): cost_basis = unrealized_gl = NULL (NOT 0 — concept-does-not-apply discriminator; a zero-everything investment account returns 0/0/0). AS-OF (Lock 15 Decision 19; server-derived-only per Lock 15 mod #2, V1.1 consumers pass CURRENT_DATE): the composed fns thread p_as_of → historical eod_price + account_trans / lot_match ≤ as_of. INVOKER → cross-tenant caller sees no rows (fails closed); unpriced asset → NULL term → dropped → 0, never NaN. Per-lot decomposition is an additive future helper (buy side already per-lot; removal detail preserved in immutable lot_match), reconcilable by construction — the GL-derived basis path does not foreclose lot-level UI (V2). set search_path=''''; NOT a DEFINER allowlist entry (INVOKER) — allowlist stays 4; §10 catalogued ledger UNCHANGED BY THIS OBJECT — and NO COUNT IS STATED HERE, deliberately. A ledger-impact claim is AUTHORING-TIME PROVENANCE: it belongs in a migration header, which is a dated artifact, not in a catalog comment, which reads as LIVE STATE. Read ADR-011 Decision 4 live. Decision-3 unchanged (no new FK column). Sec joint-review-mandatory (financial calc + multi-tenant); RLS verification → SELF-228 two-tenant battery. §2.1.6 MV-vs-COST-BASIS AUDIT-TRACE (SELF-227, comment-only — no body/signature/logic change): investment-account contributions to NAV use CURRENT MARKET VALUE (eod_price × qty × fx), NOT cost basis; cost basis is confined to 049.cost_basis / unrealized_gl (this function''s own investment-class output columns) + future §2.2.x cost-basis-display surfaces. PRD §2.1.6 / SELF-227.';

comment on function pfin.fn_compute_nav(date, boolean) is
  'SECURITY INVOKER uniform roll-forward net-worth read (ADR-027 §5 / Lock 11; SELF-322 / ADR-039). 019''s valuation VERBATIM + AS-OF OPEN/CLOSED scoping GATED on p_active_only (was is_active; the boolean was RETIRED at 059 per ADR-042 and the predicate is now closed_at is null or closed_at::date > p_as_of). p_active_only=FALSE → byte-identical to 019 (ALL accounts — the book/as-of engine: 037 GL memo + historical trend). p_active_only=TRUE → OPEN AS OF p_as_of (the §2.1.1 headline via netWorth.ts), and it is now TEMPORALLY SOUND AT ANY p_as_of. ⚠ THE OLD "SOUND ONLY AT p_as_of=current_date" CONSTRAINT IS STRUCK, NOT REWORDED (ADR-039 amendment, ratified by ADR-042): it rested on is_active being current-state, and closed_at IS temporal, so filtering it into a past as_of does not rewrite history — it is the one thing a boolean could never express. ⚠ AND p_active_only IS NOW A PROVABLE NO-OP ON VALUE: fn_compute_nav(d, true) = fn_compute_nav(d, false) for EVERY d. Past closure the account holds zero (gate legs 1+2 at closed_at, leg 3 plus the transfer-in fence after it); before closure it is not closed as of d and the predicate includes it. IT NONETHELESS STAYS, and the reason is recorded because a provably-no-op parameter READS AS DEAD CODE and deleting it is the next reader''s instinct: it is a PostgREST signature (removal is a breaking API change), and FALSE remains the semantically meaningful all-accounts-INCLUDING-CLOSED reading that the 037 memo path takes via the 1-arg wrapper — the equality of the two values does not make the two QUESTIONS the same. WHAT WOULD MAKE IT LOAD-BEARING AGAIN, stated per the symmetric rule (every exemption names its dependency; every fence states what makes it necessary) — the no-op property is a CONSEQUENCE OF FOUR FENCES IN OTHER FILES, and weakening ANY of them restores a divergence this parameter would then be selecting between: (1) the 058 close gate''s three legs; (2) the 058 transfer-in fence, which is what stops value arriving after closure; (3) the POSITION-NEUTRALITY of the two tables that fence EXEMPTS — account_trans_split by 029''s Sigma=parent, account_trans_annotation by 004 immutability; (4) this predicate remaining AS-OF rather than current-state. ⚠ (4) is the one with no footprint: the naive current-state re-point `acc.closed_at is null` is behaviourally IDENTICAL under V1 until a closed account exists, so the flip would be chosen silently — the QA assertion that true ≡ false at a PRE-CLOSURE date, where the shared contribution is non-zero, is the only thing that catches it. securities leg filters via LEFT JOIN pfin.account on holdings.account_id; cash leg via pfin.account — both gate on the as-of open/closed predicate ONLY when p_active_only, and the securities leg carries the FAIL-CLOSED conjunct acc2.account_id is not null (a LEFT JOIN miss must EXCLUDE, not include; the naive closed_at is null alone fails OPEN). Makes the ADR-038 foot-to-NAV invariant EXACT: Σ 049.current_market_value(active) = fn_compute_nav(as_of, true). INVOKER (cross-tenant → 0, fails closed); unpriced asset → NULL → dropped → 0, never NaN. set search_path=''''; NOT a DEFINER allowlist entry (stays 4); §10 catalogued ledger UNCHANGED BY THIS OBJECT — and NO COUNT IS STATED HERE, deliberately. A ledger-impact claim is AUTHORING-TIME PROVENANCE: it belongs in a migration header, which is a dated artifact, not in a catalog comment, which reads as LIVE STATE. Read ADR-011 Decision 4 live. Decision-3 unchanged. EXECUTE revoked from PUBLIC, granted to authenticated. §2.1.6 MV-vs-COST-BASIS AUDIT-TRACE (SELF-227, comment-only — no body/signature/logic change): investment-account contributions to NAV use CURRENT MARKET VALUE (eod_price × qty × fx), NOT cost basis; cost basis is confined to 049.cost_basis / unrealized_gl (+ future §2.2.x cost-basis-display surfaces). PRD §2.1.6 / SELF-227.';
