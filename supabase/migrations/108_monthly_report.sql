...
comment on column pfin.monthly_report.rendered_payload is
  ...
  'report, in-app and PDF alike, reads THIS, never a recomposition. ⚠⚠ "THE '
  'COMPOSITION HELPER''S RETURN" NAMES THE INTENDED WRITER, NOT AN ENFORCEABLE ONE, '
  'AND THE DIFFERENCE IS THE POINT (Sec FLAG-5, PR #636). Nothing in the database '
  'checks that this JSONB came from pfin.fn_render_monthly_report: the finalize path '
  'is SECURITY INVOKER by design, so under R4 (B) the write is performed BY THE '
  'TENANT''S OWN SESSION, and a tenant who reaches the column directly can store any '
  'JSONB their own RLS admits. ⚠ THAT IS NOT A GAP TO BE CLOSED HERE — a fence '
  'comparing the stored payload against a recomposition would have to recompose, '
  'which is precisely what freezing exists to avoid, and a DEFINER writer would '
  'compose one tenant''s report with another tenant''s visibility. SO THE PAYLOAD IS A '
  'SELF-ATTESTATION BY THE TENANT ABOUT THEIR OWN REPORT, and the tamper-evidence is '
  'the AUDIT ROW plus the immutability trigger: the audit row records that a '
  'generation happened, for which tenant and at what as-of, and cannot be edited or '
  'deleted; this column records what the tenant asserts the report said, and cannot '
  'be edited after finalization either. The integrity claim available here is '
  '"unchanged since finalization", NOT "faithfully composed" — do not read the '
  'stronger one, and do not write a comment elsewhere that implies it. ⚠ The ruling''s '
