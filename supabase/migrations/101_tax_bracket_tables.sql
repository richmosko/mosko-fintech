...
comment on column pfin.tax_bracket_schedule.schedule_label is
  ...
  'rejection) — self-inflicted, own row, no cross-tenant reach (Sec E-2); the same '
  'asymmetry covers interior control characters and ANSI escapes, which only the app '
  'fence refuses (Sec N-6) and which escaped render makes inert. Widening the '
