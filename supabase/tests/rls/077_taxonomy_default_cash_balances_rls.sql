...
-- │  (c) DOMAIN-AGNOSTIC REACH — the user set is `select distinct users_id  │
-- │      from pfin.user_taxonomy` with NO `domain=...` filter. A user whose │
-- │      ONLY pre-existing row is CASHFLOW-domain is still "already          │
-- │      provisioned" and IS reached — the statement text has no domain     │
-- │      conjunct to silently narrow. Tenant C below exercises exactly this.│
...
--   invoked). plan(14): 1 precondition (P1) + 2 reach (BF1-BF2) + 1 unreachable
...
select is(
  (select count(*) from pfin.user_taxonomy
    where domain='asset' and cat='Cash' and sub_cat='Cash Balances'
      and users_id in (:'ta', :'tc')),
  0::bigint,
  '(P1) precondition: neither A nor C carries a Cash Balances row before the backfill re-run'
);

-- =====================================================================
-- BACKFILL RE-RUN — 077's own two statements, verbatim, re-invoked. This is
-- the ONLY way to exercise the backfill against a non-empty user_taxonomy in
-- a scratch DB where 077 already ran once against zero users.
-- =====================================================================
...
select is(
  (select count(*) from pfin.user_taxonomy
    where users_id = :'ta' and domain='asset' and cat='Cash' and sub_cat='Cash Balances'),
  1::bigint,
  '(BF1) already-provisioned tenant A (asset-domain prior rows) receives exactly ONE Cash Balances row from the backfill re-run'
);
select is(
  (select count(*) from pfin.user_taxonomy
    where users_id = :'tc' and domain='asset' and cat='Cash' and sub_cat='Cash Balances'),
  1::bigint,
  '(BF2) DOMAIN-AGNOSTIC REACH: tenant C, whose ONLY pre-existing row is CASHFLOW-domain, is still "already provisioned" (the derivation carries no domain filter) and receives exactly ONE Cash Balances row'
);
...
-- =====================================================================
-- (BF4) NO-LEAK TOTAL (d) — exactly TWO Cash Balances rows exist cluster-
--   wide after the re-run (A + C), scoped at role=postgres, structural.
-- =====================================================================
select is(
  (select count(*) from pfin.user_taxonomy
    where domain='asset' and cat='Cash' and sub_cat='Cash Balances'),
  2::bigint,
  '(BF4) no-leak total: exactly 2 Cash Balances rows exist (A + C) after the backfill re-run — no stray row for B or any other users_id'
);
...
select is(
  (select users_id from pfin.user_taxonomy
    where domain='asset' and cat='Cash' and sub_cat='Cash Balances' and users_id = :'ta'),
  :'ta'::uuid,
...
select ok(
  not exists (
    select 1 from pfin.user_taxonomy
     where domain='asset' and cat='Cash' and sub_cat='Cash Balances' and users_id = :'tc'
  ),
  '(ISO2) under RLS as tenant A: C''s Cash Balances row is NOT visible — cross-tenant read fails closed'
);
...
select ok(
  ...
  '(ISO3) under RLS as tenant C (reverse direction): A''s Cash Balances row is NOT visible'
);
select set_config('role', 'postgres', true);
...
select is(
  (select count(*) from pfin.user_taxonomy
    where users_id = :'tc' and domain='asset' and cat='Cash' and sub_cat='Cash Balances'),
  1::bigint,
  '(IDEM2) idempotent on re-run: C''s Cash Balances row count stays exactly 1 after a second invocation'
);
select is(
