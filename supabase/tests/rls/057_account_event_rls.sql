...
-- (D6b) authors no SECURITY DEFINER function. Allowlist widened to 4 entries at `111`
-- (SELF-345 / AH, 2026-09-05): fn_emit_audit_log is the reserved slot's now-authored
-- occupant (ADR-011 Decision 9), added here alongside self209's (a1)/(a2)/(E-iii) so
-- this schema-wide query — checked against the WHOLE pfin schema, not just 057's own
-- objects — does not read 111's legitimate authoring as an escalation in THIS file.
select is(
  ...
  0,
  '(D6b) `057` authors ZERO SECURITY DEFINER functions -> the allowlist stays at its 4 authored entries; any escalation here is a Sec-joint-review ledger event, not a free ALTER'
);
