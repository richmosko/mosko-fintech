...
--       = 36 asset + 27 cashflow, migration-seeded).
...
--   taxonomy_default is migration-seeded (63 rows) — read, never re-seeded. All in a rolled-back
--   txn.
--
...
--   db-tests.yml) reaches this against the 001→041 reset stack. plan(16).
-- =====================================================================
...
--  - taxonomy_default is already populated by 041 (63 rows) — the provisioning source.
-- ---------------------------------------------------------------------
...
-- (1b) A now owns exactly 63 rows (the full committed default set copied through).
select is(
  (select count(*) from pfin.user_taxonomy where users_id = :'ta')::bigint,
  63::bigint,
  '(1b) owner provisioned all 63 default rows: A''s user_taxonomy count = 63 (every taxonomy_default row passed the INSERT WITH CHECK)'
);
...
-- (3a) A runs the SAME provision statement again → all 63 rows conflict → do nothing → no error.
select lives_ok(
  $$ insert into pfin.user_taxonomy (users_id, domain, cat, sub_cat, tax_relevant, tax_character, display_order, notes)
     select auth.uid(), domain, cat, sub_cat, tax_relevant, tax_character, display_order, notes
     from pfin.taxonomy_default
...
-- (3b) A STILL owns exactly 63 rows — the second run inserted 0 (no duplicates).
select is(
  (select count(*) from pfin.user_taxonomy where users_id = :'ta')::bigint,
  63::bigint,
  '(3b) idempotent re-provision: A''s count is STILL 63 after the second run — ON CONFLICT DO NOTHING double-seeds nothing'
);

-- =====================================================================
-- BLOCK 4 — taxonomy_default: global shared-read, no tenant scoping, both tenants identical.
-- =====================================================================
-- (4a) authenticated A reads the whole default set (using(true) global shared-read) = 63.
select _rls.set_tenant_aal(:'ta'::uuid, 'aal1');
select is(
  (select count(*) from pfin.taxonomy_default)::bigint,
  63::bigint,
  '(4a) taxonomy_default global shared-read: authenticated A SELECTs all 63 rows (using(true) — the provisioning source is readable to any authenticated user)'
);
select set_config('role', 'postgres', true);

-- (4b) authenticated B (mfa_policy='totp') at aal1 reads the SAME 63 rows — taxonomy_default is
...
select is(
  (select count(*) from pfin.taxonomy_default)::bigint,
  63::bigint,
  '(4b) taxonomy_default readable identically across tenants + NOT aal-gated: B (totp) at aal1 reads the same 63 rows — global reference data, no tenant predicate, no step-up gate'
);
select set_config('role', 'postgres', true);
