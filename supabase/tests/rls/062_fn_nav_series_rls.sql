...
select ok(
  (select regexp_replace(p.prosrc, '--[^' || chr(10) || ']*', '', 'g') !~ 'compute_nav'
     from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'pfin' and p.proname = 'fn_nav_series'),
  '(N2) …and it still holds with SQL COMMENTS STRIPPED, i.e. against approximately EXECUTABLE TEXT. ⚠⚠ CORROBORATING ONLY — NEVER PROMOTE THIS TO PRIMARY, AND NEVER RELAX (N1) IN ITS FAVOUR. ITS STRIPPER IS NAIVE BY CONSTRUCTION: it removes `--`-to-end-of-line and nothing else — no block-comment handling, and it would wrongly strip inside a string literal containing `--`. It is a regex pretending to be a lexer, and it is correct here only by accident of this body''s content, not by construction. THE ONLY REASON THAT IS SAFE: (N1) asserts the same property over RAW, UNSTRIPPED prosrc and is therefore STRICTLY STRONGER — anything this leg could miss, (N1) has already failed on — so (N2) can never be the sole gate for any input and its defect cannot produce a false pass. It earns its place solely as the leg that would still catch a real CALL if someone relaxed (N1) to permit a legitimate body comment. ⚑ A REDUNDANT LAYER THAT CANNOT FAIL INDEPENDENTLY IS THE THING LATER MISTAKEN FOR COVERAGE — the same defect as the retired (E3), as a wrongly-scoped fence reading green, and as a `docs(…)` prefix implying prosrc held. If (N1) is ever weakened, this leg does NOT inherit its authority: replace it with a real lexer or delete it');
select ok(
