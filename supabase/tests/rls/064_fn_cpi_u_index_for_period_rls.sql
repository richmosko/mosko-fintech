...
select ok(
  ...
  '(A2) LOAD-BEARING: PUBLIC does NOT hold EXECUTE. `create function` grants EXECUTE to PUBLIC by default, so 066''s explicit revoke is load-bearing, and its removal is SILENT: every behavioural leg in this file would stay green while every role in the cluster came to HOLD EXECUTE. ⚠ IF YOU ARE READING THIS BECAUSE IT WENT RED, TRIAGE IT AS A LEAST-PRIVILEGE REGRESSION, NOT AS A DATA EXPOSURE. This is NOT the whole of the access control. Per D9 there are THREE independent fences — this EXECUTE ACL, USAGE on schema pfin, and table-level SELECT on both source tables (the helper is SECURITY INVOKER, so the caller''s own privileges apply) — and holding EXECUTE is not the same as being able to CALL. anon holds neither of the other two, so a red here means roles that were ALREADY privileged gained a grant they should not have, which is worth fixing promptly and is not an incident. Escalating it as public exposure would be a false-severity alarm. ⚠ THAT DOWNGRADE RESTS ON A FACT ASSERTED ELSEWHERE, NOT ON THIS PROSE — the watchers are named so you can check them instead of trusting this sentence: 053 leg (h6) pins anon holding NO SELECT on pfin.cpi_u_index; 063 legs (h10)/(h11) pin anon holding NO SELECT on pfin.cpi_u_nonpublication and NO USAGE on schema pfin. All three run in the SAME required pgTAP lane as this file, so the event that would falsify the downgrade cannot land without reddening one of them. ⚠ IF ANY OF THE THREE IS ALSO RED, THIS DOWNGRADE IS VOID: the other fences are down and a red (A2) may be a real exposure. Both source tables are GLOBAL with `using (true)` SELECT policies, so a grant to anon is a plausible future change rather than an exotic one — which is why the downgrade is stated as CONDITIONAL on those legs rather than as a standing fact. Cited by leg label, not line number, because line numbers drift'
);
select ok(
