...
**Service topology skeleton — five Coolify `dockercompose` resources on one host, per ARCH §5's container topology. ⚠ Read the network note under the table BEFORE creating anything: inter-service reach is a property of DECLARED NETWORK ATTACHMENT, never of Coolify project membership.**

| Coolify service | Base Directory | Build pack | Notes |
|---|---|---|---|
| `app` (V1 web-app) | `api/` | **Compose** — `api/docker-compose.yaml`, carrying the `# fence-app-private-bind: target` sentinel (ADR-073, 2026-09-19: `app` moved off the plain Dockerfile+Base-Directory pack, retiring the last holdout; the committed compose is what makes its `expose:`-only/no-`ports:` shape lintable rather than a UI setting, and it is where the `external:` network attachment to the Supabase stack is declared). Built from `api/Dockerfile`, which is unchanged. | The only service assigned a public Domain. `expose:` is how Coolify's Traefik reaches it container-to-container without host publication — `ports:` and `network_mode: host` are fence violations, `expose:` and a Domain are not. Everything else in the fleet is internal-only. |
...
| `provider-sync` | `workers/provider-sync/` | **Compose** — [`workers/provider-sync/docker-compose.yaml`](../workers/provider-sync/docker-compose.yaml) (build pack must be Compose, not bare Dockerfile, per §7's own CA-1/CA-4 text: the committed compose file is what makes the admission port's `expose:`-only, no-`ports:`, no-Domain shape lintable rather than a UI setting nothing can check) | **CA-4 hard prerequisite (§7), RESTATED CORRECTLY (ADR-073): this service and `app` MUST be ATTACHED TO THE SAME DOCKER NETWORK** — internal DNS (`http://provider-sync:8081`) resolves across a shared network attachment, and **Coolify project membership grants no attachment at all.** Declare the attachment in each compose's own top-level `networks:` block (`external:`, naming the shared network), and verify resolution at creation, not after — an unattached placement is the exact failure shape §10 CA-2's smoke test exists to catch downstream. **Do NOT "fix" a resolution failure by assigning a public Domain or publishing a port.** |

⚠ **The one-project premise this paragraph used to carry is FALSIFIED — corrected 2026-09-19 at [ADR-073](../DECISIONS.md#adr-073). It previously read that all four services are created under one Coolify project because the `provider-sync` ↔ `app` internal-DNS dependency (CA-4) requires it. It does not. Coolify's unit of Docker-network identity is the APPLICATION, not the project:** every `dockercompose` resource gets its own network named for its own UUID, and project membership attaches nothing. **CA-4 is satisfied by `app` and `provider-sync` being attached to the same network — not by their sharing a Coolify project**, and the attachment is explicit or it does not exist.

**Measured, not argued.** `pfin-migrator` was created **inside the Supabase stack's own Coolify project** and still could not reach `db` until its committed compose declared the stack's network `external:`. After the attachment (§6.8 step 2, deployment `vpwlnr4t9o0mci7qkmsq7odf`): the container sits on both its own network and the stack's, `getent hosts db` resolves, `pg_isready -h db -p 5432` returns *accepting connections*. Same project, no reach, until an explicit attachment was declared. §4 (1d) and §10 CA-7 already said as much — *"confirmed **not** automatic and **not** project-scoped, has to be flipped on both sides"* — and this paragraph is what disagreed with them.

**Two mechanisms, and only one is used here.** *(b)* an **`external:` network declared in the service's own committed compose**, naming the target application's Docker network by name — **MEASURED WORKING on Coolify 4.3.18 (2026-09-19) and the standing convention**: it attaches exactly one new member, it stays in committed, reviewable config, and it is what every fleet compose does. *(a)* Coolify's **predefined-network toggle** (`settings.connect_to_docker_network`) on both resources — the other mechanism, NOT used here; `false` on every resource today; per [ADR-072](../DECISIONS.md#adr-072) Amendment 4 it attaches `db` to a network shared with *"every other resource"* toggled onto it, which is *"a real widening of `db`'s reachable-from set"*. **Do not adopt (a) without a written statement of what it widens and every resource then sharing that network named by name.**

**Coolify project and environment placement is therefore an organizational convenience with no connectivity or security meaning** — place resources wherever is legible, and let the `networks:` blocks decide what can reach what. Env-var wiring per service is §5 (STUB — not this PR).
...
| 6 | SCRIPTED (operator supplies the JWT) | `scripts/smoke-pfin-exposure.sh pfin-app --compose-service app --jwt <user-jwt>` — a fixed command since PR #833; the operator still supplies the token (it crosses via a 0600 seed file over SSH stdin, never either machine's argv), but the probe itself is no longer by hand. |
...
| Step | Status | Detail |
|---|---|---|
| (i) | SCRIPTED | `provision-app.sh --apply` — deletes the stale plain-Dockerfile `pfin-app` (asserting zero deployments + zero env names first, refusing otherwise), creates it fresh as `dockercompose` in the stack's own project/environment, and sets `APP_STACK_NETWORK_NAME`. Does NOT deploy. |
...
| (vii) | SCRIPTED (`.env` edit + `provision-vps.sh --apply`) then BY-HAND (the flag-flip authorship + the merge that re-exercises the trigger) | flip `DEPLOY_ON_SUCCESS` + re-exercise the trigger — §7 step 7 below, unchanged. |
...
1. **Recreate `pfin-app` as a `dockercompose` resource — deletes the stale shell, does not deploy.**
   ...
   ```
   [`scripts/provision-app.sh`](../scripts/provision-app.sh) (new, this ruling; sibling to `scripts/provision-migrator-app.sh`) resolves `pfin-supabase-stack`'s own live project/environment identity (never hard-coded — see the script's own header for the exact field-path mechanism and its stated provenance uncertainty), asserts the existing `pfin-app` resource has **zero deployments and zero env-store names** before deleting it (refuses otherwise — this script never destroys a resource that turns out to hold real state), creates it fresh as `dockercompose` (`base_directory=/api`, `docker_compose_location=/docker-compose.yaml`, branch `main`), and reads the stack's live Docker network to set `APP_STACK_NETWORK_NAME` on the new resource (unconditional overwrite, non-secret — same discipline as the migrator's `MIGRATOR_STACK_NETWORK_NAME`). **Record the printed `APP_STACK_NETWORK_NAME` value** — step 5 needs it as the literal argument to `--require-network`.
   **Idempotent**: re-running against an already-`dockercompose` `pfin-app` skips the delete and only re-asserts/re-sets the network var.
...
- **CA-2 — admission-endpoint external-reachability NEGATIVE smoke (SELF-212 Option-C; ship-block; DevOps-owned deploy assertion):** post-deploy, empirically assert the provider-sync admission endpoint (`:8081`) is **NOT** reachable from outside the private Docker network. This is the empirical backstop the limb-(a) env-signal heuristic (§7 CA-1) is only a proxy for — it covers the Coolify FQDN-var non-update fail-open case CA-1's regex is meant to catch.
  ...
  - **POSITIVE control (must SUCCEED); RESTATED CORRECTLY per ADR-073 (§3) — this is an attachment check, not a project-membership check:** from a sibling container **attached to the same Docker network as `provider-sync`** (each compose's own `external:` declaration), `http://provider-sync:8081` health path returns 2xx — proves internal reach works (so the negative result above is "correctly private," not "app simply down").
  - **CA-4 network-attachment check:** the positive control passing IS the same-network-internal-DNS assertion — if `http://provider-sync:8081` does not resolve from the api/ container, api/ and provider-sync are not attached to the same Docker network (fix the attachment before proceeding; Coolify project co-location grants no attachment at all; do NOT "fix" by assigning a public Domain).
...
- **CA-7 — Supabase datastore external-reachability NEGATIVE smoke (§4 (1d); ship-block; DevOps-owned deploy assertion).** Same shape as CA-2 above, different subject: post-deploy, empirically assert `api-gw` and `supavisor` are **NOT** reachable from outside the private Docker network, now that both are `expose:`-only (§4 (1d) — the fix for the live 2026-09-10 incident where `supavisor` came up bound to `0.0.0.0:5432`/`0.0.0.0:6543`, caught only by probing, not by any error). This is §1's `nmap` baseline's load-bearing counterpart — §1 checks before anything exists to answer; this checks after the stack is actually up and Docker has programmed its own NAT rules, which is the only point where the assertion means anything.
  ...
  - **POSITIVE control (must SUCCEED) — this doubles as the CA-4-style network-attachment prerequisite for §6, verify it BEFORE §6 runs, not after; RESTATED CORRECTLY per ADR-073 (§3) — this is an attachment check, not a project-membership check:** from a sibling container (the `app`/`workers/*` resources created at §6) **attached to the Supabase stack's own Docker network** (each compose's own `external:` declaration, per §7.1's `APP_STACK_NETWORK_NAME` convention), `http://api-gw:8000` and the pooler's service name on `5432`/`6543` must resolve and respond. **If they don't resolve, `app` (or the worker under test) is not attached to the Supabase stack's network** — the CA-4 failure shape (§7's own precedent for `provider-sync` ↔ `app`) — **fix the network attachment, do not "fix" it by assigning either Supabase service a public Domain, and do not "fix" it by moving the resource into the same Coolify project (project membership grants no attachment at all).** Confirm the attachment is present on both sides as part of this check, never assumed from Coolify project co-location.
...
| 12 | ✅ `pfin-app`/`pfin-supabase-stack` cross-project topology — **resolved**: F/CTO ruled option A, 2026-09-19 (`pfin-app` becomes a `dockercompose` resource with an `external:` Docker-network attachment to the stack — the migrator's own proven shape). Implemented this PR: `api/docker-compose.yaml`, `scripts/provision-app.sh`, `deploy-app.sh --require-network`/`--resolve-host`, §3's topology text corrected (ADR-073, Architect) to a declared-network-attachment model rather than Coolify-project-membership | F/CTO (ruled) / DevOps (implemented) / Architect (§3 correction) | §3 / §7.1 step 1 |
