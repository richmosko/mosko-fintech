...
| Step | Status | Detail |
|---|---|---|
| (i) | SCRIPTED | `push-production-secrets.sh` preflight, then `--apply --skip-missing-resource` |
| (ii) | BY-HAND read + SCRIPTED write | `PUBLIC_SUPABASE_ANON_KEY`'s value is read from the Supabase-stack Coolify resource's own env tab (non-secret, safe to view/copy — no scripted read path exists for a non-allowlisted name today, see the note at (ii) below); `PUBLIC_SUPABASE_URL` is a fixed literal, no read needed. Both written via one `coolify-env.sh set` call. |
| (iii) | SCRIPTED | `mint-supabase-jwt-keys.sh --apply --app-name pfin-app --verify-live` |
| (iv)–(v) | SCRIPTED | `deploy-app.sh pfin-app --expect-base-directory /api --apply --health-path /` — deploy, poll, on-box running-container read, and the health probe fold into this one command |
| (vi) | BY-HAND | authenticated `pfin`-relation smoke through the deployed app's own PostgREST path — needs a live shell (or a user JWT this record has no scripted way to mint pre-DNS/pre-signup); command given below, not a fixed one-liner this runbook can supply end-to-end |
| (vii) | BY-HAND | flip `DEPLOY_ON_SUCCESS` + re-exercise the trigger — §7 step 7 below, unchanged |
...
1. **Push the production_only secrets already mappable to `pfin-app`.**
   ...
   **Pushed to `pfin-app` this run:** `SUPABASE_SERVICE_ROLE_KEY` (RT-26 allowlist, sole app-side holder), `PDF_WORKER_SIGNING_KEY` (also targeted at `pdf-render`, skipped — that resource doesn't exist yet), `WORKER_ADMISSION_SHARED_SECRET` (also targeted at `provider-sync`, skipped), `DISCORD_WEBHOOK_URL` (also targeted at `etl`+`provider-sync`, skipped).
...
2. **Set the two non-secret app env vars.**
   ```sh
   BOX_IP=<box-ip> scripts/coolify-env.sh set pfin-app \
     PUBLIC_SUPABASE_URL=http://api-gw:8000 \
     PUBLIC_SUPABASE_ANON_KEY=<value> \
     --apply
   ```
   `PUBLIC_SUPABASE_URL=http://api-gw:8000` is a **fixed literal**, not derived per-deploy — `api-gw`'s Coolify-internal service DNS name and port (§4 (1d); the same address the CA-4/CA-7 same-project checks at §7/§10 already assert `app` must be able to reach). It requires `pfin-app` and the Supabase-stack resource to sit in the **same Coolify project** with `connect_to_docker_network` enabled on **both** sides — not automatic (§4 (1d)); verify before running this step, not after.
   `<value>` for `PUBLIC_SUPABASE_ANON_KEY`: read the Supabase-stack Coolify resource's **own** `ANON_KEY` entry from its Environment Variables tab and copy it verbatim — non-secret by construction (role:anon JWT, RLS + ADR-029 aal2-gated, publishable; §5), so viewing/copying it is not a leak. **Marked BY-HAND above because `coolify-env.sh`'s own `SET_ALLOWLIST` gate runs before its store-state read, so it cannot preflight-read a non-allowlisted stack-side name** — no scripted read path exists for this specific value today; flagged as a small follow-up, not built here. Whatever value is set here is provisional: step 3 overwrites it unconditionally with a freshly-minted JWT before this app resource is ever deployed (step 4), so a copy of the stack's current value is sufficient, not a fresh mint of its own.
...
3. **Mint the real keys onto `pfin-app` — LAST, always.**
   ...
   ```
   Quoting the ruled ordering verbatim (§5, "Two writers of `SUPABASE_SERVICE_ROLE_KEY` on `app`", Sec joint-review 2026-09-13): **"Required ordering: run `push-production-secrets.sh` FIRST, `mint-supabase-jwt-keys.sh --apply --app-name` LAST — because mint overwrites unconditionally, running it last guarantees the real key wins; the reverse order clobbers the real key with a `.env` value and the app comes up holding a wrong `service_role` key."** The same two-writer shape applies to `PUBLIC_SUPABASE_ANON_KEY` (step 2's value is a placeholder for exactly this reason — mint's own propagation step overwrites both `PUBLIC_SUPABASE_ANON_KEY` and `SUPABASE_SERVICE_ROLE_KEY` unconditionally when `--app-name` resolves).

4. **Deploy, poll, and read back the running container — one command.**
   ```sh
   BOX_IP=<box-ip> scripts/deploy-app.sh pfin-app --expect-base-directory /api --apply --health-path /
   ```
   [`scripts/deploy-app.sh`](../scripts/deploy-app.sh) (new, this PR) resolves `pfin-app`, refuses to proceed unless its live `base_directory` equals `/api` (the identity guard against a typo'd/stale name resolving to the wrong resource — strike-proven offline, `scripts/ci/fence-deploy-app-strikes.sh`), `POST /deploy`s, polls to a terminal state (same 90×4s ceiling as `coolify-env.sh --deploy` / `provision-migrator-app.sh`), then reads back the container's RUNNING state on the box (this container has no Dockerfile `HEALTHCHECK`, so a `health=healthy` filter would hang forever — RUNNING is the honest on-box predicate). `--health-path /` additionally curls the resource's own Coolify-assigned fqdn from the operator's machine and reports the HTTP status — best-effort, not fatal on a non-200 (the pre-DNS sslip.io-style placeholder fqdn may not route via Traefik yet; see the Flag below).

5. *(folds into step 4 via `--health-path /` — no separate command.)*

6. **`pfin`-exposure smoke through the deployed app's own Data-API path (BACKLOG.md §7.36 item 64).** No user has ever signed up (Q5: signup off, invite-only), so a real authenticated JWT is not available pre-invite — use the **anon-bearer, non-JWT variant** already exercised live and recorded at [`standup-log.md:695`](../docs/records/v1final/standup-log.md), executed from inside `pfin-app`'s own running container so the request originates from exactly where the app dials `api-gw`:
   ```sh
   ssh -o BatchMode=yes -i <automation-key> root@<box-ip> \
     "docker exec \$(docker ps --filter 'name=nzfkslmj8cm6ba86bdizuvd8' --filter 'status=running' --format '{{.Names}}' | head -1) \
      node -e \"const k=process.env.PUBLIC_SUPABASE_ANON_KEY; require('http').get({host:'api-gw',port:8000,path:'/rest/v1/user_settings?select=users_id&limit=1',headers:{apikey:k,Authorization:'Bearer '+k,'Accept-Profile':'pfin'}},r=>{let b='';r.on('data',d=>b+=d);r.on('end',()=>console.log(r.statusCode,b))})\""
   ```
   Expect `401 {"code":"42501","message":"permission denied for schema pfin"}` — per `standup-log.md:695`, this proves `pfin` **is** exposed (the request reached the schema, not a `PGRST106` schema-not-exposed refusal) **and** the B-1 anon-zero-grant fence holds. Once an authenticated user JWT exists (post-invite), re-run with a real `Authorization: Bearer <user JWT>` and expect `200` per §6.9 step 6 instead. Marked BY-HAND because it needs a live shell into the box and there is no fixed single command this runbook can supply that covers both the pre-invite and post-invite cases identically.
...
- **No scripted read path exists for a non-`SET_ALLOWLIST` Coolify env-store value** (step 2's `<value>` read). Small, real gap — noted rather than built around; a future `coolify-env.sh get`-style subcommand would close it. Not built in this PR (scope: the write path + its allowlist, not a new read primitive).
