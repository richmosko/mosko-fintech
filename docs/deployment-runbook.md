...
- **SCRIPTED** (`scripts/provision-migrator-app.sh` / `scripts/provision-supabase-stack.sh` — both ASSERT this on every run, and both SET it in their own create body). Both scripts call the same strike-proven predicate, `scripts/ci/check-source-commit-in-build.sh`, against a fresh `GET /api/v1/applications/<uuid>` read, on EVERY run (preflight prints the current value; `--apply` PATCHes to `true` and re-reads if it finds it off, and FAILS CLOSED, naming the field, if it is still not `true` after the PATCH — before triggering a deploy). The Coolify UI location above stays documented as the fallback if the API is unreachable, same convention as this runbook's other by-hand fallbacks.
- **Verify read:** MEASURED 2026-09-19 on Coolify 4.3.18: `GET /api/v1/applications/<uuid>` → the response carries it at `settings.include_source_commit_in_build` — read `false` before team-lead's live PATCH on `anz4uzfdumcfgc92wnfpov4i`, `true` after, over that exact path.
...
- **MEASURED 2026-09-19:** the first deploy of `pfin-migrator` (`scripts/provision-migrator-app.sh --apply`, §6.8 step 3) FAILED at exactly this FATAL, because the application was created with `include_source_commit_in_build=false` — the setting above was, until this date, a BY-HAND UI step with no scripted assertion, so nothing caught the gap before the deploy ran. Fixed live by a direct `PATCH /api/v1/applications/<uuid>` + read-back (`true`) to unblock the cutover, then scripted as described above so a stranger never hits this. **The network/DNS question this box's cutover was ultimately testing was NOT reached on that attempt** — the build never got far enough to exercise it.
...
- **`$PROD_DB_URL` — where it comes from, for each of the two shells that use it. Gap closed 2026-09-13 (standup step 6, Phase B): §4 and this section use the bare shell variable `$PROD_DB_URL` throughout without ever defining it — it is not an ambient env var on the box, the operator's Mac, or in any `.env` this repo commits.**
  ...
  - **The psql / `show server_version` verification snippets elsewhere in this file** (§4's bring-up verification, §4.1's TimeZone read-back) run against the **`db` service**, not the migrator — the migrator image deliberately carries no Postgres client (`infra/supabase/migrator/Dockerfile` installs `ca-certificates curl tar` only; `supabase db push` speaks the wire protocol directly). Use `docker compose --project-name <uuid> exec -T db psql -U postgres -d "$POSTGRES_DB" -c '…'`, which authenticates over the container's local socket and needs no password in any command line. **Do not add `postgresql-client` to the migrator image to make this bullet true** — the lean-image posture is deliberate.
    ...
    **The interactive §6.1 / §6.2 / §6.3 handoffs — use the helper, not the raw form (BACKLOG §7.36 item 33):**
    ...
    ```
    `scripts/db-shell.sh` reads `BOX_IP` and `SUPABASE_STACK_UUID` from the repo-root `.env` — `BOX_IP` is written there by `scripts/provision-vps.sh --apply`; `SUPABASE_STACK_UUID` by `scripts/record-coolify-uuids.sh --apply` (looks it up on the Coolify API by the resource's **name**, `pfin-supabase-stack`). Neither is a secret — an internal box address and a Coolify resource UUID.

    ⚠ **`SUPABASE_STACK_UUID`, not `MIGRATOR_SERVICE_UUID` — corrected 2026-09-19, live finding (§6.8 step 5).** This vehicle execs the `db` **service**, which lives in the Supabase-stack Coolify application regardless of where `migrator` lives. Before Amendment 4 moved `migrator` off the stack, `MIGRATOR_SERVICE_UUID` and the stack's own uuid were the same value, so reading either worked; after the move they diverge, and `db-shell.sh --as ...` reading `MIGRATOR_SERVICE_UUID` FAILED against the standalone migrator application's own project (no `db` service there at all) — that is exactly what F/CTO hit mid-§6.8. `MIGRATOR_SERVICE_UUID` is still correct for `--migrator-url` below (it execs `migrator` itself).

    **The raw form it wraps, for reference or if the helper is unavailable:** `docker compose --project-name <uuid> exec -it db psql -U postgres -d "$POSTGRES_DB"` (§6.1/§6.2) or `-U supabase_admin` (§6.3) — a TTY is required for the prompt, and that prompt is the whole mechanism. `<uuid>` here is `SUPABASE_STACK_UUID` — see above for where it comes from. ⚠ **If reaching any of these from off-box (F/CTO's own Mac, not an already-open box shell), the `ssh` hop itself needs `-t`** (`ssh -t root@<box-ip> 'docker compose ... exec -it db psql ...'`) — measured 2026-09-14: a bare `ssh` here gives `psql` no controlling terminal at all, silently, with no prompt and no error. `<box-ip>` is `BOX_IP` — see above. **The helper carries `-t` and both placeholders already filled in — this is the whole reason it exists.**
...
2. **`migrator` — do NOT generate a new value.** It is already minted (`provision-supabase-stack.sh`'s `MINT_SECRETS`, §6 above) and baked into the running container. Look it up from there, never printed to this record or any chat — **use the helper, which carries the OPERATOR-ONLY guard below by construction** (BACKLOG §7.36 item 33):
   ...
   ```sh
   ssh -t root@<box-ip> 'docker compose --project-name <migrator-app-uuid> exec -T migrator sh -c "echo \$PROD_DB_URL"'
   ```
   `<box-ip>` = `BOX_IP`; `<migrator-app-uuid>` = `MIGRATOR_SERVICE_UUID` (the migrator application's OWN uuid, not the stack's — see the ⚠ in the vehicle note above) — both from `.env`. The password is the substring between `migrator:` and `@db` in the printed URL.
...
**⚠ BUILT, BACKLOG.md §7.36 item 59 (ADR-072 Amendment 6 consequence (i)) — `scripts/migrator-orchestrate.sh` now DEPLOYS the migrator resource on every fire, before ever executing the Scheduled Task.** This discharges Decision 5(1)'s ratified-but-previously-unimplemented sequencing (rebuild → run task → deploy app — Amendment 6 (A)'s finding). Concretely, right after the pre-fire task-command integrity check: (1) a **name guard** — `GET /applications/$MIGRATOR_SERVICE_UUID` must name `MIGRATOR_APP_NAME` (default `pfin-migrator`), or the run refuses (exit 16) rather than deploy at a possibly-drifted uuid; (2) `POST /deploy?uuid=$MIGRATOR_SERVICE_UUID`, polled to a terminal Coolify deployment status (`finished` / `failed` / `cancelled-by-user`, measured from Coolify v4.3.18's own `ApplicationDeploymentStatus` enum) — **a precondition-wait, never a success criterion** (Amendment 6 consequence 3 — Decision 3's ruling on deployment-status reliability is unchanged); (3) a **pre-execute commit assertion** — the finished deployment's own `commit` field (its deployed-commit authority, per this section's own citation below) must equal `$MIGRATOR_EXPECT_SHA`, or the run refuses (exit 18) before ever executing the Scheduled Task. **A `main` advance between this fire's trigger and this deploy call RED's the commit assertion — correctly**, since Coolify's git-based deploy always rebuilds the CURRENT branch HEAD, never a pinned commit (measured — the public API's `/deploy` route has no commit-pinning parameter for a `dockercompose` application); the recovery is the newer fire already queued behind this one under the workflow's `concurrency` group, or a deliberate re-fire — not a relaxed assertion. **A hand redeploy of the migrator resource before firing is no longer a step a stranger needs to remember** (§6.6 Step 1.5 and §6.8 step 13's own text are updated to say so) — the orchestrator now does it on every fire. Wall-clock: a build now runs ahead of every apply (up to 15 minutes, `DEPLOY_POLL_MAX_ATTEMPTS`), so expect a fire to take longer than before — `.github/workflows/migrator-trigger.yml`'s job timeout was raised from 15 to 30 minutes for exactly this reason.
...
3. **⚠ CHANGED, BACKLOG.md §7.36 item 59 — expect exit 18, not exit 3, and expect it BEFORE any `db push` runs.** This recipe's own premise — dispatch with `MIGRATOR_EXPECT_SHA` set to a sha that PREDATES the image's actual current build — now gets caught by the NEW pre-execute commit assertion, not the old post-execution tag check: item 59's deploy-then-execute section deploys the migrator resource FIRST on every fire, and Coolify's git-based deploy always rebuilds the CURRENT branch HEAD (measured — the `/deploy` route has no commit-pinning parameter), so the freshly-deployed image's `commit` will be `main`'s CURRENT tip, not the older sha this run was dispatched with. The mismatch is caught at exit 18, **before the Scheduled Task ever executes.** **Expect:** the job goes **RED**, exit **18** — `migrator-orchestrate.sh`'s pre-execute commit-mismatch message, naming both the deployed commit and the sha this run was dispatched with. (The OLD exit-3 tag-based check, further below in the script, is unchanged and still fires in its own scenario — a container whose BAKED sha disagrees with what the Scheduled Task's own output reports at execution time, a narrower case than "the box hadn't rebuilt at all" that this recipe used to exercise.)

✅ **RESOLVED BY THE SAME CHANGE — this strike is NO LONGER a write against production.** The paragraph below is left for history; item 59's deploy-then-execute section means this specific strike now refuses (exit 18) before the Scheduled Task ever runs `supabase db push` — the mismatch is caught at the DEPLOY step, not after a real apply. The strike still triggers a REAL deploy of the migrator resource (a real rebuild, real wall-clock, a real Coolify deployment record) — that is not nothing — but it no longer touches the production database.
...
⚠ **This is a HAND pass — the automated trigger and this step's `redeploy-by-hand` are two different vehicles, and BACKLOG.md §7.36 item 59 only changed one of them.** Steps 1–2 of this re-bootstrap run BY HAND, outside `scripts/migrator-orchestrate.sh` entirely (Step 3 below is the first step that goes through the orchestrator). Item 59 made `migrator-orchestrate.sh` deploy the migrator resource on every fire it runs — it did **not** make this by-hand pass unnecessary, because this pass never calls that script. **What item 59 DOES change here: use `scripts/provision-migrator-app.sh --apply` for this step, not "Coolify UI → Deploy"** — the scripted form runs the same deploy-then-poll-then-verify sequence this step already checks for by hand below, and its own post-deploy verification (env-name confinement) is a bonus this step gets for free. If a real trigger fire (§6.7, or a real migration push) is what brings this box up instead of the manual re-bootstrap, the orchestrator's own deploy-then-execute section does this redeploy automatically and this step is not needed at all.
...
| 3 | SCRIPTED | `scripts/provision-migrator-app.sh --apply`. **Note, added 2026-09-19:** this step's own scripted run now also ASSERTS `settings.include_source_commit_in_build == true` (PATCH-and-fail-closed if not) before it deploys — see §4's "Source commit availability" note above for why; this is the setting whose absence FAILED this box's first real attempt at this step. |
...
3. **Create the migrator resource and its store, with a FRESH credential value.** `BOX_IP=<box-ip> scripts/provision-migrator-app.sh --apply`. This creates the `pfin-migrator` Coolify application (same project as the stack), mints `MIGRATOR_DB_USER`/`MIGRATOR_DB_PASSWORD` into its OWN store (a value that has never existed anywhere else — this is the "fresh value" the ROTATE decision below depends on), attaches its network per step 2's outcome, and deploys. **Pass criterion (built into the script's own "Verification" step, but restated here as the gate for this procedure):** the new `migrator` container's env carries `MIGRATOR_DB_USER`/`MIGRATOR_DB_PASSWORD`/`PROD_DB_URL`/`PGSSLMODE` and NONE of `POSTGRES_PASSWORD`/`JWT_SECRET`/`SERVICE_ROLE_KEY`/`VAULT_ENC_KEY`/`ANON_KEY`/`SECRET_KEY_BASE`. **STOP if this fails** — do not proceed to step 4 with a container that failed its own confinement check.
   ...
   **MEASURED 2026-09-19:** the first `--apply` run of this step created `pfin-migrator` (uuid `anz4uzfdumcfgc92wnfpov4i`) but its first DEPLOY FAILED at the Dockerfile's sha guard — `FATAL: GIT_SHA/SOURCE_COMMIT build-arg is empty` — because the application was created with `include_source_commit_in_build=false` (§4's "Source commit availability" note; at the time, a BY-HAND UI step with no scripted assertion). Fixed live by a direct API `PATCH` + read-back (`true`) to unblock the cutover; `scripts/provision-migrator-app.sh` now sets this field on create and ASSERTS it (PATCH-and-fail-closed) on every run, before deploying, so a stranger running this step from scratch does not hit this. **The network question step 2 above exists to answer was NOT reached on that attempt** — the build never got far enough.
...
5. **Supervised credential handoff — set the new resource's credential as the role's real password.** **Stays BY-HAND deliberately** — pasting the value at an interactive `psql` prompt is the only channel this runbook trusts for a credential in transit; a script that automated the paste would need to hold the value somewhere first, which is the exposure this step exists to avoid. As `supabase_admin` (the §6.3 shape — `scripts/db-shell.sh --as supabase_admin`), read the freshly-minted `MIGRATOR_DB_PASSWORD` off the box (`grep -m1 '^MIGRATOR_DB_PASSWORD=' /root/.pfin/migrator-app.env` — never paste it, never echo it into a transcript) and run `\password migrator` interactively, pasting the value at the prompt (never on the command line — same discipline as every other `\password` handoff in this runbook). **This is the ROTATE, not a carry** — the point is that the new resource's value has never been the role's password before this step, which is what makes the outcome of step 8 (old-credential-fails) a real test rather than a tautology (Sec's reasoning, [ADR-072](../DECISIONS.md#adr-072) Amendment 4: *"a carried value makes the remedy's success UNFALSIFIABLE"*).
   ...
   **Both sub-steps of this step run FROM THE OPERATOR'S MACHINE, not on the box — clarified 2026-09-19 (there is no repo checkout on the box; MEASURED that day: no `.git` under `/root`, `/opt`, or `/srv`).** `scripts/db-shell.sh --as supabase_admin` (the `psql` session itself) opens from the operator's machine, over `ssh -t`, exactly like every other `db-shell.sh` invocation in this runbook. The `grep -m1 '^MIGRATOR_DB_PASSWORD=' /root/.pfin/migrator-app.env` read is a SEPARATE hop, also from the operator's machine, over `ssh root@<box-ip> "grep -m1 '^MIGRATOR_DB_PASSWORD=' /root/.pfin/migrator-app.env"` — the file lives on the box, the command that reads it runs from off-box. Do the grep first (a second terminal, or before opening the `psql` session), have the value in view, then run `db-shell.sh --as supabase_admin` and paste it at the `\password` prompt.

   ⚠ **The `grep` hop is OPERATOR-ONLY, at a human terminal — an agent must NOT run it.** Unlike `scripts/db-shell.sh --migrator-url`, a raw `ssh ... grep` carries no mechanical guard: nothing about it refuses a non-TTY caller, so an agent running it writes `MIGRATOR_DB_PASSWORD` straight into a session transcript, which is precisely the 2026-09-14 disclosure and cost a rotation (§6.0 Step 0.2 carries the same warning for the same reason). The value also lands in this shell's scrollback — prefer a fresh shell, and close it when the `\password` handoff is done.
...
13. **The Amendment 6/7 orchestrator assertions re-fire once.** The next real trigger fire (a migration merge, or a manual dispatch per §6.7) exercises `migrator-orchestrate.sh`'s sha precondition and delivery assertion against the NEW resource for the first time. Expect the same shape as any other clean fire (§6.5's "STATUS UPDATE" entries) — a fire that behaves differently here would mean something about the cutover, not about the orchestrator, changed.
    ...
    ✅ **AS-EXECUTED, 2026-09-19, AND THE HAND STEP THIS SURFACED IS NOW GONE.** The first fire (run `35467316084`) went RED at the sha gate: the migrator image had been baked at `ec245418`, but the run was triggered for `463ae8d9` — two doc PRs had merged since the last build, and nothing before item 59 rebuilt the image as part of firing. The recovery at the time was a HAND `scripts/provision-migrator-app.sh --apply` (at `9151b40d`), after which the re-fire (run `35467590683`) went GREEN. **BACKLOG.md §7.36 item 59 (this section's own PR) is what removes that hand step**: `migrator-orchestrate.sh` now deploys (rebuilds) the migrator resource itself on every fire, before ever executing the Scheduled Task, so a `main` that advanced since the last build no longer needs an operator to notice and redeploy by hand — the orchestrator's own deploy call picks up the current branch HEAD every time. A future stale-image case now surfaces as the pre-execute commit assertion (exit 18) naming both shas, not as a silent apply against an old image.
...
**First-deploy PROCEDURE — numbered, stranger-runnable (F/CTO gate, 2026-09-19: "this should be a procedure that can be run by a stranger with minimal by-hand intervention"; same gate that produced §6.9 and `scripts/coolify-env.sh`).** This is the procedure the bullets above ground — it does not restate their reasoning, it sequences them. Preconditions: `pfin-app` already exists as a Coolify resource (measured live 2026-09-19: uuid `nzfkslmj8cm6ba86bdizuvd8`, `base_directory=/api`, branch `main`, 0 deployments ever, 0 env names in its store, status `exited:unhealthy`), and §6's migrations + role handoffs have run.

**SCRIPTED / BY-HAND audit.**

| Step | Status | Detail |
|---|---|---|
| (i) | SCRIPTED | `push-production-secrets.sh` preflight, then `--apply --skip-missing-resource` — **read the preflight's `MISSING from local .env` line before `--apply`** (Sec N2): the four names this step's own text names as "pushed" are conditional on all four being present in the operator's local `.env`; the script skips (prints, does not fail) any that are absent. |
| (ii) | SCRIPTED (+1 network precondition read, folded into the same command) | `deploy-app.sh pfin-app --expect-base-directory /api --require-network-with pfin-supabase-stack` (preflight-only — no `--apply`) reads and asserts the same-project/environment + `connect_to_docker_network`-on-both-sides precondition BEFORE the literal write below, THEN `coolify-env.sh set pfin-app PUBLIC_SUPABASE_URL=http://api-gw:8000 --apply` sets the fixed literal. Corrected (Sec F3, PR #833 joint review): an earlier draft of this row said "no read of any kind needed," which was true of the literal-write command but false of the step as a whole — the network precondition it depends on had no command at all. |
| (iii) | SCRIPTED | `mint-supabase-jwt-keys.sh --apply --app-name pfin-app --verify-live` — mints and propagates the real `PUBLIC_SUPABASE_ANON_KEY` / `SUPABASE_SERVICE_ROLE_KEY` onto `pfin-app`, LAST. **Never hand-`coolify-env.sh set PUBLIC_SUPABASE_ANON_KEY`** (Sec N1) — this step is its sole and authoritative writer; a hand-set value would only ever be visible until this step next runs, and inviting the practice defeats the point of the allowlist carve-out being non-secret-but-still-mint-owned. |
| (iv)–(vi) | SCRIPTED | `deploy-app.sh pfin-app --expect-base-directory /api --require-env PUBLIC_SUPABASE_URL,PUBLIC_SUPABASE_ANON_KEY,SUPABASE_SERVICE_ROLE_KEY --apply --health-path /` — the names-only post-mint presence assertion, deploy, poll, on-box running-container read (refuses if more than one container is RUNNING — Sec F4), and the health probe all fold into this one command |
| (vii) | SCRIPTED | `smoke-pfin-exposure.sh pfin-app` — the pre-invite anon-bearer `pfin`-exposure smoke, runs unattended, exits non-zero on anything but the expected 401/42501 (exit 2 on a precondition the smoke itself can't attempt under, e.g. a connection failure — distinct from exit 1's classified-failure-shape meaning) |
| (viii) | SCRIPTED (`.env` edit + `provision-vps.sh --apply`) then BY-HAND (the flag-flip authorship + the merge that re-exercises the trigger) | flip `DEPLOY_ON_SUCCESS` + re-exercise the trigger — §7 step 7 below, unchanged. Corrected (Sec Q7, PR #833 joint review): an earlier draft marked this row bare BY-HAND, which under-claimed — two of its four sub-actions (`provision-vps.sh --apply`, which is idempotent and scriptable) are already scripted; only the `.env` edit's authorship and the actual merge-and-watch are irreducibly human. |

1. **Push the production_only secrets already mappable to `pfin-app`.**
   ```sh
   BOX_IP=<box-ip> APP_RESOURCE_NAME=pfin-app scripts/push-production-secrets.sh --skip-missing-resource                  # preflight
   BOX_IP=<box-ip> APP_RESOURCE_NAME=pfin-app scripts/push-production-secrets.sh --apply --skip-missing-resource
   ```
   `APP_RESOURCE_NAME` overrides the script's `app` default to the real resource name, `pfin-app` — the script's own default would otherwise resolve nothing. `--skip-missing-resource` is required, not optional, at this stage: `etl`/`pdf-render`/`provider-sync` don't exist yet, so a bare `--apply` would abort naming them. Expect **exit 3** (a partial run) — that is the CORRECT outcome here, not a failure; treat only exit 1/2 as real errors (see the script's own `--help` for the code table).
   **Pushed to `pfin-app` this run — conditional on presence in the operator's local `.env` (Sec N2, PR #833 joint review):** `SUPABASE_SERVICE_ROLE_KEY` (RT-26 allowlist, sole app-side holder), `PDF_WORKER_SIGNING_KEY` (also targeted at `pdf-render`, skipped — that resource doesn't exist yet), `WORKER_ADMISSION_SHARED_SECRET` (also targeted at `provider-sync`, skipped), `DISCORD_WEBHOOK_URL` (also targeted at `etl`+`provider-sync`, skipped). **Read the preflight's own `MISSING from local .env` line before `--apply`** — this script skips (prints, does not fail) any `production_only` name with no value in the operator's `.env`, so "pushed" above is a claim about the mapping table, not a guarantee any one of these four actually lands; `WORKER_ADMISSION_SHARED_SECRET` absent fails **closed** at the admission endpoint by the manifest's own ruling (no exposure), but the other three should be confirmed present before trusting step 4's `--require-env` to be checking a value this step actually supplied.
...
2. **Assert the network precondition, then set the one non-secret app env var this step needs.**
   ```sh
   BOX_IP=<box-ip> scripts/deploy-app.sh pfin-app --expect-base-directory /api \
     --require-network-with pfin-supabase-stack
   BOX_IP=<box-ip> scripts/coolify-env.sh set pfin-app PUBLIC_SUPABASE_URL=http://api-gw:8000 --apply
   ```
   The first command (preflight-only — no `--apply`) is `--require-network-with`'s own scripted read of the precondition the second command's literal depends on (Sec F3, PR #833 joint review — this row previously stated the precondition in prose with no command to assert it). It refuses, naming the failing side, unless `pfin-app` and `pfin-supabase-stack` share the same Coolify project/environment AND both carry `connect_to_docker_network == true` — not automatic (§4 (1d)). ⚠ **FIELD-PATH PROVENANCE, stated not overclaimed:** the two API fields this check reads (`environment_id`, `settings.connect_to_docker_network`) are inferred by analogy to the sibling `settings.include_source_commit_in_build` field this repo has already measured live, but have **not themselves** been independently confirmed against a real `GET /applications/<uuid>` response — same posture that field carried before its own live measurement. If the operator's run disagrees with what this check expects, that is new information about the correct field path, not evidence the precondition itself is wrong.

   ⚠ **MEASURED 2026-09-19 (team-lead), read-only via the box — this command WILL currently refuse, correctly.** `pfin-app` (`nzfk…`) is in Coolify project 2 / environment 2; `pfin-supabase-stack` (`nz7m…`) is in project 1 / environment 1; `connect_to_docker_network` is FALSE on both. `http://api-gw:8000` therefore cannot resolve from `pfin-app` today. This CONTRADICTS §3's stated premise ("all four services under one Coolify project… a per-project network is what `db` service-name DNS resolves on") — and matches the migrator's own prior evidence (same project as the stack, and it STILL needed the `external:` network to reach `db`, per §6 mechanism (b)). **This is an open topology ruling for F/CTO, not resolved by this PR**: (A) make `pfin-app` a Compose resource with an `external:` network to the stack (the migrator's own proven shape, straightforward to apply here since `pfin-app` has never deployed and can be recreated with no data loss), or (B) toggle `connect_to_docker_network` on both sides (the mechanism ADR-072 Amendment 4 graded as a real widening of `db`'s reachable-from set). **Step 2's literal (`http://api-gw:8000`) and this section's "same Coolify project" framing are UNCHANGED pending that ruling** — the `--require-network-with` command above is built to go RED against exactly this live state and say why, which is the correct behavior of a fail-closed precondition check, not a defect in the check.
   `http://api-gw:8000` is a **fixed literal**, not derived per-deploy — `api-gw`'s Coolify-internal service DNS name and port (§4 (1d); the same address the CA-4/CA-7 same-project checks at §7/§10 already assert `app` must be able to reach).
   **`PUBLIC_SUPABASE_ANON_KEY` is deliberately NOT set here, and never should be by hand.** Step 3's mint is its sole and authoritative writer and overwrites it unconditionally regardless of what (or whether) it was set beforehand — corrected from an earlier draft of this procedure that set a "provisional" copy here for no benefit: a value overwritten before it is ever read by a deployed container is dead work, not a safety margin, and it carried a BY-HAND Coolify-dashboard read this step doesn't need. Step 4's `--require-env` closes the actual gap that draft was reaching for (proving the value exists AFTER mint, not before). **Never run `coolify-env.sh set pfin-app PUBLIC_SUPABASE_ANON_KEY=...` by hand** (Sec N1) — it is in `SET_ALLOWLIST` for the manifest-refusal's own belt-and-braces reasons, not as an invitation to write it there.

3. **Mint the real keys onto `pfin-app` — LAST, always.**
   ```sh
   BOX_IP=<box-ip> scripts/mint-supabase-jwt-keys.sh --apply --app-name pfin-app --verify-live
   ```
   Quoting the ruled ordering verbatim (§5, "Two writers of `SUPABASE_SERVICE_ROLE_KEY` on `app`", Sec joint-review 2026-09-13): **"Required ordering: run `push-production-secrets.sh` FIRST, `mint-supabase-jwt-keys.sh --apply --app-name` LAST — because mint overwrites unconditionally, running it last guarantees the real key wins; the reverse order clobbers the real key with a `.env` value and the app comes up holding a wrong `service_role` key."** The same two-writer shape applies to `PUBLIC_SUPABASE_ANON_KEY` — this is mint's OWN write of that name (not a second writer racing step 2, which never touches it), so this step is the sole and authoritative source for both values on `pfin-app`.

4. **Assert the required names, deploy, poll, and read back the running container — one command.**
   ```sh
   BOX_IP=<box-ip> scripts/deploy-app.sh pfin-app --expect-base-directory /api \
     --require-env PUBLIC_SUPABASE_URL,PUBLIC_SUPABASE_ANON_KEY,SUPABASE_SERVICE_ROLE_KEY \
     --apply --health-path /
   ```
   [`scripts/deploy-app.sh`](../scripts/deploy-app.sh) (new, this PR) resolves `pfin-app`, refuses to proceed unless its live `base_directory` equals `/api` (the identity guard against a typo'd/stale name resolving to the wrong resource), THEN refuses to proceed unless all three named env vars are present in the store BY NAME (`--require-env` — a names-only `GET .../envs` check, no value is ever read/compared; the guard against deploying a container that will throw at its first request because step 2/3 didn't land) — both guards strike-proven to refuse BEFORE any `/deploy` call, offline (`scripts/ci/fence-deploy-app-strikes.sh`). Only then does it `POST /deploy`, poll to a terminal state (same 90×4s ceiling as `coolify-env.sh --deploy` / `provision-migrator-app.sh`), and read back the container's RUNNING state on the box (this container has no Dockerfile `HEALTHCHECK`, so a `health=healthy` filter would hang forever — RUNNING is the honest on-box predicate). `--health-path /` additionally curls the resource's own Coolify-assigned fqdn from the operator's machine and reports the HTTP status — best-effort, not fatal on a non-200 (the pre-DNS sslip.io-style placeholder fqdn may not route via Traefik yet; see the Flag below).

5. *(folds into step 4 via `--require-env` — no separate command.)*

6. *(folds into step 4 via `--health-path /` — no separate command.)*

7. **`pfin`-exposure smoke through the deployed app's own Data-API path (BACKLOG.md §7.36 item 64).**
   ```sh
   BOX_IP=<box-ip> scripts/smoke-pfin-exposure.sh pfin-app
   ```
   [`scripts/smoke-pfin-exposure.sh`](../scripts/smoke-pfin-exposure.sh) (new, this PR) resolves `pfin-app`, `docker exec`s into its running container (a plain-Dockerfile-pack resource — no compose project exists to address with `docker compose exec`, so this reuses `deploy-app.sh`'s own `docker exec`-by-uuid primitive), and issues an anon-bearer request against a `pfin` relation with `Accept-Profile: pfin` — the anon key crosses via the container's OWN env, never the wire, never printed. No user has ever signed up (Q5: signup off, invite-only), so this default mode is the only one available pre-invite; it exits **0** on the expected `401 {"code":"42501",...}` (per [`standup-log.md:695`](../docs/records/v1final/standup-log.md): proves `pfin` **is** exposed — not `PGRST106` — **and** the B-1 anon-zero-grant fence holds) and **non-zero** on anything else, including a `200` (treated as a security anomaly, not a pass — strike-proven, `scripts/ci/fence-smoke-pfin-exposure-strikes.sh`). Once an authenticated user JWT exists (post-invite), re-run with `--jwt <user-jwt>` and it instead expects `200` per §6.9 step 6 — the two modes do **not** accept each other's success shape (a `401` that passes anon-mode FAILS jwt-mode, and vice versa).

8. **§7 step 7 — flip the `DEPLOY_ON_SUCCESS` gate, then re-exercise the trigger once.** Once `app`'s first (manual) deploy above is confirmed healthy and the migrate-only leg of §6.5 Phase D step 12 has passed (the `DEPLOY_ON_SUCCESS=0` suppression line, green job): set `DEPLOY_ON_SUCCESS=1` in `.env`, re-run `BOX_IP=<box-ip> scripts/provision-vps.sh --apply` (idempotent — rewrites only `/etc/pfin/migrator-trigger.conf`), then **push a second no-op migration and watch the trigger fire again**, confirming the orchestration script's `api GET "/deploy?uuid=$APP_UUID"` call actually fires this time. ⚠ **Do not skip the re-exercise** — Sec's condition on this gate: the deploy leg must not ship unexercised; flipping the flag without a second live fire is a qualifier nobody checked.
...
| 12 | `pfin-app` and `pfin-supabase-stack` are in DIFFERENT Coolify projects/environments with `connect_to_docker_network` FALSE on both — MEASURED 2026-09-19 (team-lead), contradicts §3's stated "one Coolify project" premise. §7.1 step 2's `http://api-gw:8000` literal will not resolve until this is ruled. Options: (A) `pfin-app` becomes a Compose resource with an `external:` network to the stack (the migrator's own proven shape — no data loss, `pfin-app` has never deployed); (B) toggle `connect_to_docker_network` on both sides (a real widening of `db`'s reachable-from set, ADR-072 Amendment 4). **One-way-door-adjacent (changes the fleet's network topology) — F/CTO ruling required before §7.1 step 1 can execute past step 2.** | F/CTO | §3 / §7.1 step 2 |
