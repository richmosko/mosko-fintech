...
- **First bootstrap (supervised, until SELF-395) — rewritten 2026-09-16, ownership by construction.** §6.3's supervised pre-step (`supabase_admin`, minimal — creates the `migrator` role via `118`'s own file, flips database ownership, sets the password, flips `LOGIN`) runs **before** any migration applies → **then `migrator` itself applies the full 001–118 set from its own container**, so every object it creates is `migrator`-owned from the first row → **then the interactive §6.1/§6.2 `\password` handoffs below** → §5 env → §7 containers. E cannot perform the interactive handoffs yet; SELF-395 (client-side SCRAM scripting, SECURITY-GATED, not yet built) is what will let it own the full hands-off bootstrap.
...
**On this image, `postgres` is NOT a superuser — measured live, 2026-09-14.** `select rolsuper from pg_roles where rolname='postgres'` → `f` (it does hold `rolcreaterole`/`rolcreatedb`, which is why §6.1/§6.2 below still work as `postgres` — `\password` only needs `CREATEROLE`/`ADMIN OPTION` on the role, per each section's own "Operator privilege" line). Its prompt is `postgres=>`. The true superuser on this image is `supabase_admin` (`rolsuper=t`, prompt `postgres=#`) — **§6.3's `ALTER DATABASE … OWNER TO migrator` requires actual superuser and fails as `postgres`** (`ERROR: must be able to SET ROLE "migrator"`, measured live). **For simplicity, run all three of §6.3's statements as `supabase_admin`**, not `postgres` — §6.3 below is corrected accordingly. §6.1/§6.2 are unaffected and stay as `postgres`.
...
### 6.3 `migrator` role provisioning and first bootstrap — SUPERVISED pre-step, then `migrator` applies the full set

**⚠ SUPERSEDED IN DRAFT — pending Amendment 5. Do NOT execute the procedure below.** Sec's ADR-072 Decision A ruling (2026-09-16) accepts a DIFFERENT shape (A2, `pfin_owner`) for the durable fix and VETOES two legs of the shape this section currently describes (A1) — most sharply, moving `055`/`116`'s role creation into the `migrator`-run apply is vetoed outright (an applier that creates `pfin_etl`/`pfin_provider_sync` auto-inherits ADMIN OPTION on them and can `SET ROLE` into `service_role` transitively — the exact reach C8 exists to bound). This section stays in the repo as the record of what was drafted and why it does not survive Decision A; it will be rewritten again once Architect's Amendment 5 (`pfin_owner`, role creation and `service_role` grants relocated to the supervised pre-step per Decision A §3) lands. **A stranger must not run this procedure as written.**

**Rewritten 2026-09-16 (F/CTO-ruled, Sec-accepted at first pass; since SUPERSEDED by Decision A — see the notice above): ownership by construction, not by a later transfer.** The prior shape of this section — `postgres` (later corrected to `supabase_admin`) runs the whole first bootstrap, then a supervised handoff flips ownership onto `migrator` after the fact — reliably left every `pfin` object, and `supabase_migrations` itself, owned by whichever role ran the apply. On this box that produced a real gap (§7.36 item 32, `supabase_migrations`, and its wider cousin — every `pfin` table/function/view/sequence/type, measured 2026-09-16: **39 tables, 97 indexes, 34 sequences, 7 views, 116 functions, 3 enum types, all `postgres`-owned, zero `migrator`**). **The fix is ordering: create the bounded `migrator` role FIRST, then let `migrator` itself run the apply that creates everything else — so nothing is ever created by a role other than the one that will own it.** *(ADR-072 Decision 4 + Amendments 1/4/5; Sec joint-review, migrator-ownership consult, 2026-09-16.)*

**Why `118` cannot be the first thing `migrator` runs.** Migration [`118_migrator_role.sql`](../supabase/migrations/118_migrator_role.sql) is what CREATES `migrator` — **NOLOGIN, NOINHERIT, CREATEROLE, with NO password**, inert. A role cannot create itself, so `118` must be applied by something else, once, before `migrator` exists at all. That "something else" is a **minimal, supervised pre-step run by `supabase_admin`** — see §6.0 for why `supabase_admin`, not `postgres` (the true superuser on this image; `postgres` measures `rolsuper = f`).

**Why the pre-step runs `118`'s own file, not a hand-written copy of it.** Sec's condition: create the role with `118`'s exact attribute set, in a way that keeps `118` the single source of truth and cannot drift. A separate hand-maintained `CREATE ROLE …` statement here would be a second copy of `118`'s attribute list, and the two would silently disagree the next time either one changes. Instead, **the pre-step runs `118`'s literal file** — the same bytes the migration set carries, executed once outside the CLI's own tracking. `118` creates only the `migrator` role, its `comment on role`, and its own C8 attribute assertion (verified: no other schema, table, or grant in that file) — so running it directly is genuinely minimal, and there is no second copy to drift.

**Ordering — do not reorder:**

> **pre-step, `supabase_admin`, interactive: run `118`'s file → `ALTER DATABASE … OWNER TO migrator` → `\password migrator` → `ALTER ROLE migrator LOGIN`** → **`MIGRATOR_DB_*` env already minted on-box (`provision-supabase-stack.sh` MINT_SECRETS, §6's credential bullet) and injected into the migrator service (§5)** → **`migrator` applies the FULL migration set (001–118, including a second, tolerant pass over `118` itself) from its own container, using its own baked `PROD_DB_URL` — no override** → **§6.1/§6.2 worker-role handoffs, as today** → **§7 containers**

**The pre-step.** Run **once** against the target database, in an interactive `psql` session, **as `supabase_admin`** (never from a committed file for the `\password` line; `118`'s own file IS the committed file for the role-creation line, and that is the point):

```sh
# (0) 118's own file, executed directly — the single source of truth for
#     migrator's attribute set. Minimal: this file creates ONLY the role,
#     its comment, and its own C8 assertion.
psql -U supabase_admin -d <app_db> -f supabase/migrations/118_migrator_role.sql
```
```sql
-- (1) superuser-only; migrator is not CREATEDB and cannot run this itself
ALTER DATABASE <app_db> OWNER TO migrator;
-- (2) prompts; verifier computed CLIENT-SIDE; role still NOLOGIN -> inert
\password migrator
-- (3) carries no secret
ALTER ROLE migrator LOGIN;
```

`\password` sets **only** the password — the credential lands while the role is still `NOLOGIN`, and `LOGIN` then flips onto an already-credentialed role, so **LOGIN-with-no-password never exists at any instant** (the state `118` is built to prevent). The single-statement form `ALTER ROLE migrator WITH LOGIN PASSWORD '…'` is **PROHIBITED** (Sec B10) — statement logging writes it to the server log in cleartext, and it lands in `~/.psql_history`. Generate the password with `openssl rand -hex 32` if minting fresh; **in production the value is already minted on-box** by `provision-supabase-stack.sh`'s `MINT_SECRETS` as `MIGRATOR_DB_PASSWORD` — look it up, don't regenerate it (`scripts/db-shell.sh --migrator-url --i-am-a-human`, §6.0 Step 0.2), and set `\password` to match. **Operator privilege:** the pre-step's three DDL statements all require true superuser (`supabase_admin` on this image, not `postgres` — measured, `ALTER DATABASE … OWNER` fails as `postgres` with `ERROR: must be able to SET ROLE "migrator"`).

**⚠ If a prior `postgres`-run (or `supabase_admin`-run) apply already exists on this box — as it does today — wipe it FIRST, before the pre-step, so the re-apply creates everything fresh under `migrator`.** Run as `supabase_admin`, after confirming the gates below:

**Gates — measured, not assumed, before any DROP:**
- **Zero rows in every `pfin` table except its four seed tables.** Measured 2026-09-16, all 39 tables: `asset`=7, `posting_prototype_default`=30, `tax_character`=5, `taxonomy_default`=38 (each sourced from an `INSERT` inside a migration file — `016`, `084`, `011`, `041` respectively — so the re-apply recreates them automatically; no separate seed script exists); every other table = 0.
- **`auth.users` = 0.** Measured 2026-09-16 — signup is off, no user has ever registered.
- **The enumeration of everything 001–118 create OUTSIDE `pfin`** (Sec's blind-spot condition) **is empty for SCHEMA-RESIDENT objects — tables, functions, views, sequences, types, schemas.** ⚠ **This enumeration's frame was schema-resident objects only; it did NOT cover roles or role memberships (cluster-level, not schema-resident) — see the separate roles/memberships enumeration below, added on Sec's follow-up review.** Static: no `create` statement (schema-qualified `public.*`/`auth.*`/`storage.*`/`extensions.*`, event triggers, publications, or an unqualified `create` that would land in `public`) exists anywhere in 001–118 outside `pfin.*` — grepped directly, every `auth.` hit in the set is a *reference* (`auth.uid()` inside an RLS policy or DEFINER function body), never a `create … auth.…`. `alter default privileges` and `grant … to postgres` also do not execute anywhere — the two grep hits (`084`, `055`) are prose describing what was *not* done, not statements. Live, on the box: **zero `postgres`-owned relations, functions, types, or schemas outside `pfin`** — every non-`pfin` object belongs to the Supabase image's own baseline roles (`supabase_auth_admin`, `supabase_admin`, `supabase_functions_admin`) or to `migrator` (`supabase_migrations`, already transferred). The only `postgres`-owned things outside `pfin` are `pg_toast` internal tables (Postgres-internal, not user-addressable, unaffected by any `ALTER … OWNER TO` and out of scope for this fix) and the `extensions` schema container itself (pre-existing Supabase-image baseline, not created by 001–118, untouched by this wipe). **No schema-resident residue needing an explicit statement.**

- **Roles/memberships enumeration (cluster-level, added on Sec's follow-up review — NOT covered by the schema-resident enumeration above):**
  | role | created by | memberships granted | grantor | `postgres` holds ADMIN OPTION on it |
  |---|---|---|---|---|
  | `pfin_etl` (`055`) | `postgres` | `authenticated`, `service_role` | `postgres` | yes (measured: `t`) |
...
```sql
drop schema pfin cascade;
drop schema supabase_migrations cascade;
```

**Why `supabase_migrations` too, and why this simplifies rather than adds a step:** letting `migrator` re-create its own ledger schema retires item 32's manual `ALTER SCHEMA`/`ALTER TABLE … OWNER TO migrator` statements from the from-scratch path entirely — the schema is owned by its creator from the first row. `drop … cascade` does not touch roles; `pfin_etl`/`pfin_provider_sync`/`migrator` and their passwords survive untouched, and `055`/`116`/`118` report-don't-repair against pre-existing roles on re-apply. `auth`, `public`, `storage`, `vault`, `extensions` are untouched by either DROP.
...
**Expected output:** every one of 001–118 applies in order (including a second, tolerant pass over `118` — see below), `supabase_migrations.schema_migrations` is created and owned by `migrator` from the start, and the CLI reports success with 118 rows tracked (119 once the next real migration lands). This is the **same verb, same tracking table** as every other apply in this runbook — only the identity, and now the ordering, differ.

**`118`'s tolerant guard, and why it matters on this second pass.** When `migrator` applies the full set, it re-executes `118`'s file (the same one the pre-step already ran) — this time finding the role **already exists**, already `LOGIN`, already owned by `migrator`. `118`'s idempotency guard must **report, not repair**, on that pre-existing state (the same report-don't-repair posture `055`/`116` already carry for their own roles) while **still hard-failing on an actual C8 attribute violation** — that is the fence this section relies on to prove the pre-step and the full apply agree. ⚠ **`118`'s own header line — "`rolcanlogin` is FALSE at migration time and TRUE only in a provisioned environment" — is no longer accurate as an assertion**: on this re-apply path it is **always** true by the time `118` runs its second pass. Reword to an observation, not an assertion — **booked for Architect, comment-only, `BACKLOG.md` §7.36 (this PR's item).**

**Every SUPERVISED apply from here forward (any migration requiring true superuser — a new extension, `ALTER SYSTEM` — the case the ADR-072 Decision-4 tripwire is designed to force) begins with `set local role migrator;`, never `ALTER ROLE … SET role`.** This is Sec's cheaper mitigation for the recurrence direction the by-construction fix does not close on its own: a future `supabase_admin`-run supervised apply would otherwise create `supabase_admin`-owned objects, reopening the same gap in the other direction. `set local role` is visible in the artifact, scoped to the transaction, and fenceable — the same reasoning migration `116` leg (r12) already applies to forbid a `rolconfig` session default. `pfin_owner` (a durable, dedicated NOLOGIN owner role both `supabase_admin` and `migrator` could assume) stays booked as the eventual shape but is **not required yet** — it earns its keep only once a third apply identity genuinely exists.
...
-- (2) database ownership -- set by the pre-step, not by any migration
...
-- expect: f | f
-- (4) the ownership census itself -- expect ZERO postgres-owned pfin objects
...
-- (5) the ledger schema -- expect migrator, 118 rows (119 once the next real migration lands)
select pg_get_userbyid(nspowner) from pg_namespace where nspname = 'supabase_migrations';
select count(*) from supabase_migrations.schema_migrations;
-- (6) the four seed tables re-populated -- expect asset=7, posting_prototype_default=30, tax_character=5, taxonomy_default=38
...
Then confirm `rest` reports `healthy` (`docker inspect .State.Health.Status`) now that `pfin` exists again. `rolcanlogin = f` at (1) means the pre-step's `\password`/`LOGIN` did not run. Owner ≠ `migrator` at (2) means the `ALTER DATABASE` flip was skipped. Any `postgres` row at (4) means something in this ordering broke — stop and diagnose before proceeding to Phase C/D; do not paper over it with a manual `ALTER … OWNER TO` (that is the emergency-transfer shape Sec rejected in favor of by-construction).

**`SECURITY DEFINER` functions — the ownership question is asked once, at authoring time, not retrofitted.** Because `migrator` creates every `pfin` function from the start, `prosecdef = true` functions execute as `migrator` from their first row — no owner-swap event, no allowlist re-justification needed for anything that only reaches *inside* `pfin` (RLS bypass here is by table ownership, and `migrator` owns every `pfin` table, so `relforcerowsecurity` behaves the same as it did under `postgres`). **What does NOT automatically carry over is any DEFINER function reaching OUTSIDE `pfin`** — `vault.decrypted_secrets` is the measured case (`007`'s header records `postgres` can select it; `migrator` is a different role with no such measurement). Architect's ADR-011 Decision 9 allowlist table needs a per-entry statement of what privilege the owner needs and whether `migrator` holds it, not merely that the entry is allowlisted — **not this PR's scope; flagged for Architect/Sec.**

**The by-design tripwire (do not "fix" it).** `migrator` holds no `ADMIN OPTION` on `postgres`/`authenticator`/`pfin_etl`/`pfin_provider_sync` (created by `postgres` at cluster init, not by `migrator`) and is not superuser, so a future migration that `ALTER ROLE`s or `COMMENT`s those roles — or that needs true superuser — **fails when applied unsupervised as `migrator`**. That forces a supervised apply (now: `set local role migrator;` if the target objects are `migrator`-owned already, or a true superuser pass if not) and a Sec conversation, rather than silently widening a standing DDL credential. It is the intended Decision-4 behaviour, unchanged by this rewrite. **This ADMIN-option limit is an instance of the general ownership-gap class this section exists to close, not a special case specific to role comments** — `118`'s own comment made that claim narrower than it is; see the reword booking above.
...
4. **Pre-step, `supabase_admin`, interactive, minimal (§6.3):** run `118_migrator_role.sql` directly (creates `migrator` NOLOGIN/NOINHERIT/CREATEROLE, inert) → `ALTER DATABASE … OWNER TO migrator` → `\password migrator` → `ALTER ROLE migrator LOGIN`. **If a prior apply already exists on this box, wipe first** (`drop schema pfin cascade; drop schema supabase_migrations cascade;`), behind the three measured gates §6.3 states (zero non-seed `pfin` rows, `auth.users = 0`, the outside-`pfin` enumeration empty). **Prepare all three passwords before starting — §6.0's Step 0.**
5. **The apply, `migrator`, from its own container, no `--db-url` override (§6.3):** `docker compose … exec -T migrator supabase db push --workdir /workspace`. Applies 001–118 in order — including a second, tolerant pass over `118` itself (report-don't-repair on the pre-existing role, still hard-fails a real C8 violation) — creating every `pfin` object AND `supabase_migrations` (now created and owned by `migrator` from the first row; **item 32's manual `ALTER SCHEMA`/`ALTER TABLE … OWNER TO migrator` statements are retired**, not carried forward). Then the §6.1 (`pfin_etl`) and §6.2 (`pfin_provider_sync`) worker-role handoffs, same two-step credential shape, same deploy pass, unchanged from before.
6. **Verify — the ownership census, not the read verb alone (§6.3):** role attributes, database ownership, no app-role membership, **zero `postgres`-owned `pfin` objects** (the census query — this is the property the whole rewrite exists to establish), `supabase_migrations` owned by `migrator` with 118 rows tracked, the four seed tables re-populated (`asset`=7, `posting_prototype_default`=30, `tax_character`=5, `taxonomy_default`=38), `rest` healthy. **The true write proof is Phase D's first real migration, watched** — and per Sec's #775 ruling, the re-apply above already discharges item 32's write proof, the ownership property, and the from-scratch path in one pass; Phase D now proves the CI **transport**, not content, so its vehicle can be the smallest real `comment on` fixing a stale `pfin` comment, not a no-op.
