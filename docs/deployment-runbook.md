...
**⚠ The Coolify resource's "Include Source Commit in Build" setting MUST be ON — added 2026-09-17 (ADR-072 Amendment 6, PR #794).** Source-verified in Coolify 4.3.18's own code (`app/Jobs/ApplicationDeploymentJob.php`'s `generate_env_variables()`; the API field itself in `app/Http/Controllers/Api/ApplicationsController.php` and its own `tests/Feature/Api/ApplicationSourceCommitSettingApiTest.php`): the deploying commit's sha (`SOURCE_COMMIT`) is only added to the build-time environment/build-arg set when the application's `include_source_commit_in_build` setting is `true` — **default `false`, confirmed by Coolify's own test**. `infra/supabase/docker-compose.yml`'s `migrator` service now interpolates `GIT_SHA: ${SOURCE_COMMIT:-}` from this same value (PR #794) — with the setting off, that resolves empty and the migrator image's own build-time fail-closed check (`infra/supabase/migrator/Dockerfile`) FATALs, which is exactly what happened on this resource's first real build under Amendment 6 before this setting was known to matter.
- **API field name:** `include_source_commit_in_build` (boolean, top-level on the application resource — not nested under a `settings` key on write).
- **Set it via the Coolify UI** (Application → Configuration → Build tab, or wherever this Coolify 4.3.18 build exposes it) — **not scripted in `provision-supabase-stack.sh` yet.** The field name above is source-verified, not merely inferred, but per Sec's condition on this fold: an unconfirmed-in-practice name has no business being written into a script that SETS state — when this is scripted, it should ASSERT (read the value back, fail if off) rather than SET, and that's a separate, later change.
...
3a. **Verify line, added 2026-09-17 (ADR-072 Amendment 6, PR #791) — confirm which build-arg name Coolify 4.3.18 actually injects.** `infra/supabase/migrator/Dockerfile` accepts BOTH `GIT_SHA` and `SOURCE_COMMIT` because this was never confirmed live (no network egress in the session that wrote it) — this step is where that gets resolved on evidence instead of staying open forever:
    ...
    ```
    **Expect `SOURCE_COMMIT` (updated 2026-09-17, PR #794)** — not merely "either name," now that the mechanism is understood: `infra/supabase/docker-compose.yml`'s `migrator` service interpolates `GIT_SHA: ${SOURCE_COMMIT:-}`, so `SOURCE_COMMIT` is the only name that can ever reach the Dockerfile's `GIT_SHA` ARG through this path, and only when the resource's "Include Source Commit in Build" setting (§4) is `true`. Printing `GIT_SHA` here (rather than `SOURCE_COMMIT`) would mean some OTHER path supplied a `GIT_SHA` build-arg directly — worth investigating, not expected. Printing neither / an empty read means the setting is still off or unreadable — check §4's setting and its verify read before assuming anything else is wrong. **Once confirmed stable across a few real deploys, this is a follow-up (not blocking, and not this PR's scope): collapse the Dockerfile's dual-path `GIT_SHA`/`SOURCE_COMMIT` ARG-acceptance down to just `SOURCE_COMMIT`**, so the mechanism stops silently tolerating a name nobody uses.
...
**⚠ If that rebuild/redeploy itself FATALs with `GIT_SHA/SOURCE_COMMIT build-arg is empty` (added 2026-09-17, PR #794) — check "Include Source Commit in Build" on the resource first, before assuming a deeper problem.** That FATAL message means the Dockerfile's fail-closed check is working correctly against a build that genuinely supplied neither name — see §4's "Include Source Commit in Build" note above for the setting, its API field name, and the verify read. This is not a Dockerfile defect to chase; it is a per-resource Coolify setting to confirm.
...
**⚠ If the redeploy's build FATALs with `GIT_SHA/SOURCE_COMMIT build-arg is empty` instead of completing (added 2026-09-17, PR #794):** check "Include Source Commit in Build" on this resource first — §4's note above has the setting, its API field name, and the verify read. Report which is the case (setting off vs. some other cause) before treating this as a build-source/branch misconfiguration.
