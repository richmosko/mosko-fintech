...
**⚠ Why the `auth` grants are a separate file, not inside `roles.sql` — measured in CI (run 35172826387), not a preference.** Schema `auth` is owned by `supabase_auth_admin`. The Supabase CLI seeds `roles.sql` as a **non-superuser**, and seeding it there produced `WARNING (01007): no privileges were granted for "auth"` / `WARNING (01007): not all privileges were granted for column "id" of relation "users"` — this is a property of **who runs the file**, not of what it contains, so no ordering or split *inside* `roles.sql` could fix it. Production and CI apply the identical `auth-grants.sql`; only the applying identity differs (`supabase_admin` in both cases per this runbook's Phase 1, which is why production is unaffected).
