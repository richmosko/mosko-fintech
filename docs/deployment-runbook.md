...
| Networking | **MUST** be created in the **same Coolify project/environment** as `pfin-supabase-stack`, via an `external:` Docker-network attachment to the stack's own network (`scripts/provision-app.sh` resolves and sets `APP_STACK_NETWORK_NAME`) — **not** the `connect_to_docker_network` toggle (option B, rejected: widens `api-gw`'s reachable-from set to every other resource on that predefined network, same reasoning ADR-072 Amendment 4 applied to the migrator). `app`'s original `provider-sync` CA-4 dependency (the SELF-212 admission handshake, `http://provider-sync:8081`) is **unaffected** by this change and still applies once `provider-sync` exists. ⚠ **UNMEASURED whether Coolify 4.3.18's `dockercompose` build pack actually accepts an `external:` network declaration for THIS resource** — the migrator's own equivalent was measured working (§6.8 step 2), but that is evidence about the migrator's own deploy, not a guarantee for a second, independently-created resource; `scripts/provision-app.sh`'s own deploy step (invoked separately via `scripts/deploy-app.sh`, see below) is where this gets its first real signal for `app`. | `api/docker-compose.yaml`; `scripts/provision-app.sh` |
| Health check | No `/healthz`-shaped route exists under `api/src/routes/` (checked: none found). Until Backend adds one, configure Coolify's HTTP health check against `/` (root) — SvelteKit adapter-node answers 200 there once the app boots. **Flagged to Backend**, not invented here. ⚠ **UNMEASURED whether Coolify's Traefik routes a request to a `dockercompose`-pack service correctly with only `expose:` and no `ports:`** — this is exactly the mechanism a public Domain (and the sslip.io placeholder) depends on; step (v)'s `--health-path` probe is the first live signal. | `find api/src/routes -iname '*health*'` → empty |
