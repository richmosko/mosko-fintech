...
**Rewritten 2026-09-16 (F/CTO-ruled, Sec-accepted): ownership by construction, not by a later transfer.** The prior shape of this section — `postgres` (later corrected to `supabase_admin`) runs the whole first bootstrap, then a supervised handoff flips ownership onto `migrator` after the fact — reliably left every `pfin` object, and `supabase_migrations` itself, owned by whichever role ran the apply. On this box that produced a real gap (§7.36 item 32, `supabase_migrations`, and its wider cousin — every `pfin` table/function/view/sequence/type, measured 2026-09-16: **39 tables, 97 indexes, 34 sequences, 7 views, 116 functions, 3 enum types, all `postgres`-owned, zero `migrator`**). **The fix is ordering: create the bounded `migrator` role FIRST, then let `migrator` itself run the apply that creates everything else — so nothing is ever created by a role other than the one that will own it.** *(ADR-072 Decision 4 + Amendments 1/4/5; Sec joint-review, migrator-ownership consult, 2026-09-16.)*
