...
- [`secrets-manifest.yml`](../secrets-manifest.yml) — the CI/production **non-overlap commitment**. Two disjoint sets:
  ...
  - `production_only` (19 names — Coolify-injected on the box; never in CI): `SUPABASE_SERVICE_ROLE_KEY`, `PDF_WORKER_SIGNING_KEY`, `PLAID_CLIENT_ID`, `PLAID_SECRET`, `SIMPLEFIN_TOKEN`, `WORKER_ADMISSION_SHARED_SECRET`, `DISCORD_WEBHOOK_URL`, `PFIN_DB_PASSWORD`, `FMP_API_KEY`, `BLS_API_KEY`, plus the self-hosted Supabase stack's own nine (a dedicated block in [`secrets-manifest.yml`](../secrets-manifest.yml)): `POSTGRES_PASSWORD`, `JWT_SECRET`, `SECRET_KEY_BASE`, `VAULT_ENC_KEY`, `SERVICE_ROLE_KEY`, `ANON_KEY`, `DASHBOARD_PASSWORD`, `PG_META_CRYPTO_KEY`, `SMTP_PASS`. ⚠ **Corrected 2026-09-11** — this enumeration said "10 names" and omitted the Supabase-stack block entirely (Sec added eight of those nine on 2026-09-10; this list was never reconciled) — exactly the silent-drift failure mode this section's own next paragraph warns about. `SMTP_PASS` is the newly-declared ninth, added in the same change that made this correction (outbound-auth-email SMTP credential — [`docs/email-smtp-runbook.md`](email-smtp-runbook.md), which also documents the five non-secret SMTP_* config vars alongside it, not declared here for the same reason `SUPABASE_ANON_KEY` isn't). **`SUPABASE_ANON_KEY` is deliberately NOT in this set** — see the non-secret runtime-config list immediately below.
  - **Counts are load-bearing — check them, don't skim them.** The two lists above are a **hand-maintained mirror** of [`secrets-manifest.yml`](../secrets-manifest.yml); nothing enforces that they stay in sync, so they drift silently. The failure mode is asymmetric and unpleasant: a secret missing *here* isn't a fence breach (the non-overlap fence still passes — it reads the manifest, not this file), it's a **container that deploys without a secret it needs**. So: the `secrets-nonoverlap` job prints `N ci_only + M production_only` on every PR + push. If those numbers disagree with the `(5 names)` / `(19 names)` above, this enumeration has drifted — **the manifest is source of truth; fix this list.** (Live-checked 2026-09-11, at the same correction that fixed the stale `10` above: `python3 scripts/ci/check-secrets-nonoverlap.py` prints exactly `5 ci_only + 19 production_only` against the current manifest.) Comparing two integers CI already emits is the cheap check; diffing two prose lists by eye is the one that fails. *(This is not hypothetical: both halves of the `SIMPLEFIN_TOKEN` / `SIMPLEFIN_TOKEN_TEST` pair were missing here and went unnoticed until SELF-214 — with the fence green throughout.)*
...
- **`$PROD_DB_URL` — where it comes from, for each of the two shells that use it. Gap closed 2026-09-13 (standup step 6, Phase B): §4 and this section use the bare shell variable `$PROD_DB_URL` throughout without ever defining it — it is not an ambient env var on the box, the operator's Mac, or in any `.env` this repo commits.**
  ...
  - **The psql / `show server_version` verification snippets elsewhere in this file** (§4's bring-up verification, §4.1's TimeZone read-back) run against the **`db` service**, not the migrator — the migrator image deliberately carries no Postgres client (`infra/supabase/migrator/Dockerfile` installs `ca-certificates curl tar` only; `supabase db push` speaks the wire protocol directly). Use `docker compose --project-name <uuid> exec -T db psql -U postgres -d "$POSTGRES_DB" -c '…'`, which authenticates over the container's local socket and needs no password in any command line. **Do not add `postgresql-client` to the migrator image to make this bullet true** — the lean-image posture is deliberate. The interactive §6.1 / §6.2 handoffs use the same vehicle **without `-T`**, as `postgres`: `docker compose --project-name <uuid> exec -it db psql -U postgres -d "$POSTGRES_DB"` — a TTY is required for the prompt, and that prompt is the whole mechanism. **§6.3's handoff uses the same vehicle as `supabase_admin` instead — see §6.0.** ⚠ **If reaching any of these from off-box (F/CTO's own Mac, not an already-open box shell), the `ssh` hop itself needs `-t`** (`ssh -t root@<box-ip> 'docker compose ... exec -it db psql ...'`) — measured 2026-09-14: a bare `ssh` here gives `psql` no controlling terminal at all, silently, with no prompt and no error.
...
- **First bootstrap (supervised, until SELF-395):** the operator applies the migrations (via the migrator Scheduled Task, or the §4 `db push` — same verb, same tracking table) → **then the interactive §6.1/§6.2 `\password` handoff below** → §5 env → §7 containers. E cannot perform the interactive handoff yet; SELF-395 (client-side SCRAM scripting, SECURITY-GATED, not yet built) is what will let it own the full hands-off bootstrap.
...
1. **`pfin_etl` and `pfin_provider_sync` — generate two fresh values, kept for §5's secrets push as the ETL and provider-sync DB passwords:**
   ...
   ```
2. **`migrator` — do NOT generate a new value.** It is already minted (`provision-supabase-stack.sh`'s `MINT_SECRETS`, §6 above) and baked into the running container. Look it up from there, never printed to this record or any chat:
   ```sh
   ssh -t root@<box-ip> 'docker compose --project-name <supabase-stack-app-uuid> exec -T migrator sh -c "echo \$PROD_DB_URL"'
   ```
   The password is the substring between `migrator:` and `@db` in the printed URL.
...
**This is the ADR-072 Option-E DDL-apply identity, and it differs from §6.1/§6.2 in *who runs it* and *what it governs*.** Migration [`118_migrator_role.sql`](../supabase/migrations/118_migrator_role.sql) creates `migrator` **NOLOGIN, NOINHERIT, CREATEROLE, with NO password** — inert. Unlike the worker roles, this one is switched on so it can run *future unsupervised* `supabase db push` (migrations 119+) inside the resident migrator container. **The first bootstrap that creates `migrator` is itself run by a superuser** (ADR-072 first-bootstrap model), so the whole handoff below is a **supervised, superuser-run** step — `migrator` never switches itself on. ⚠ **Corrected 2026-09-14 — this section previously said `postgres` throughout; F/CTO's live run measured `postgres` is NOT superuser on this image and `ALTER DATABASE … OWNER TO migrator` failed under it (`ERROR: must be able to SET ROLE "migrator"`). Run this section's three statements as `supabase_admin`** (the actual superuser on this image — see §6.0). *(ADR-072 Decision 4 + Amendment 1; Option-E build chunk 1; SELF-398 Sec joint-review.)*

> **Precedence.** Described here and in [`118`](../supabase/migrations/118_migrator_role.sql)'s DEPLOY-TIME CREDENTIAL HANDOFF block. **If they disagree, `118` wins** — but see the note below: `118`'s own header also says `postgres`, and that is a known-stale copy of the same claim this section corrects, not a second, independent confirmation of it.

⚠ **`118`'s own DEPLOY-TIME CREDENTIAL HANDOFF block still says "run as `postgres`" / "SUPERUSER (`postgres`)" in several places — read live 2026-09-14, not fixed here.** Per this runbook's own migration-file-editing convention, a stale claim inside a migration's `--` header is Architect's to correct (comment-only, no DDL change), not DevOps's to patch in place. Booked at `BACKLOG.md` §7.36 for Architect.

**Ordering dependency — do not reorder:**

> **first bootstrap applied by `postgres` (§6, creating `migrator` inert)** → **`ALTER DATABASE … OWNER TO migrator` + `\password migrator` *then* `LOGIN` (this step, as `supabase_admin`)** → **`MIGRATOR_DB_*` env injected into the migrator service (§5; minted on-box by `provision-supabase-stack.sh` MINT_SECRETS)** → **migrator container brought up → subsequent applies run unsupervised as `migrator`**

**Why the OWNER flip is here and not in `118` (placement resolved).** `ALTER DATABASE … OWNER TO` requires a superuser, or `CREATEDB` **plus** membership in the new owner. `migrator` is deliberately **neither superuser nor `CREATEDB`**, so it can **never** run this statement — not even to set the owner to itself. If it rode migration `118`, every *unsupervised* re-apply (run *by* `migrator`) would fail on it. It requires a true superuser (`supabase_admin` on this image — `postgres` does not qualify, measured); the only superuser-run apply is this supervised bootstrap; therefore the flip lives here — exactly symmetric with the `\password`/`LOGIN` flip, which `055`/`116` also keep out of the migration. `migrator` must own the database before its first unsupervised apply so that `061`'s `ALTER DATABASE … SET` succeeds under it; this step, run at bootstrap, satisfies that.

**The step.** Run **once** against the target database, in an interactive `psql` session, **as `supabase_admin`** (never from a committed file; see §6.0 for why not `postgres`). `<app_db>` is the application database the stack runs (the Supabase stack's `postgres` database). **Four statements; the `\password`→`LOGIN` pair is order-load-bearing, and step 4 must run in this same supervised pass, before Phase C/D:**

```
ALTER DATABASE <app_db> OWNER TO migrator;   -- superuser-only (supabase_admin on this image); migrator is not CREATEDB
...
ALTER SCHEMA supabase_migrations OWNER TO migrator;         -- step 4 -- see below
ALTER TABLE supabase_migrations.schema_migrations OWNER TO migrator;  -- step 4 -- only table in the schema, enumerate at run time (`\dn+`/pg_tables), don't assume this stays the only one
```
...
**Step 4 — the `supabase_migrations` owner-transfer, and why it is ownership, not a `GRANT`** (Sec ruling, §7.36 item 32, 2026-09-14). **Owning the database does not extend to owning `supabase_migrations`** — `\dn+ supabase_migrations` and a `pg_class`/`pg_get_userbyid` read (both measured 2026-09-14) show the schema and its one table, `schema_migrations`, are both still owned by `postgres`; `migrator` has no `USAGE` on the schema. This split is a bootstrap-order accident, not a designed boundary — `postgres` created this schema before `migrator` existed, distinct from the `ALTER DATABASE … OWNER TO` this section's step 1 already flips — and the ADR-072 Decision 4 by-design tripwire fired exactly as intended: `supabase migration list` (read-only) failed closed under `migrator`'s own credential rather than silently widening anything. **Fix is ownership transfer, not a GRANT list**, in one sentence: `supabase_migrations.schema_migrations` is an idempotency ledger, not a security control (ADR-072 D3 puts fail-closed in the Scheduled Task's exit status, never in the ledger), so a role that already applies arbitrary DDL to this database gains no new capability by owning it, while a partial `GRANT` enumeration would rot the first time the CLI touches an object nobody granted — failing the *unsupervised* Phase-D apply at the worst possible moment, with no operator watching. **Enumerate at run time, don't copy the list above blind** — today it is one table (`schema_migrations`) with no sequences or functions in the schema (measured), but re-run the `\dn+`/`pg_class` read above before typing the `ALTER TABLE` statements, in case that has changed.

**Proof this step worked — the write verb, not the read verb.** `migration list` is a READ; it is not sufficient proof, because the verb that matters is `db push`, the WRITE `migrations 119+` will actually use. After step 4 runs, from inside the `migrator` container: `supabase db push --db-url "$PROD_DB_URL"` must report the remote is up to date with **no permission error**. **This confirms the ownership transfer is correctly shaped — it does not itself prove the unsupervised write path works end to end.** The true write proof is Phase D's first real migration (119), applied unsupervised as `migrator` through the Scheduled Task, watched.
...
**Phase B — supervised first bootstrap (§6.1/§6.2 as `postgres`; §6.3 as `supabase_admin` — see §6.0)**
...
5. The §6.3 handoff, interactive `psql` as `supabase_admin` (not `postgres` — see §6.0), load-bearing order: `ALTER DATABASE … OWNER TO migrator` → `\password migrator` → `ALTER ROLE migrator LOGIN` → **step 4: `ALTER SCHEMA supabase_migrations OWNER TO migrator` + one `ALTER TABLE … OWNER TO migrator` per table in it** (§7.36 item 32 — the ADR-072 D4 tripwire firing against the ledger schema, fixed by ownership transfer in this same supervised pass, not a later patch). Do this alongside the §6.1 (`pfin_etl`) and §6.2 (`pfin_provider_sync`) worker-role handoffs — same two-step credential shape, same deploy pass. **Prepare all three passwords before starting — §6.0's Step 0.**
6. Verify per §6.3's verify block (role attributes, database ownership, no app-role membership) and §6.1/§6.2's own verify blocks; **then prove step 4 with the write verb, not the read verb** — `supabase db push --db-url "$PROD_DB_URL"` from inside `migrator` must report up to date with no permission error (`migration list` alone is not sufficient proof). Then bring the `migrator` container up. **The true write proof is Phase D's first real migration, watched.**
