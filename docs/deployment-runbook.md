...
2. **Measure whether mechanism (b) — an `external:` network — is viable on this Coolify version, BEFORE creating anything.** `infra/supabase/migrator/docker-compose.yaml`'s own `networks:` block declares this mechanism but flags it UNMEASURED against Coolify 4.3.18's `dockercompose` build pack. `scripts/provision-migrator-app.sh`'s own "Looking up the stack's live Docker network" step performs part of this measurement (confirms the stack has exactly one non-default network to attach to) but the REAL test is whether the new application's deploy succeeds with that network declared `external: true`. **Run `BOX_IP=<box-ip> scripts/provision-migrator-app.sh --apply` (step 3 below) and treat its outcome as this measurement:**
   - **Deploy succeeds, `migrator` container comes up, `docker inspect` shows it on the expected network** → mechanism (b) works. Record this in this runbook (replace this bullet with "MEASURED WORKING, <date>") and move on.
   - **Deploy fails with a parse error naming `networks:`, or the container comes up with NO route to `db`** → mechanism (b) is not viable on this Coolify version. **STOP. Do not proceed to step 4.** Edit `infra/supabase/migrator/docker-compose.yaml` per its own "FALLBACK MECHANISM (a)" comment block (uncomment the two-line `networks:` list form, remove the `external:`-block form), enable `connect_to_docker_network` on BOTH the migrator application and the stack application via the Coolify dashboard, and **write down in this runbook, by name, every other resource that shares that predefined network at the time of adoption** — mechanism (a)'s documented cost (Amendment 4: "the axis the RT-32 / fence-migrator-private-bind fences cannot see"). Re-run step 3 after making this change.
...
7. **Only now — delete both `MIGRATOR_DB_*` names from the stack's store, and redeploy the stack.** By hand via the Coolify dashboard (Environment Variables tab on the `pfin-supabase-stack` application) — **delete, do not blank.** Then trigger a stack redeploy (`POST /deploy?uuid=<stack-app-uuid>` or the dashboard Deploy button) — a container's `Config.Env` is fixed at `docker create`, so step 8's measurement means nothing until this redeploy has happened.
...
9. **Proof measurement (1) — the NEW migrator container.** `docker inspect --format '{{range .Config.Env}}{{println .}}{{end}}' <new-migrator-container> | cut -d= -f1 | sort` — expect `MIGRATOR_DB_USER` and `MIGRATOR_DB_PASSWORD` **PRESENT**, and `POSTGRES_PASSWORD`, `JWT_SECRET`, `SERVICE_ROLE_KEY`, `VAULT_ENC_KEY`, `ANON_KEY`, `SECRET_KEY_BASE` **ABSENT**. (This repeats step 3's own check post-redeploy — re-run it, don't assume step 3's result still holds.)

10. **Proof measurement (2) — `meta`, inside the stack.** `docker compose --project-name <stack-app-uuid> exec -T meta env | cut -d= -f1 | sort` — expect **BOTH** `MIGRATOR_DB_USER` and `MIGRATOR_DB_PASSWORD` **ABSENT**. A BLANKED-not-deleted key still carries its name and correctly FAILS this check (Amendment 4's own proof-predicate design) — **do not "fix" a failing check here by switching from delete to blank; go back to step 7 and delete the key from the Coolify dashboard properly.**

    ⚠ **This is also `scripts/provision-supabase-stack.sh`'s own "Post-move: MIGRATOR_DB_* absence assertion" step, and the STRIKE-PROOF for it belongs here, on this box, once:** re-add `MIGRATOR_DB_USER=x` to the stack's store by hand, re-run `BOX_IP=<box-ip> scripts/provision-supabase-stack.sh --apply`, confirm it FAILS closed at the "Post-move" step naming the offending key, then remove it again and confirm the step passes. **Record the command and its output** (both the RED run and the GREEN re-run) in this PR's body or a linked record — this is the strike-proof Amendment 4 requires as a merge condition, and it can only be run against a live Coolify resource.

11. **Prove the OLD credential fails to authenticate.** From the box, attempt a connection to `db` using the credential value the stack's store held BEFORE step 7 (kept in a supervised scratch note during this procedure, never committed anywhere) — expect an authentication failure. This is the condition that makes step 5's ROTATE meaningful rather than cosmetic (Sec's condition, [ADR-072](../DECISIONS.md#adr-072) Amendment 4: *"confirm the OLD credential FAILS to authenticate — not merely that the new one succeeds"*). Destroy the scratch note immediately after this check.
