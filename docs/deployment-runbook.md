...
**Secrets this step produces.** Names only — never values, here or anywhere in this repo; most (not all — see the two anon-key rows below) are drawn from `secrets-manifest.yml`'s `production_only` set. **§5's secrets-provisioning procedure is still a STUB and its Sec joint-review flag is NOT discharged by this section** — this only names where these five land; rotation/injection-order procedure is §5's job.
