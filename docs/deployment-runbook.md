...
9–11. **Proof measurements — `scripts/migrator-cutover-verify.sh`.** `BOX_IP=<box-ip> scripts/migrator-cutover-verify.sh --migrator-app pfin-migrator --stack-app pfin-supabase-stack` runs all three legs and prints PASS/FAIL per leg, exiting non-zero if any fails:
    ...
    - **Leg 10 (`meta`, inside the stack)** — `docker compose ... exec -T meta env`, both `MIGRATOR_DB_USER` and `MIGRATOR_DB_PASSWORD` **ABSENT**. A BLANKED-not-deleted key still carries its name and correctly FAILS this leg (Amendment 4's own proof-predicate design) — **do not "fix" a failing leg here by switching from delete to blank; go back to step 7 and re-run it.** ⚠ This is also `scripts/provision-supabase-stack.sh`'s own "Post-move: MIGRATOR_DB_* absence assertion" step; that predicate is already strike-proven in CI against `tests/fixtures/ci/migrator-names-absent-*.env` (PR #819) — leg 10 is this box's own live confirmation of the SAME predicate, not a second instance of the CI merge condition.
