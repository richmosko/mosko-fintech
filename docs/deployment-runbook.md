...
| (ii) | SCRIPTED | `coolify-env.sh set pfin-app PUBLIC_SUPABASE_URL=http://api-gw:8000 --apply` — a fixed literal, no read of any kind needed |
| (iii) | SCRIPTED | `mint-supabase-jwt-keys.sh --apply --app-name pfin-app --verify-live` — mints and propagates the real `PUBLIC_SUPABASE_ANON_KEY` / `SUPABASE_SERVICE_ROLE_KEY` onto `pfin-app`, LAST |
| (iv)–(vi) | SCRIPTED | `deploy-app.sh pfin-app --expect-base-directory /api --require-env PUBLIC_SUPABASE_URL,PUBLIC_SUPABASE_ANON_KEY,SUPABASE_SERVICE_ROLE_KEY --apply --health-path /` — the names-only post-mint presence assertion, deploy, poll, on-box running-container read, and the health probe all fold into this one command |
| (vii) | SCRIPTED | `smoke-pfin-exposure.sh pfin-app` — the pre-invite anon-bearer `pfin`-exposure smoke, runs unattended, exits non-zero on anything but the expected 401/42501 |
| (viii) | BY-HAND | flip `DEPLOY_ON_SUCCESS` + re-exercise the trigger — §7 step 7 below, unchanged (a human authorship + a real merge, not a probe) |
...
2. **Set the one non-secret app env var this step needs.**
   ```sh
   ...
   **`PUBLIC_SUPABASE_ANON_KEY` is deliberately NOT set here.** Step 3's mint overwrites it unconditionally regardless of what (or whether) it was set beforehand — corrected from an earlier draft of this procedure that set a "provisional" copy here for no benefit: a value overwritten before it is ever read by a deployed container is dead work, not a safety margin, and it carried a BY-HAND Coolify-dashboard read this step doesn't need. Step 4's `--require-env` closes the actual gap that draft was reaching for (proving the value exists AFTER mint, not before).
