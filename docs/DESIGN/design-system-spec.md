...
| **chart-placeholder** | default · loading(skeleton) · empty(insufficient history) · cpi-unavailable(nominal-only) · stale-segment | viz-nominal/infl/fill, text-muted; chart-canvas intrinsic height (20rem) is a component-intrinsic constant, not a --space-* value (same category as count-badge's 1.25rem) |
...
| **per-bar unavailable-value hatch** *(SELF-256; bar-chart variant of the sparse-history hatch)* | full plot-height hatched swatch replaces an individual bar wherever that row's value is unresolvable (e.g. no CPI-U figure for that month) — never a blank gap, never a $0-shaped bar | **same tokens as sparse-history** (diagonal `--c-border` hatch on `--c-surface-alt`, never `--c-viz-fill`) — same semantic ("no real value to plot here"), different geometry: per-datum instead of one contiguous left-truncated region. Ratified as a sound reuse because this chart carries no competing left-truncation region to confuse it with; disambiguated further by the per-bar hit-target's aria-label naming the specific reason. Confirmed Visual ruling 2026-08-31 (SELF-256). |
| **chart-tooltip (house pattern)** *(SELF-256; first instantiation — NavHistoryChart has none)* | per-datum keyboard+hover disclosure: an invisible full-band hit-target (`role="button" tabindex="0"`) per point drives a fixed-position floating panel (`role="status"`); the SAME text is the hit-target's `aria-label`, so sighted-hover and keyboard/AT users get identical content | surface, border, shadow-2, radius-md, text-primary/muted, font-num (values). `pointer-events: none` on the panel so it never blocks the next hit-target. Adopted as the default chart-tooltip shape for future charts needing per-point disclosure. Confirmed Visual ruling 2026-08-31 (SELF-256). |
