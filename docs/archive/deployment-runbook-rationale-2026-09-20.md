# Deployment Runbook — Archived Rationale (frozen at 2026-09-20)

> **This file is FROZEN and read-only** — a verbatim snapshot of the prose (MEASURED
> findings, incident history, Sec dispositions, design rationale, "why" narrative) that
> was removed from `docs/deployment-runbook.md` when that file was converted from a
> prose runbook into a scripts-driven execution sheet (F/CTO directive 2026-09-20;
> see `DECISIONS.md` for the ADR, if one is authored, and `docs/deployment-runbook.md`'s
> own header for where the live sheet is).
>
> **Precedent:** same pattern as [`docs/archive/PRD-pre-html-migration.md`](PRD-pre-html-migration.md)
> — a frozen Markdown snapshot kept for historical/audit reference, not maintained
> going forward. Do not edit this file to reflect new findings; add those to the live
> sheet's cited script headers or to `DECISIONS.md` instead.
>
> **Organization:** by the runbook's ORIGINAL section numbering, as it stood at the
> 2026-09-20 cutover. Content is verbatim (byte-for-byte, extracted from the pre-rewrite
> file), so Sec (or anyone) can diff this file's per-section text against the prior
> `docs/deployment-runbook.md` revision to confirm nothing was silently dropped.
>
> **§7 (Workers) is NOT included here** — it was not touched by the 2026-09-20 rewrite
> (a second, concurrent DevOps PR — W-3 — owns that section) and remains live, unarchived
> prose in `docs/deployment-runbook.md` §7 until its own conversion pass.

---

## ARCHIVED — original `docs/deployment-runbook.md` lines 11-25: Overview & principles
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 26-53: Prerequisites
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 54-193: §1 Provision the VPS
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 194-253: §2 DNS / domain
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 254-323: §3 Install & configure Coolify
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 324-538: §4 Stand up Supabase from scratch (through pre-4.1)
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 539-703: §4.1 Database TimeZone
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 704-752: §5 Secrets provisioning
...
- [`secrets-manifest.yml`](../secrets-manifest.yml) — the CI/production **non-overlap commitment**. Two disjoint sets:
  ...
  - **Non-secret runtime config (NOT in `secrets-manifest.yml` — Sec-authorized, 2026-09-09):** two names are boot-required but deliberately absent from both the CI/production non-overlap sets above, because neither is a secret:
    ...
    Both are read by `api/src/hooks.server.ts`'s boot-time env guard via SvelteKit's `$env/dynamic/public`, and both are declared (non-secret) in [`api/.env.example`](../api/.env.example). Inject both as **plain (non-secret) Coolify environment variables** on the `app` service, under these exact `PUBLIC_`-prefixed names — omitting either throws at boot. Do not add either to `secrets-manifest.yml` or [`api/.env.example`](../api/.env.example)'s SECRETS section: those enumerate confidential values, and listing a publishable JWT there would assert a blast radius that does not exist and would put a production-tier name on developer machines that must hold the local stack's own value instead. (Corrected 2026-09-20, root `.env.example` retirement — the two names already sit in `api/.env.example`'s own non-secret block above the SECRETS section; this warning was always about NOT promoting them into the confidential enumeration, not about which file that enumeration now lives in.)
...
- Per-surface `.env.example` files (each enumerates ONLY its container's permitted secrets — the enumeration *is* the confinement property): ⚠ **NEEDS RE-GRADING, not a flat retraction — measured 2026-09-14.** Measured live on the running stack: every service **inside a multi-service Coolify application** (the Supabase-stack resource — `db`/`auth`/`rest`/`api-gw`/`supavisor`/`meta`/`studio`, one Coolify app UUID) receives the whole env store via `env_file:` in Coolify's own rendered compose, regardless of what its `.env.example` or declared block enumerates. **`migrator` is no longer inside this application** — ADR-072 Amendment 4 / `BACKLOG.md` §7.36 item 29 (2026-09-18) moved it to its own one-service Coolify application, `infra/supabase/migrator/docker-compose.yaml`, specifically because this multi-service finding falsified its confinement claim under the old (sibling-service) topology; see `infra/supabase/migrator/.env.example`'s own corrected note on what its enumeration now proves (confinement-by-construction again, in a one-service application, though not independently re-measured against THAT shape — `scripts/provision-migrator-app.sh`'s own verification step checks it by `docker inspect`, not by declaration). The V1 web app and each worker below are **separately-deployed** single-service Coolify applications and were **not measured** — whether the same mechanism applies to a one-service app is an open question, not assumed either way. [ADR-072](../DECISIONS.md#adr-072) Amendment 3 (PR #760) withdraws Amendment 1's PRESERVED claim for the multi-service case; the migrator-specific remedy landed at `BACKLOG.md` §7.36 item 29 (this PR) — **item 28, the stack-wide exposure for `db`/`auth`/`rest`/`api-gw`/`supavisor`/`meta`/`studio`, remains open and is NOT closed or narrowed by item 29.**
  - [`api/.env.example`](../api/.env.example) — V1 web-app container (service_role + PDF signing key + worker admission shared secret + Discord URL). ⚠ **No `PLAID_WEBHOOK_SECRET`, and no Plaid credential at all.** An earlier revision of this line said "Plaid creds + webhook secret"; both halves are wrong at the tree. The webhook secret was retired at [ADR-037](../DECISIONS.md#adr-037) — Plaid v27 webhook verification is asymmetric ES256/JWK against a PUBLIC key, so there is no shared secret to hold, and `api/.env.example` says so explicitly rather than merely omitting it. `PLAID_CLIENT_ID`/`PLAID_SECRET` are held by the **provider-sync worker only** (the delegated JWK fetch); keeping `api/src` credential-less is the load-bearing property that makes that delegation work. The variable name never appeared in this runbook — what survived here was a **prose paraphrase describing a secret `.env.example` no longer contains**, which is why a name-grep found nothing to sweep. (Corrected 2026-09-20 — a SEPARATE repo-root `.env.example` this line originally pointed at was retired by F/CTO ruling; `api/.env.example` is now the single canonical enumeration for this surface, not a rename of what this line always meant.)
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 753-807: §6 Apply migrations (top matter, before 6.0)
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 808-855: §6.0 Before you start §6.1/§6.2/§6.3
...
**§6.1/§6.2 are now SCRIPTED — `scripts/db-role-handoff.sh` — BACKLOG.md §7.36 item 68, W-2.** Both sections below describe the procedure the script executes; run the script rather than typing the raw `psql` session by hand. The script drives the SAME two statements (`\password <role>` piped over stdin, never a `-c` literal, then `ALTER ROLE <role> LOGIN;`), generates the credential locally (`openssl rand -hex 32`), delivers it to the box as a 0600 seed over SSH stdin (never argv, never an env dump), verifies the handoff by connecting AS the role over TCP with the generated credential, and pushes the SAME value onto the worker's own Coolify resource as `PFIN_DB_PASSWORD` — full mechanism detail (including why `\password` via piped stdin is used instead of a hand-rolled SCRAM hash, verified locally against a throwaway Postgres instance) is in the script's own header, not restated here.

**⚠ Connecting identity: `supabase_admin`, not `postgres`, for the SCRIPTED path.** §6.0's own text above (and §6.1/§6.2's prose below) describes the BY-HAND procedure as run `as postgres` — correct for a human at an interactive `psql` session (`postgres` holds `CREATEROLE`, sufficient for `\password`/`ALTER ROLE`). `scripts/db-role-handoff.sh` instead connects as `supabase_admin` (the box's true superuser) throughout, because its own preflight check — confirming "no password set" via `pg_authid.rolpassword`, not just `pg_roles.rolcanlogin` — needs read access `postgres` does not have (verified locally: a non-superuser `CREATEROLE` role gets `permission denied for table pg_authid`). `supabase_admin` is a strict superset of what `postgres` can do here, so this does not weaken either role's own required-privilege statement — flagged as a judgment call for Sec review, not silently assumed.

**SCRIPTED / BY-HAND audit — `scripts/db-role-handoff.sh <pfin_etl|pfin_provider_sync>` (same script, role argument selects §6.1 or §6.2):**

| Step | Status | Detail |
|---|---|---|
| Preflight (role exists? already LOGIN? password already set?) | SCRIPTED | Reads `pg_roles.rolcanlogin` + `pg_authid.rolpassword IS NOT NULL` as `supabase_admin`. Refuses if the role does not exist (migration not applied yet); refuses an initial handoff over an already-LOGIN-with-password role unless `--rotate` is passed. |
| Generate the credential | SCRIPTED | `openssl rand -hex 32`, generated on the OPERATOR's own machine (never on the box, never by the box) — the script's own local process, discarded from its variables immediately after the seed delivery below. |
| Deliver the credential to the box | SCRIPTED | Piped over SSH stdin into a 0600 seed file under `/root/.pfin/` — never a command-line argument on either side. |
| `\password <role>` then `ALTER ROLE <role> LOGIN;` (or `\password` alone, under `--rotate`) | SCRIPTED | `BOX_IP=<box-ip> scripts/db-role-handoff.sh <role> --apply [--rotate]` — the two statements piped into `psql` via a here-string, inside the `db` container. |
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 856-924: §6.1 pfin_etl role provisioning
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 925-975: §6.2 pfin_provider_sync role provisioning
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 976-1215: §6.3 pfin_owner + migrator provisioning and first bootstrap
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 1216-1239: §6.4 CI trigger provisioning
...
3. **Set the local `.env`** (`scripts/provision.env.example` is the single, CI-fence-enforced list of every name this repo's scripts read from it — not restated here): `CI_MIGRATE_SSH_PUBKEY` (path to the `.pub` file from step 2), `MIGRATOR_SERVICE_UUID`, `MIGRATOR_TASK_UUID`, `APP_UUID` (from step 1), and `DEPLOY_ON_SUCCESS` (default `0` — Sec-ruled deploy gate, Phase D deploy-gate consult: the first live exercise of a remotely-reachable trigger holding a deploy-capable token should do the smallest thing it can, so a successful migration apply withholds the app deploy until this is deliberately flipped to `1` at §7 step 7).
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 1240-1345: §6.5 Migrator bring-up — operator execution order
...
**What remained unmeasured after step 6, ordered by size — the largest first:** ~~the `DEPLOY_ON_SUCCESS=1` leg is now the LARGEST unmeasured item in this whole chain~~ **MEASURED, 2026-09-20 (stage B, run `35537705094`) — see §7 step 7 below.** Every fire before this one ran with the app deploy suppressed; this fire is the first to actually trigger a production deploy from the migration path, and it passed: apply SUCCEEDED, `pfin-app` redeployed at the triggering commit, one container, `200` in-container. Still unmeasured, unchanged by this fire: the pre-fire read-back's MISMATCH branch (exit 10, residual (4) above) — step 5 exercised only the agreement path; and exits **13/14/15** and the **≥2-new-uuid** branch (residual (3) above), live.
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 1346-1401: §6.7 fail-probe positive-control recipe
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 1402-1501: §6.6 Re-bootstrap execution plan — THIS box
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 1502-1573: §6.8 Migrator standalone-resource CUTOVER PROCEDURE
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 1574-1642: §6.9 pfin Data-API exposure flip PROCEDURE
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 1881-1890: §8 Observability
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 1891-1900: §9 Cutover & teardown of pfindash.com
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 1901-1932: §10 Verification / smoke-test
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 1933-1951: §11 User deletion / GDPR erasure
...
## ARCHIVED — original `docs/deployment-runbook.md` lines 1952-1969: Open flags (roll-up)
