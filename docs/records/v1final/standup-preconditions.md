...
**Status: AWAITING F/CTO.** Baseline `main` @ `90420b38`. Author: DevOps. Sec position: placeholder under each question below, pending Security Engineer's parallel review — do not treat this PR as mergeable-as-ready until both are filled.
...
- **(A) Production never exposes `pfin` via the Data API; RLS is defense-in-depth behind a server-side tenant-bound path.** **Not viable as measured.** No such tenant-bound path exists in `api/` (fact 1) — building one means hand-rolling a direct-Postgres transport (a `pg` pool + `SET LOCAL ROLE` + JWT-claim GUC binding, the same shape `TenantBoundClient`/`TenantBoundConnection` implement for workers) across all 45 files that currently call `.schema('pfin')`, and re-deciding, file by file, which of the 127 call sites become raw SQL vs. RPCs against a still-exposed `public` schema. It also formally reopens ADR-023 — a ratified, Sec-conditioned, one-way-door decision that already evaluated and rejected this exact shape ("B", fact 3). **What breaks if chosen:** every dashboard, settings, and MFA-recovery screen in the app, the moment a real tenant uses it — undetected today only because signup is off (Q5) and no tenant has been onboarded yet.
- **(B) Production adds `pfin` to `PGRST_DB_SCHEMAS`, matching dev and matching ADR-023.** RLS is the sole control on that path — which is what ADR-023 already designed and what every migration's grant statements + the two-tenant battery (C6) already assume is running. **What it changes on the box:** one Coolify env-store edit on the `rest` service (and any sibling env file Coolify renders from it, per the `BACKLOG` item 27/28 findings on this stack's env-injection mechanism — not this PR's scope to re-litigate). **What it changes in the repo:** correct `docs/deployment-runbook.md:495`'s already-partially-flagged paragraph to state the reconciled value cleanly (removing the "unconfirmed against the live box" hedge once fixed); correct `standup-log.md:272`'s §5f entry to stop asserting a value that was never true in production; correct `provision-supabase-stack.sh:672`'s `NONSECRET_DEFAULTS` default and the `:912-921` "expected unhealthy" comment so a future fresh stand-up doesn't re-mint the wrong default. **Step 7/step 10 smoke checks this unblocks:** `production-standup.md` step 10's reachability checks assume the app can read/write `pfin` through the deployed Data API; today they would fail (or worse, silently pass on schemas that expose nothing tenant-bearing) — this is stage 2's carrier-fix + a real smoke test, not merely a doc correction. **V1-SHIP-BLOCK-relevant test:** the SECURITY §4.5 two-tenant RLS battery (ADR-023 C6) is the standing proof obligation this option is already required to satisfy independent of this ruling.
- **(C) `pfin` exposed but PostgREST role grants revoked so the surface is inert.** **Not supported by measurement — no such configuration exists on the tree.** ADR-023's own `008` migration (and its `015`/`019`/`020`/`021` successors) *are* the grants that make PostgREST-on-`pfin` functional (`service_role` USAGE + least-privilege writes; `authenticated` reads under RLS) — revoking them breaks the same 127 call sites Option A breaks, for the identical reason (fact 1). This collapses into Option A's failure mode; named per the brief but not a distinct live option.

**DevOps recommendation: (B), not close.** This isn't a fresh architectural choice between three tradeoffs — it's a production box that is not running the architecture its own tree (dev config, every migration's grants, ADR-023's ratified text) already committed to. Option A would mean reopening a one-way-door ADR and rewriting the entire `api/` data-access layer to build a transport that has never existed; Option C doesn't exist as a buildable configuration. The answer to item 22's cross-referenced question: **`pfin`'s RLS is, and per ADR-023 was always designed to be, the Data API's only control on that path** — not a defense-in-depth layer behind something else, because nothing else is there. Land the env change before step 7's smoke checklist runs (step 10's checks assume it), and land the three carrier corrections in the stage-2 PR this ruling authorizes.

**Sec position:** ⟨PLACEHOLDER — Security Engineer to paste commit-ready text here⟩
...
5. **This record's own Q5 ruling (§G.4, already F/CTO-decided) bounds the exposure.** Production `GOTRUE_DISABLE_SIGNUP: "true"` is hardcoded in `infra/supabase/docker-compose.yml:172-174` with a comment naming it as the Q5 gate mechanism, not an operator tunable. `GOTRUE_MAILER_URLPATHS_INVITE` **is** set (`:188`), so the F/CTO's own admin-created tenant **will** receive one invite/magic-link email during the soak, in stock GoTrue copy, unless wired before that invite is sent. No stranger receives any auth email until the BACKLOG §7.36 allowlist lands and signup turns on (Q5) — the same gate this item's Option (C) below would tie to.
6. **`GOTRUE_MAILER_URLPATHS_*` vs. existing SvelteKit routes:** `api/src/routes/auth/callback/+server.ts` exists and is the generic Supabase-SSR PKCE/magic-link/recovery callback handler (plus `auth/signout`); `api/reauth/{start,complete}` is a separate re-auth flow, unrelated to GoTrue's mailer links. The actual `MAILER_URLPATHS_*` values live in the Coolify env store (not the repo) and were not read for this paper (out of the "names only" measurement scope) — consistency is qualitative: one generic callback route is the expected shape for this integration pattern, and nothing in the route list contradicts it.

**Options.**

- **(A) Ship V1 with stock GoTrue emails; book branded templates to V2 or post-cutover.** Cost: zero — this is the current state. Operator action: none. PRD requires nothing here (fact 4); the one email sent during the soak goes to the F/CTO, who knows what system sent it (fact 5). CI: nothing new to watch.
- **(B) Wire `GOTRUE_MAILER_TEMPLATES_*` to URLs served by `app`'s own public domain, once it has one.** Mechanism measured present, not hypothetical: `app` already gets a public HTTPS domain at the §2/§9 DNS cutover (`pfindash.com`), and SvelteKit's `adapter-node` serves `api/static/**` at the app's root — no new container. Config delta: copy the 3 existing `supabase/templates/*.html` files (plus author a 4th for `invite`, which doesn't exist yet) into `api/static/email-templates/`, and set 4 `GOTRUE_MAILER_TEMPLATES_*` env vars on the Supabase-stack Coolify resource to `https://pfindash.com/email-templates/<name>.html`. **This cannot fully land before step 7** for the invite email specifically — it depends on the DNS cutover, which is §9, after step 7 — so choosing (B) alone does not close item 24 pre-step-7; it's the post-cutover destination this item points toward. The cross-project Docker-network alternative (fact 3) is available sooner but couples the two independently-deployed Coolify projects' networks for the first time in this direction, for no gain over waiting for the domain that's coming anyway.
- **(C) Defer wiring to after step 7 but before the DNS cutover (§9); not a step-7 blocker.** Signup is off in production per this record's own Q5 ruling (§F/§G.4) — no stranger sees any auth email until the operator allowlist (BACKLOG §7.36) ships and signup is switched on, which is itself gated to land before that switch. The only email in flight before then is the F/CTO's own invite. **This makes item 24 non-blocking for step 7**: nothing in step 7's or step 10's smoke checklist reads email content, and the Q5 ruling already establishes that no real user sees the result of this decision until well after step 7.

**DevOps recommendation: (A) now, converging into (C)'s reasoning — not a step-7 blocker; revisit at the DNS cutover using (B)'s measured mechanism.** Ship stock copy today (zero cost, PRD-silent, single known recipient). Tie the branding wire-up to the same BACKLOG §7.36 allowlist milestone Q5 already gates signup behind, using `app`'s own public domain (Option B) once it exists — that is strictly cheaper than standing up any dedicated static-serving surface, and keeps this item off the step-7 critical path entirely, which the measured facts (PRD silence + Q5's sole-invited-tenant soak) support without qualification.

**Sec position:** ⟨PLACEHOLDER — Security Engineer to paste commit-ready text here⟩

### I.3 What follows this ruling

Stage 1 (this PR) is the ruling paper only — no carrier edits, no ADR edits, no `DECISIONS.md` changes. Once F/CTO rules Q-22 and Q-24 above (and Sec's position is recorded), stage 2 is a separate DevOps dispatch: the `PGRST_DB_SCHEMAS` Coolify env change + the three named carrier corrections (`deployment-runbook.md:495`, `standup-log.md:272`, `provision-supabase-stack.sh:672` + `:912-921`) for Q-22; and, if Q-24 lands on (C), a BACKLOG §7.36 note tying the templates wire-up to the same allowlist milestone as Q5, or, if F/CTO wants it sooner, the `api/static/email-templates/` + 4-var Coolify change from Option (B). Neither stage 2 action is authorized by this PR alone.
