...
| 6 | Deploy the four services from one `main` sha | DevOps | 🟡 **Phase A DONE 2026-09-14** (`APP_UUID`, migrator Scheduled Task, `migrator` container confirmed `Up`, `supabase-go` confirmed present on `main` `4774189d`); Phase B step 4 ⛔ **blocked**, 3rd attempt — bootstrap apply hit a third real bug (`supabase/templates/**` missing, CLI validates the full config path set), fix on `feat/standup-step6-phase-b4-apply`, unmerged |
