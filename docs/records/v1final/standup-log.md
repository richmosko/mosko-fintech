...
**Sec read the pinned v2.107.0 source directly** (`apps/cli/src/legacy/shared/legacy-db-config.parse.ts:402`/`:644`/`:532`): `sslmode` resolves as `url.searchParams.get("sslmode") ?? svc("sslmode") ?? PGSSLMODE` — no force, contrary to the prior working hypothesis. The actual explanation: the query parameter most likely never reached the CLI at all — the DSN is built inside a **single-quoted** `sh -c` body, and a surviving bare `?` there is exactly the kind of thing that looks correct in the file and isn't at execution time. Version-pin (candidate fallback) rejected — item 20 stays open, no premise to build on; TLS-on-`db` rejected — its own trigger (an off-host reachability change) never fired.
...
| 2026-09-14 | 6 | A URL query parameter (`?sslmode=disable`) is sufficient to override the CLI's sslmode resolution | The query param never reached the CLI's resolution logic at all — Sec traced it to `legacy-db-config.parse.ts`'s `url.searchParams.get("sslmode") ?? svc("sslmode") ?? PGSSLMODE` chain and the DSN being built inside a single-quoted `sh -c` body, where a surviving bare `?` looks correct in the file and isn't at execution time. `PGSSLMODE=disable` set as an environment variable (belt-and-braces, both on the compose `environment:` block and the bootstrap `exec` call) is what actually connected. | ✅ Fixed — both mechanisms now set; bootstrap apply completed clean, all verifications passed; PR #759 |
