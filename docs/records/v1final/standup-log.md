...
| 6 | Deploy the four services from one `main` sha | DevOps | 🟡 **Phase A DONE 2026-09-14**; Sec ruled on TLS (§7.36 item 26, F/CTO-ratified) — `sslmode=disable` ACCEPTED for migrator, landed in compose+runbook; `MIGRATOR_DB_PASSWORD` rotated (disclosure incident closed); Phase B step 4 ⛔ **still blocked**, 5th attempt — CLI ignores `?sslmode=disable`, forces `require` regardless; Sec's fallback question (CLI pin vs. reopen TLS-on-db) not yet decided |
...
⚠ **A second, more consequential finding surfaced verifying the rotation — reported and cleared with F/CTO/Sec before proceeding (see PR #757 thread for the full exchange).** A names/booleans-only measurement of the running `migrator` container (`docker compose exec -T migrator env | cut -d= -f1`, plus per-name non-empty checks — never a raw value dump) showed it holds **66 real, non-empty environment names**, not the 1 the compose file declares (`PROD_DB_URL`) — including `JWT_SECRET`, `SERVICE_ROLE_KEY`, `POSTGRES_PASSWORD`, `VAULT_ENC_KEY`, `ANON_KEY`, `SECRET_KEY_BASE`. The same pattern held for `meta` (7 declared, 74 actual). **This falsifies the C7 "confinement-by-non-reference" property** as previously stated (that Sec's own credential-disclosure disposition had cited as bounding the blast radius of the first incident) — the property was inferred from the compose file's declared `environment:` block, not measured against the live container. **Mechanism, source-cited:** `docker inspect` on the container shows `com.docker.compose.project.environment_file` pointing at a per-deploy `.env` Coolify itself generates (no `env_file:` directive exists anywhere in our compose source); every container also carries Coolify-injected `SERVICE_NAME_*` discovery variables absent from our compose entirely. **Coolify's own deploy pipeline expands the full per-resource env store into every service container it starts, regardless of that service's declared `environment:` block** — not a defect introduced by this PR, and not something the upstream reference compose's own declarations would explain (measured: our `meta` block, mirroring upstream, declares only 7 names). **Scoped the original disclosure exactly, per F/CTO/Sec request:** re-reading the first incident's own filtered command (`env | grep -i "ssl\|PROD_DB_URL"`) shows it printed exactly 3 lines — `COOLIFY_URL`, `PROD_DB_URL`, `COOLIFY_FQDN` — and only `PROD_DB_URL` carried a credential (`COOLIFY_URL`/`COOLIFY_FQDN` matched the filter by accident, because their *values* contain `sslip.io`). **The completed rotation fully covers that incident.** The broader confinement finding is a separate, standing item — booked, not fixed, in this PR (§7.36; also feeds ADR-072's C7 record, held for Sec/whoever owns that ADR edit).
