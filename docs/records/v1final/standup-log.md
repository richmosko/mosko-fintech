...
| 27 | `realrun27-remaining.clean.log:26-49` — `remaining-checks`: TZ-1/CA-7 VERIFIED; RLS FAILED — the 4 zero-grant tables now report DENY-ALL "structural conjunction verified... row observation INCONCLUSIVE (privileged count is 0 too — nothing to isolate, never reported as DENY-ALL fully demonstrated on an empty table)", a materially different (and more careful) report shape than run 23's bare "0 policies" WARN; separately, **`pfin.asset`: 7 row(s) visible to a session with NO tenant identity established (`SET ROLE authenticated`, no `request.jwt.claims`) — expected 0, logged explicitly as "a live RLS bypass, not a fixture artifact."** auth-login: login/signup-validation both VERIFIED; the Resend send-acceptance probe FAILED with `rc=127` (command not found inside the auth container). Overall exit 2. | the DENY-ALL/INCONCLUSIVE reporting shape (Sec's own population-declaration ask) is BACKLOG item 93, booked this PR; the `pfin.asset` RLS bypass and the Resend `rc=127` precondition are NOT booked in this PR — **flagged to team-lead/Sec as open, unresolved findings from this run's own raw log, status elsewhere unconfirmed by this DevOps agent** |

**State after run 27 / PR #882:** `provision.sh` steps 1 through 26 (`cutover`) are ALL now genuinely scriptable, and the `dns` step in particular has FOUR independent live VERIFIED confirmations (runs 22 partial/24/26, plus this PR's own fence coverage) — the last unconditional-MANUAL step in the registry (`cutover`) is now a real, reconfirmed-every-run check, not a permanent human stop. `docs/deployment-runbook.md`'s Part 3 "four unavoidable manual moments" list updated accordingly (item 2: the incumbent tear-down itself stays BY HAND and one-way; verifying it is gone is now SCRIPTED). `remaining-checks`, by contrast, has FAILED on every one of its four logged runs (23/25/27, plus an unlogged run) so far — TZ-1 and CA-7 pass consistently; RLS and auth-login have not both passed together in any captured run. BACKLOG items 88 (curl-`000`-fails-open audit class), 89 (the "025 exclusion (ii)" migration mis-citation, routed to Architect), 90 (the redeploy-idempotence gap, run 24), 91 (provision-vps.sh's argv-exposed Hetzner token), 92 (scheduled maintenance scheme), and 93 (smoke-remaining-checks.sh's population-declaration gap, run 27) are the open follow-ups from this run cycle. The run-27 `pfin.asset` RLS bypass and Resend `rc=127` findings are surfaced above but **not** booked as BACKLOG items by this DevOps agent — they sit outside this role's scope (RLS is an Architect/Sec/QA surface) and their current disposition is unconfirmed from this agent's own vantage point.
