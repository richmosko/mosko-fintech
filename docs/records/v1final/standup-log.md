...
**Baseline:** started on `main` `0a935732` (post-#836), ended on `main` `75368e19` (post-#840); #841 (D-1/D-2 deploy-app.sh fixes) fixes the container-resolve step this record's step 6 hit, pending merge as of this writing — the required post-deploy re-run (D-3 below) happens after it lands.
...
**Pending, after #841 merges:** one idempotent re-run of step 6 (`deploy-app.sh ... --apply`) so `--require-network`/`--resolve-host`/`--health-path` are measured by the script itself, not by hand (D-3). **Stage B (Domain/DNS cutover, §7 step 7's `DEPLOY_ON_SUCCESS` flip + re-exercise, backfill walk, Plaid Link) has NOT started.**
