...
**Sitting-sheet step 6 — PR #806 (migration `120`) merges, the push trigger fires for real, MEASURED, 2026-09-18. PHASE D IS PROVEN, BOTH TRANSPORT AND CONTENT.** PR #806 merged at `9030a62b`. The real **`push`-triggered** run (`35394373476` — not a manual dispatch, the actual steady-state path) paused at the `production-migrator` gate. Redeploy confirmed before approval: `.build-sha == 9030a62b`, `120_account_comment_linked_source_correction.sql` last in the container's migrations directory. Approved. Orchestrator: integrity check OK → pre-fire uuid snapshot → execute → bound to execution `qxtjevwmpms2swokbkjqwwam` by set difference → *"outcome verified via the execution's own message: build-sha (9030a62b...) matches the triggering commit, ledger top row (120) matches the newest migration file (120)"* → SUCCEEDED, deploy SUPPRESSED. **This is residual (1)'s discharge: the FIRST non-degenerate delivery-assertion pass — something was actually applied (`119` → `120`), not the `119 == 119` no-op every prior pass observed.** **F/CTO's own independent `supabase_admin` read on the box, outside the orchestrator's own self-report, confirms this from two further angles:** `select max(version) from supabase_migrations.schema_migrations` → `120` (agrees with the orchestrator's `PFIN-LEDGER-TOP` by two independent reads, not one script trusting itself); `obj_description('pfin.account'::regclass, 'pg_class') like '%was DEFERRED%'` → `t` (migration `120`'s own comment content is live in the catalog, not merely a version-number bump). Full detail recorded at `docs/deployment-runbook.md` §6.5's status line (this same PR).

**What remains unmeasured after step 6:** the pre-fire read-back's MISMATCH branch (exit 10, residual (4)); exits **13/14/15** and the **≥2-new-uuid** branch (residual (3)), live; the **`DEPLOY_ON_SUCCESS=1` leg** — the actual app-deploy call — deliberately never exercised this sitting, proven separately at §7 step 7.
