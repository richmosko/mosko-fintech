...
| 29 (ADR-074 auth-email stand-up, ties to §7.3's own DevOps ownership, not a `dns`/cutover run) | `realrun29-stack-dryrun.clean.log:14-19` — the `--dry-run` preflight for `provision-supabase-stack.sh` correctly detected a real Resend SMTP credential in `.env` and reported it would OVERWRITE the stack's SMTP_* placeholders. The subsequent real `--apply` (F/CTO terminal output, 2026-09-23, not captured to a `temp/` log) PATCHed the Coolify store's `SMTP_PORT` to `587` successfully but did NOT redeploy: `EMAIL_ENV_CHANGED` only diffed `SITE_URL`/`MAILER_TEMPLATES_*`, never any `SMTP_*` key, so the running `auth` container kept serving its prior port (team-lead box measurement: auth container env `GOTRUE_SMTP_PORT=465` against a store already reading `587`). The subsequent invite call then died on a fixed-name `docker inspect supabase-envoy` lookup (Coolify names every stack container `<service>-<uuid>-<timestamp>`, never the compose file's own `container_name:`); once worked around, it 504'd — traced via `nc -z` from the `auth` container's own network namespace: outbound port 465 BLOCKED, 587 OPEN (a Hetzner default egress policy). | #886 `e2d49dc2` (container resolution by compose project+service, never a fixed name), #887 `1ffe4b9d` (`SMTP_PORT` 465→587), #888 `05de4510` (redeploy fires unconditionally on an operator SMTP/SITE_URL seed; post-check's own `auth` container also resolved by compose, not a fixed name) |
| 30 | `realrun30-remaining.clean.log:28-59` — `remaining-checks`: TZ-1 VERIFIED (`:50`), CA-7 VERIFIED (`:51`), RLS (`:28-39`): **5 of 22 tables proven isolated; 17 inconclusive with populations declared** (4 allowlisted DENY-ALL tables — structural conjunction verified, row observation inconclusive on an empty table; `pfin.asset` HYBRID — global-row visibility verified, tenant isolation inconclusive on 0 owner-bearing rows; the remaining 12 empty, nothing to isolate), never the bare word VERIFIED the script's own roll-up prints; auth-login (`:44-46`): login 200, signup-validation 400, Resend send-acceptance 200; overall MANUAL (`:53-54`) — the email-confirmation round trip itself. **The founding account was then created live** (F/CTO terminal output + team-lead relay, 2026-09-23, not itself in this log): `scripts/invite-user.sh --apply` sent the invite, the link resolved to `/auth/confirm?token_hash=…&type=invite`, followed through to `/reset-password`, password set — `auth.users` measured `1/1/1/1`, as relayed. **This live success ran via a HAND-RUN `deploy-app.sh` against a store already holding `SMTP_PORT=587`, NOT PR #888's own code** — #888's unconditional-redeploy branch and its compose-resolved post-check have not yet executed live. Only `MAILER_TEMPLATES_INVITE` is proven delivered/served this way. `CONFIRMATION` is the one template PR #888's post-check asserts at the container, but that post-check has not executed live either (see the hand-run note above) and no confirmation email has ever been click-tested; `RECOVERY`/`MAGIC_LINK`/`EMAIL_CHANGE` are unverified at both layers. GoTrue fetches each `MAILER_TEMPLATES_*` value as an HTTP URL at send time and falls back to its own stock body on a fetch error, so this is a per-template failure mode, not a single pass/fail — the invite's success gives the other four no coverage. F/CTO may next run the forgot-password round trip, which would close `RECOVERY`. | n/a for `remaining-checks` itself (its own scriptable legs VERIFIED/proven as stated); the live invite + password-set was hand-executed, not scripted by this run; every production `--apply` now restarts the stack (PR #888's accepted cost — F/CTO may revisit) |

**State after run 28 / PR #882 (merged `391954b4`) + this PR:** `provision.sh` steps 1 through 26 (`cutover`) are ALL now genuinely scriptable, and the `dns` step in particular has FIVE independent live VERIFIED confirmations (runs 22 partial/24/26/28, plus this PR's own fence coverage) — the last unconditional-MANUAL step in the registry (`cutover`) is now a real, reconfirmed-every-run check, not a permanent human stop, and has itself been reconfirmed live post-merge (run 28). `docs/deployment-runbook.md`'s Part 3 "four unavoidable manual moments" list updated accordingly (item 2: the incumbent tear-down itself stays BY HAND and one-way; verifying it is gone is now SCRIPTED). `remaining-checks`, by contrast, has FAILED on every one of its four logged runs (23/25/27, plus an unlogged run) so far — TZ-1 and CA-7 pass consistently; RLS and auth-login have not both passed together in any captured run. BACKLOG items 88 (curl-`000`-fails-open audit class), 89 (the "025 exclusion (ii)" migration mis-citation, routed to Architect), 90 (the redeploy-idempotence gap, run 24), 91 (provision-vps.sh's argv-exposed Hetzner token), 92 (scheduled maintenance scheme), 93 (smoke-remaining-checks.sh's population-declaration gap, run 27), 94 (`resolve_app()`'s own argv-exposed Coolify token, same class as 91, travels with a false-precedent comment fix), and 95 (low-priority: live-exercise TINKER-WRITE-ALLOW-09's arrival-FINDING path, still unmeasured after runs 26/28) are the open follow-ups from this run cycle. Run 27's `pfin.asset` classification (design-by-ADR-060, not a bypass — see the corrected row above) and the Resend `rc=127` precondition were both fixed in PR #883 (merged `2f7498dd`), not open findings from this run cycle. This PR (`chore/run28-ledger-provenance`, #884) also fixes the DIRTY-banner ambiguity run 28's own qualifier (a) surfaced, and normalizes `fence-tinker-write-allowlist.sha256`'s filename field to match its own `.txt` header's documented regeneration command (hash value unchanged, confirmed by diff).
