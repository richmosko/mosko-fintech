...
### §7.3 — the §2/§9 Domain/DNS cutover, EXECUTED against production, 2026-09-21/22 (runs 21–25, PRs #877–#880 + this PR) — DevOps, team-lead relaying live run results; ⚠ this DevOps agent worked from team-lead's own dispatch messages, not the raw run logs directly (`temp/runlogs-2026-09-21/realrun2{1,2,4}*.clean.log`, gitignored, not present in this agent's own worktree) — run 21/22/24's own facts below are cited at the line numbers team-lead gave; run 23 is recorded only to the single fact Sec's own PR #879 review surfaced in passing (below), and this entry does not claim fuller run-23 coverage than that

| run | finding | fix PR (tip) |
|---|---|---|
| 21 | `www.pfindash.com` already existed as an A record (not a CNAME); `assign-app-domain.sh` only ever looked for a CNAME, so Porkbun refused the CNAME create with HTTP 400 (`curl -fsS` on the write call discarded the response body, so the operator only ever saw a bare transport error, never Porkbun's own `message` field) | #877 `e33896f0` (www-type dispatch A/CNAME/none; status-preserving `porkbun_api()`; Sec F-1 key-scrub) |
| 21 | (same run, second defect) `docker_compose_domains` PATCH succeeded but the read-back parser was wrong — Coolify returns a JSON **string** whose own content is a JSON **object** keyed by compose service name, not the flat comma-separated list the parser assumed (COOLIFY-FACT-15) | #878 `a71594c5` (parser fix; redeploy-required-before-env-read addenda; Sec's 3-round sslip-probe predicate correction, closing with the curl-faithful-exit-status fixture fix) |
| 22 | DNS/ports already correct (no-ops); `docker_compose_domains` PATCH read-back exact-matched; redeploy finished; post-redeploy env read + sslip probe both correct — but the cert poll FAILED after its full bound: it asserted a bare `== "200"` on `/`, which can never pass once the app gates an unauthenticated `/` behind auth (measured live: `https://pfindash.com/` → `303` → `/login` (`200`), `ssl_verify_result=0`, real Let's Encrypt cert CN=`pfindash.com`) | #879 `010fbd7d` (initial fix: `-L --max-redirs 5`, final 2xx + `ssl_verify_result==0` — Sec's own round-1 review then found this predicate too loose (a redirect chain to an off-domain host, or a scheme-downgrade to plain `http://`, would both pass) |
| 23 | (fact surfaced during Sec's PR #879 review, not independently recorded here beyond this) a step's own failure output printed nothing useful on the FAILED path — team-lead's own flag, closed by PR #879's `poll_domain_serves()` exhaustion `die()` naming the last observed (token, code, `ssl_verify_result`, effective-url) tuple explicitly | fixed as part of #879 `d9f104a4` |
| 24 | dns step VERIFIED end to end (THIRD independent confirmation, after run 22's first): new container `df261696df7e` carried the expected env values; sslip probe MEASURED both sides identically (`http=404`/`https=503`, no second route — corroborating COOLIFY-FACT-16 under a second run); cert poll's ROUND-2 corrected predicate reported `final http 200, effective https://pfindash.com/login` for BOTH apex and www — the first LIVE confirmation of that specific predicate (rounds 1/2 had been fence-verified only until this run) (realrun24, lines 75–79 per team-lead's own citation) | n/a for the dns step itself (VERIFIED, not a fix); booked BACKLOG.md item 90 (the redeploy-fires-on-every-run idempotence gap this run exposed) and appended this run's own measurement to COOLIFY-API-MEASURED.md's FACT-06 |
| 25 (F/CTO, direct Hetzner API measurement, not a `provision.sh` run) | `GET /v1/servers` (token via `curl -K` stdin config, never argv) against the Hetzner project: exactly ONE server — `pfin-prod-1`, `cax21`, running, created `2026-09-10`. The incumbent `pfindash.com` box had therefore ALREADY been torn down by hand before this measurement (exact tear-down date not recorded here — F/CTO reported "days ago" relative to 2026-09-22) | this PR (`feat/cutover-step-verified`) — `run_cutover()` in `scripts/provision.sh` is no longer an unconditional MANUAL stop; with `--confirm-cutover` it lists the Hetzner project's servers and VERIFIES exactly one, named `pfin-prod-1`, printing `ok  cutover: the Hetzner project holds exactly one server (pfin-prod-1); the incumbent is gone`. More than one server → MANUAL, naming the extras (this script never deletes anything, on any branch). Sec review pending on this PR (token-never-in-argv path). |

**State after run 25 / this PR:** `provision.sh` steps 1 through 26 (`cutover`) are ALL now genuinely scriptable and, on the current production state, all VERIFIED — the last unconditional-MANUAL step in the registry (`cutover`) is now a real, reconfirmed-every-run check, not a permanent human stop. `docs/deployment-runbook.md`'s Part 3 "four unavoidable manual moments" list updated accordingly (item 2: the incumbent tear-down itself stays BY HAND and one-way; verifying it is gone is now SCRIPTED). BACKLOG items 88 (curl-`000`-fails-open audit class, from PR #878's Sec re-review), 89 (the "025 exclusion (ii)" migration mis-citation, Sec finding on this PR, routed to Architect), and 90 (the redeploy-idempotence gap, run 24) are the open follow-ups from this run cycle.
