...
- [ ] **L-1 SELF-365** — comment: rulings §G.1–§G.7 landed at this record (path + sha); AC (b) is read as PRD §3.4(b); the security-catalog check formerly at (b) is re-homed to the Phase 6 exit walk (S-2); (c) clause (1) / (4) readings per §G.4 / §G.5; sub-issue list = B.1–B.4 (+ B.5 as blocker).
- [ ] **L-2 SELF-375** — comment: definition ratified verbatim (§C.1 wording); carrier M-3 (recorded query per month record + QA battery leg on `audit_log_surface_name_vocab`); M-1 staged at BACKLOG §5.6; **citation correction** — "PRD §6" in the description means `pm-findings.md` §6 (D-2). Executor for the leg: QA (team-lead assigns).
- [ ] **L-3 SELF-378** — comment: ruled T-1 (population unchanged, pinned on SELF-351); T-3 staged at BACKLOG §7.35 item 1; T-2 not taken (Sec-visible impersonation-population change never proposed as posture). Disposition for the issue itself: team-lead (Done-by-ruling, or kept open until the SELF-351 comment lands).
- [ ] **L-4 SELF-351** — comment pinning the AC: *the monthly-report cron's tenant population is account-owning tenants — `select distinct users_id from pfin.account` — per F/CTO ruling 2026-09-07 (SELF-378 T-1); a no-account tenant receives no draft, no pending item, no notification; the empty-state cue is BACKLOG §7.35 item 1.*
