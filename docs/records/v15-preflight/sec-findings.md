...
## R2.9 — Round 2b: answers to Architect's round-2 positions

⚠ **Ref note — the ref moved between the brief and this pass, and the brief was correct when written.** The round-2b brief named `origin/meta/v15-preflight-arch` @ `54af84a`, `architect-findings.md` md5 `842a3f7a…`. I verified that: `git show 54af84a:…| md5` returns `842a3f7aa6c4be6d1171d1d65438d291`, an exact match. **But the branch tip is now `d87c9dc`** (`architect-findings.md` md5 `ef390235c5b44fdd9be16482f215e8a3`; `rederived-acs.md` md5 `d53dd3970920ff1f5ecf9a8780f5a8d4`), Architect having pushed a round 2b — *"apply PM's by-block fixes; PM concedes the one-way door"* — after the brief was cut. **I answer against `d87c9dc`, the live tip**, and record the skew rather than silently answering a superseded document. `origin/meta/v15-preflight-pm` has likewise moved to `77425b3` (`pm-findings.md` md5 `eb7bcb2cc0bec0a737dad1708e79e45d`). `origin/main` unmoved at `b90b846`, so every tree measurement in this file stands as taken.

### (1) ⟨RULING R-1⟩ — I move to B. Architect is right and my A was worse than he says.

**I withdraw lean A.** Not softened, not conditioned: A is wrong, and F/CTO should read this as a straight concession.

**Architect's claim (i) is correct, and A's defect is worse than "the index forbids it" — A is internally incoherent.** My round-1 text read: *"the locked partial-UNIQUE already makes 'the current final' unambiguous; `superseded` becomes a presentation label computed from `(target_month, generated_at)` ordering."* Those two clauses contradict each other in the same sentence. `UNIQUE (users_id, target_month) WHERE generation_status = 'final'` makes the current final unambiguous **by permitting exactly one such row** — so there is never a set of `final` rows to order by `generated_at`, and never anything to label `superseded`. I asserted the constraint as A's *support* while proposing a mechanism the constraint's own semantics exclude. **I did not check my lean against the locked index before calling it "the only option where nothing is added and nothing is weakened."**

The two ways A could be made to work are both amendments, and one is worse than an amendment:
- **Retire or narrow the locked partial index** — an amendment to Decision 15's locked text, which is exactly the cost I claimed A avoided.
- **Insert every row as `draft` and derive `final`** — then the partial index **never fires on any row**, a locked uniqueness constraint reduced to a dead one. ⚠ That is the *constraint that cannot fail* shape I flag in other people's work, arrived at by my own proposal, and it is strictly worse than a recorded amendment because a dead index looks identical to a working one.

**Architect's claim (ii) is also correct.** Promotion `draft → final` is an UPDATE of `generation_status`, and every commentary save before it is an UPDATE, so Decision 2's blanket *"UPDATE/DELETE blocked"* was never literally true of this table under Lock 11's own locked vocabulary. I treated the blanket phrasing as the binding fact and reached for a mechanism to preserve it; the binding fact is the vocabulary. **His reframing is the right one: the question is not how to avoid an UPDATE, it is where the mutability window closes and what fences it.**

**Position: Option B as he shapes it** — the trigger permits any column while `draft`; `generation_status` on the single monotone `final → superseded` transition; nothing else, ever; `users_id` / `target_month` fenced in every state; no DEFINER, no allowlist change, the locked index working as written. I accept his named losing side (Decision 2's blanket claim stops being literally true of this table and the ADR must say so) and note his mitigation is the right one: a **monotone transition on one column** is mechanically checkable, where a column allowlist is a list that grows.

**Four residual conditions on B. They are additions to a sound shape, not objections to it.**

1. ⚠ **The restated shape governs UPDATE and drops DELETE.** Decision 2's rule is *"UPDATE/DELETE blocked"*; B's three clauses are all about UPDATE. `authenticated` must hold INSERT on this table for the on-demand path, and if it holds DELETE, a user can destroy a `final` report — an audit-class artifact whose PRD contract is indefinite retention with *"user-initiated deletion of historical reports"* explicitly V2+. **Require: DELETE blocked on any row not in `draft`, by the same trigger, with a battery leg.** This is the enumeration-stops-one-short shape applied to a two-verb rule restated with one verb.
2. **State that the trigger is NOT role-conditional.** A trigger binds `service_role` where a policy does not, which is precisely why Decision 2 specifies the *DB-trigger layer* — so B's mechanism is correct as chosen. But A2 item 4(iii) names its `service_role` bypass trigger explicitly and A1 names nothing, and the cron **will** be performing the `final → superseded` UPDATE under `service_role`. The realistic failure is an `if current_user = 'service_role' then return new; end if;` added later to make the cron work. **Require: the same monotone rule binds `authenticated` and `service_role` identically, and the battery proves the refusal under BOTH roles** — a leg run only as `authenticated` passes with the exemption in place.
3. **Constrain the legal INSERT states, not only the legal transitions.** A transition guard governs UPDATE; it is silent on a row written directly in the target state. With `authenticated` holding INSERT, a row can be POSTed straight in as `final`, taking the month's single `final` slot without ever passing the author-before-generate gate — the gate bypassed at the DB layer rather than in the UI. **Require: the AC states which states a row may be INSERTed in, and the trigger enforces it.** (Whether a skip-authoring path legitimately inserts `final` directly is a design call, not mine.)
4. **`superseded` is terminal.** B's clause (ii) implies it by naming one transition; say it, so the battery has an obvious leg. And one line for the runbook: this trigger is the **only** applicable layer for an RLS-exempt writer, and per ADR-011 Decision 4's 2026-09-03 amendment it goes inert under `session_replication_role = replica` — so any bulk-load or restore path touching this table owes an explicit post-load validation step.

**Does B discharge my A1 AMBER (item 6)? Yes on the first half, conditionally on the second.**
- **Whole-row-on-final mechanism — DISCHARGED.** B's clause (iii) *"nothing else, ever"* is exactly the mechanism item 6 lacked. My AMBER said everything needed was named in the block but the mechanism was missing; B supplies it.
- **`service_role` on A1 — discharged only once condition 2 is written into the AC.** The trigger is the right mechanism and binds `service_role` by construction, but the block still says nothing about it while its sibling block does, and that asymmetry is what I flagged. **With conditions 1–4 folded, A1 moves AMBER → GREEN.**

### (2) §8.5 — I back **A10**, and the argument is the trigger's attachment point, not tidiness.

**Position: open A10 as a new A-item carrying `sec-joint-review`.** My R2.5 item 3 established the write is D2-mandatory; what A10 settles is *where the trigger attaches*.

The governing rule is ADR-011 Decision 2 — Lock 11 is one of its three named ratified surfaces — plus my standing joint-review list. **The reasoning that decides the fold-vs-open question is ADR-064 Decision 5's, and I am citing it as a precedent for the reasoning rather than as jurisdiction over this table** (its rule is scoped to `pfin.account_trans` write surfaces and does not reach `monthly_report`; conflating the two would be the false-composite class). Read verbatim at `b90b846`, D5 records that the PR which would have destroyed positions reasoned in good faith that *"Sec look is courtesy, not mandated on its face (app-layer fix on an already-fenced path)"*, and rules: **"The trigger is the surface, not the layer, and not the author's assessment of risk."**

That sentence is the whole answer. Folding the on-demand write into P5 does not change what the surface is; it changes only which layer the issue *looks like* it belongs to — a SvelteKit page — and D5 says that assessment is not the trigger. Architect's mechanical point is the same one from the other side: folded, the reviewer is Frontend and the gate is not `sec-joint-review`. And D5's own ⚠ bullet is decisive for the vehicle: it records that the rule *"is not self-enforcing"* and that the routing trigger *"needs to sit where the router and the builder actually look."* **A separately-labelled issue is that place. A fold relies on a reviewer remembering, which is the rot D5 exists to prevent.**

**Fallback, stated so this is not binary.** If F/CTO prefers PM's fold-all-five, the fold is acceptable **on two conditions**: P5 itself carries `joint-review:sec`, and its AC names the Decision 2 obligation and the RT-25 server-derived-`data_as_of` obligation explicitly rather than inheriting them. I would rather have the label on P5 than a nineteenth issue that gets dropped at promotion — which is Architect's own named losing side for A10 and a real risk.

### (3) §8.7 / A4 sequencing — **my named disagreement at R2.5 is WITHDRAWN**, on two conditions.

Architect has conceded F-5 in my favour, adopted F-6, re-scoped A6's block in place, and added the sequencing constraint that A4's `package.json` may not land before the manifest fence. That is further than I asked, and holding a disagreement past a concession would be bad faith. **Withdrawn.**

Two conditions, and the first matters more than the sequencing does:
1. ⚠ **A sequencing constraint stated in an AC is a convention with no mechanism, and conventions with no mechanism rot silently.** Make it a **blocking dependency edge** from the successor issue to A4 in Linear, not a sentence. Better still, give the fence a shape that makes the ordering moot: **audit `workers/pdf-render/package.json` if it exists and pass if it does not.** Then the fence can land any time, is a no-op until A4 creates the file, and bites on A4's first commit — the ordering requirement disappears rather than being remembered. Note this differs from the shipped RT-22 Dockerfile fence, which exits 2 on a missing target; that is correct there because its target already exists, and copying it here would red CI from the day it lands.
2. **The successor's catch criterion and its golden fixture come to me before merge** — a fence-boundary change is joint-review-mandatory and an escalation trigger regardless of who proposes it. A fence that does not fail closed is theatre, and the fixture is the only thing that proves it does.

⚠ **One thing must not be lost in the close-and-successor split:** the reason the gap exists is documented *inside the shipped fence's own header* (it names manifest inspection as a deliberate non-catch). The successor issue must cite that header, or a future reader finds a fence that already says it does not do this and concludes the successor is redundant.

### (4) §8.6 item 2 / S-5 — convergence recorded.

Both leans are now **(a) ship the column with the dormant fence**: I moved from B in round 2 (ADR-035 names a V1.6 consumer, so the instance is deferred-with-a-consumer rather than orphaned, and Decision 3's own `#15`/`#16` record shows fold-ins get forgotten — a fence that must be *remembered* at V1.6 will not exist at V1.6), and Architect kept (a) while recording that my B was stronger on the fence-cannot-fire point. **My condition is the answer to exactly that concern and it stands: the paired QA leg is labelled construction-only in its own text** — it asserts the fence exists and is attached, not that it fires — so no later reader mistakes a leg that cannot fail for coverage. PM abstains (no product stake), so this is no longer a split.

### (5) N-3 — open, no action taken.

The ARCH `:208` reading (*"user-session only — never invoked from a worker"*, against A7's cron invoking A3) remains open pending Architect's 2b statement of intent. I have taken no position and still do not know which reading is intended; noting only that whichever it is, the sentence should end up unambiguous on the tree rather than resolved in a records file.

### Carried from the siblings' later refs — one item that resolves a residual of mine.

PM's round 2 (via Architect §9.5) supplies the point that settles the escaping-control home under **every** S-2 outcome, and it is better than either my R-5 or Architect's §8.4 conditional: **under S-2 Options A/C the control is discharged structurally, but the PROOF LEG is still owed, because ADR-013 INV-2 spans both engines regardless of who escapes.** That makes **P6 the home in all cases** — vindicating my round-1 R-5 lean for a reason I had not given, while my round-2 M-2 retraction (the *control's* placement follows S-2) stays correct. **Control placement follows the ruling; the obligation to prove it does not.** The §7.32 booking must still be cited by name wherever it lands, and its scope note travels with it — it was drafted against `schedule_label` before the commentary columns and `owner_id_header_text` existed and reaches them only through its *"every other free-text field"* clause.

### R2.9 ledger statement

No catalogued §10 instance is added, removed, reordered or renumbered by this round; no layer attribution moves; no Decision 3 label is drafted or reallocated. Path B throughout, no tally carried. The CI-fenced set and the §10 catalogued set remain different sets and are not reconciled here — R2.9 item (3) concerns the CI-fenced side only.
