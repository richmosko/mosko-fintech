...
**Round-2 third pass — Sec's round 2 applied.** Sec's round 2 (`origin/meta/v15-preflight-sec` @ `04bec6e`, md5 `e3cdafb95f7ae2fcd8d73ab8ba5851fd`, hashed before reading) carries per-block pre-verdicts at **R2.6** against Architect's **round-1** blocks (`66288e0`). Applied here: the one **RED** (A7 item 5 → `⟨RULING⟩` **A7-AUDIT**) and the two genuinely-new **AMBER** (A1's immutability mechanism + `service_role` fence; A4's resource-loading fence and **N-4** compose-manifest wiring), plus **N-5** (commentary length bounds), **N-3** (the ARCH `:208` reading, stated at A3 and cited at A7), **D-4/N-6** (marked `⟨RULING D-4⟩`, **not** decided), and Sec's S-1 / S-2 positions in `architect-findings.md`. ⚠ **Three of Sec's five AMBER were already fixed in the second pass** (A2's RT-21→RT-20, A8's review classification, P10's conditional tri-axis) — **ref-skew, not disagreement**, marked in their blocks.
...
>    - ⚠ **Each commentary column carries a CHECK-enforced length bound** (Sec **N-5**), mirrored in P3's Zod schema. Bare `TEXT` here would leave P3's *"length bounds"* battery as a **single-layer app control on a Lock 14 write path**, and ADR-011 Decision 4's user-facing-surface class is explicitly a multi-layer commitment. The consumer is a browser engine rendering a PDF, where unbounded prose is a memory and render-time cost it is not on a scrollable web page. **Bound chosen by PM against the parity artifact** (PM set 120 for the owner string; the commentary bodies are prose and want a larger number — `⟨PM⟩` the figure, not the mechanism). **Catch criterion (Sec):** a body one byte over is rejected 400 at the app layer **and** the same value is rejected by the DB when submitted directly through PostgREST — *"two facts that can disagree, which is what makes it a real second layer."*
> 2. **`included_reconciliation_event_ids INTEGER[]` — S-5 is CLOSED and CONVERGED on (a): ship the column with its dormant Lock 11 mod #9 fence.** Sec moved from B to (a) at **R2.9(4)** on two grounds worth keeping: ADR-035 names a **V1.6 consumer**, so the instance is *deferred-with-a-consumer* rather than orphaned; and Decision 3's own `#15`/`#16` record shows **fold-ins get forgotten — a fence that must be *remembered* at V1.6 will not exist at V1.6.** PM abstains (no product stake), so this is no longer a split.
>    - ⚠ **Sec's condition, which answers the fence-cannot-fire objection Architect recorded against its own lean, and it stands: the paired QA leg is labelled CONSTRUCTION-ONLY in its own text** — it asserts the fence **exists and is attached**, not that it fires — *"so no later reader mistakes a leg that cannot fail for coverage."*
>    - The migration header states in terms that `pfin.reconciliation_event` has no writer at this sha, so the fence is correct, mandatory and **dormant**, and names the revival condition (the V1.6 statement control tie-out successor recorded at [ADR-035](DECISIONS.md#adr-035)).
...
> 6. **Mutability window and its ENFORCEMENT MECHANISM** — PM **D-6** for the window; ⚠ **Sec R2.6 AMBER (bordering RED) for the mechanism, which round 1 asserted without naming.** Commentary is editable while the report is `draft` and frozen at `final`.
>    - ⚠ **A whole-row-on-final immutability trigger is required, not a two-column fence.** Round 1 fenced `users_id` + `target_month` and *asserted* that a `final` row is immutable, with **no item naming what blocks UPDATE on a final row**. Built verbatim that ships the wave's canonical Decision 2 surface under-fenced. The trigger permits: (i) any column while `generation_status = 'draft'`; (ii) the single monotone `final → superseded` transition on `generation_status` (per `⟨RULING⟩` Sec R-1, Architect's option B); (iii) nothing else, ever. `users_id` and `target_month` are fenced in **every** state (Lock 12's Sec catch: a parent re-tenant orphans A2's children from their original tenant).
>    - ⚠ **`service_role` must be fenced on THIS table too** (Sec R2.6 A1). ADR-011 Decision 2 verbatim requires append-only *"across both `authenticated` AND `service_role` roles"*; **A2 item 4(iii) carries a `service_role` bypass DB-trigger for the child and A1 had no equivalent.** Both halves are Decision 2's, and a child fenced against a role its parent is not is a fence with a door beside it. ⚠ Recall that `service_role` carries `rolbypassrls`, so on this surface the **trigger is its only applicable layer** (ADR-011 D4's 2026-09-03 amendment) — there is no RLS behind it to catch a miss.
>    - ⚠ **`⟨RULING⟩` Sec R-1 is CLOSED — Sec CONCEDES to Option B outright** (`sec-findings.md` @ `fe757d1` **R2.9(1)**: *"A is internally incoherent"* — A's own support sentence proposed deriving `superseded` from a `generated_at` ordering over a set the locked partial index permits **at most one row of**). **The mechanism above is the ratified shape.** Sec attaches **four conditions as additions to a sound shape, not objections**, and with them **A1 moves AMBER → GREEN**:
>      - **(a) DELETE is blocked too — Decision 2 is a TWO-VERB rule and clauses (i)–(iii) govern only UPDATE.** `authenticated` must hold INSERT for the on-demand path; if it also holds DELETE, **a user can destroy a `final` report** — an audit-class artifact whose PRD §2.6.4 contract is indefinite retention, with *"user-initiated deletion of historical reports"* explicitly V2+. **Require: DELETE blocked on any row not in `draft`, by the same trigger, with a battery leg.** ⚠ Sec names the shape: *"enumeration-stops-one-short, applied to a two-verb rule restated with one verb."*
>      - **(b) The trigger is NOT role-conditional.** The same monotone rule binds `authenticated` and `service_role` **identically**, and the battery proves refusal **under both**. ⚠ The realistic defect is named: the cron performs the `final → superseded` UPDATE under `service_role`, so someone later adds `if current_user = 'service_role' then return new; end if;` to make it work — **and a leg run only as `authenticated` passes with that exemption in place.**
>      - **(c) Legal INSERT STATES are constrained, not only legal transitions.** A transition guard governs UPDATE and is **silent on a row written directly in the target state**: with `authenticated` holding INSERT, a row can be POSTed straight in as `final`, **taking the month's single `final` slot without ever passing the author-before-generate gate** — the gate bypassed at the DB layer rather than in the UI. The AC states which states a row may be INSERTed in, and the trigger enforces it. (Whether a skip-authoring path legitimately inserts `final` directly is a design call — see P4 item 1.)
>      - **(d) `superseded` is TERMINAL** — clause (ii) implies it by naming one transition; state it so the battery has an obvious leg. **Runbook line, required:** this trigger is the **only** applicable layer for an RLS-exempt writer and, per [ADR-011 Decision 4](DECISIONS.md#adr-011)'s 2026-09-03 amendment, **goes inert under `session_replication_role = replica`** — so any bulk-load or restore path touching this table **owes an explicit post-load validation step**.
...
>    - ⚠ **`⟨RULING D-4⟩` (sitting agenda item R11) — the commentary column IDENTIFIER is a cross-sibling disagreement and is NOT decided here** (Sec **D-4 / N-6**, which records it as needing a decision *"and neither sibling's file records that the other disagrees"*). **PM (§3 / D-3): the product phrase governs the heading and the identifier `commentary_equity` STANDS** — schema wording governs identifiers. **Architect (F-3): rename to `commentary_marketable_securities`**, because a column on an immutable audit-class table is permanent and `082` already retired the label. **Sec: no security objection either way.** PM's round 2 (§12.3 F-3) adds that PM *does not object* to the rename but that it edits a **Gate B ratify record**, so **whichever identifier is ruled, the decision is stated in the §9 consolidated ADR and must not arrive by migration alone** — Architect concurs with that vehicle constraint unconditionally, and it is the one part of D-4 that is settled. **The heading is `Marketable Securities` under every outcome.**
...
>    - ⚠ **`⟨RULING⟩` N-3 — ARCH `:208` says this helper is *"user-session only — never invoked from a worker"*, and A7 has the cron invoking it. Architect states the intended reading; F/CTO ratifies it, because a third ratified artifact is bearing on S-3.** Verified verbatim at `b90b846` (Sec **N-3**, re-measured here): *"All **reads** flow through a single `SECURITY INVOKER` read-composition helper (**user-session only — never invoked from a worker**), which is also the render source the PDF worker reaches through the V1 web-app's `/internal/pdf-render` endpoint rather than the database directly."*
>      - **Intended reading: the clause constrains the SESSION CONTEXT, not the process identity — and it is PDF-scoped.** *"User-session only"* means the helper always executes under a session where RLS applies and `auth.uid()` resolves to the tenant whose data is being read. The **cron satisfies that by impersonating** (`SET LOCAL ROLE authenticated` + `set_config('request.jwt.claims', …)`) — at the database layer that caller **is** a user session. What the sentence forbids is the thing its own second clause is about: a **worker reaching the database directly**, outside any user session, which is exactly what the PDF worker must never do.
>      - **Why the general reading cannot be right:** under it **A7 could not exist at all**, and ADR-011 Decision 15 / Lock 11 **mod #4 locks a *"V1-SHIP-BLOCK cron tenant-binding discipline"*** — a lock that would be meaningless if the cron could never invoke the helper it binds a tenant for. A reading that voids a ratified V1-SHIP-BLOCK mod is the wrong reading.
>      - ⚠ **This makes Sec F-4 sharper, not softer.** If *"user-session only"* is a session-context constraint, then **claims-without-role does not satisfy it** — `rolbypassrls` remains in force and the session is not a user session in the only sense that matters. The ARCH sentence and Sec F-4's α are the **same requirement stated twice**, which is corroboration.
>      - **Owed:** ARCH `:208` is narrowed to say *"never invoked from a worker's own database connection; a worker composing per tenant does so under an impersonated user session (Lock 11 mod #4)"* — so the next reader does not hit this. Rides the §9 consolidated ADR's doc PR.
...
> 3. ⚠ **The dependency manifest is the live gap** (Sec **F-6**): the moment this issue lands a real Puppeteer app, the standard shape is `COPY package*.json .` then `RUN npm ci`, and the RT-22 fence is **documented not to look at manifests** — `pg` can enter through a path the fence cannot see while the fence reports clean.
>    - ⚠ **The round-2 sequencing constraint here is WITHDRAWN and REPLACED by a fence shape** (Sec **R2.9(3)** condition 1, which withdrew Sec's own F-6 disagreement in exchange): the re-scoped RT-22 fence **audits `package.json` if present and passes if absent**, so it can land at any time, no-ops until this issue creates the file, and bites on this issue's first commit. **The ordering requirement disappears rather than being remembered** — *"a sequencing constraint stated in an AC is a convention with no mechanism, and conventions with no mechanism rot silently."* If that shape is not adopted, the ordering becomes a **blocking dependency edge in Linear**, never a sentence in an AC.
...
>    - Under S-2 Option A the worker holds no listener and loads only from a known app URL, so this fence is not required; it is cheap enough to carry regardless, and the AC should say which outcome it is written under.
> 4c. ⚠ **Compose manifest + RT-27 private-bind wiring — required under S-2 Option B or C** (Sec **N-4**). `scripts/ci/fence-admission-private-bind.sh` finds its target by an in-file sentinel in a **Coolify Compose manifest**, and `workers/pdf-render/` has **none**, so this container's admission endpoint would come up with **no private-bind fence over it**. Ship a manifest carrying the sentinel and wire the RT-27 job to it — *"a wiring change, not a new fence"*, since the script is already generic over its target. **Owner: DevOps; joint-review-mandatory** (a CI fence-boundary change is a standing escalation trigger); ships with a golden fixture that fails closed.
>    - ⚠ **This is an INTRA-instance coverage expansion of RT-27 and changes NOTHING on the §10 catalogued ledger.** Precedent cited rather than re-argued: SECURITY §4.5's RT-30 entry records an *"INTRA-instance allowlist expansion (RT-26 3→4)"* with *"§10 ledger UNCHANGED"*. **No new catalogued instance may be drafted for this channel and no ledger edit is owed** — stated because catalogueing it *"would look like diligence"* (Sec).
...
> 2. **Paired with a golden violation fixture and an inversion-mode step**, exactly as the Dockerfile fence is paired today. *A fence that does not fail closed is theatre* (Sec F-6); the existing job is the pattern to copy. ⚠ **The catch criterion and its fixture come to Sec before merge** — a fence-boundary change is joint-review-mandatory and an escalation trigger regardless of who proposes it (Sec **R2.9(3)** condition 2).
> 2b. ⚠ **FENCE SHAPE — audit `workers/pdf-render/package.json` IF IT EXISTS, and PASS IF IT DOES NOT.** This **replaces** the A4 sequencing constraint rather than supplementing it (Sec **R2.9(3)** condition 1, which withdrew Sec's F-6 disagreement and asked for this instead): *"a sequencing constraint stated in an AC is a convention with no mechanism, and conventions with no mechanism rot silently."* Under this shape the fence **can land at any time, is a no-op until A4 creates the file, and bites on A4's first commit** — the ordering requirement **disappears rather than being remembered**. ⚠ **This deliberately differs from the shipped RT-22 Dockerfile fence, which exits 2 on a missing target**; that is correct there because its target already exists, and **copying it here would red CI from the day it lands.** If for any reason the pass-if-absent shape is not adopted, the ordering becomes a **blocking dependency edge in Linear**, never a sentence in an AC.
> 2c. ⚠ **The successor AC cites the shipped fence's own header** (Sec **R2.9(3)**, final note). The reason this gap exists is documented *inside* `scripts/ci/fence-rt22-pdf-worker-dockerfile.sh` — it names manifest inspection as a deliberate non-catch. **Without that citation a future reader finds a fence that already says it does not do this and concludes the successor is redundant.**
...
>
> **Dependencies.** Downstream: **A4 — item 3 of that block.** ⚠ **No ordering edge is required if item 2b's pass-if-absent shape is adopted**; if it is not, the edge is a blocking Linear dependency, not an AC sentence.
...
>    - ⚠ **`⟨RULING⟩` N-3 applies here, and the AC must carry the reading it is built under.** ARCH `:208` says the helper is *"user-session only — never invoked from a worker"*. **Architect's intended reading — that the clause constrains the session context, which this cron satisfies by impersonating, and that it is PDF-scoped — is stated in full at the A3 block and is not restated here.** Under that reading A7 is coherent and Lock 11 mod #4's cron tenant-binding discipline is what it binds. **Under the general reading A7's whole shape changes**, so this is read at the sitting rather than discovered at A7's joint review.
...
> 5. ⚠ **`⟨RULING⟩` A7-AUDIT — the ADR-011 Decision 1 clause (d) audit row has no writable home. Sec R2.6 grades round 1's *"Lock 13 mod #4 audit-log entry, same transaction"* the file's only RED; it is struck.** PM **D-8** found the named table dropped; **Sec N-2 measured one step further and the precise state is what makes this a ruling rather than a pointer fix:**
>    - `pfin.plaid_sync_audit` appears in `007`, `008`, `015`, `047` and **is not a live table**.
>    - Its successor **does** exist — `pfin.linked_source_sync_audit` (`015:441`), whose `users_id` is commented *"resolved tenant (Decision 1 clause (d))"* and whose `detail jsonb` is commented *"tenant-resolution chain"*. So a D1(d) surface is on the tree.
>    - ⚠ **But it cannot take a report-generation row.** `source` is `check (source in ('webhook','scheduled_poll'))` and `provider` is checked against a provider list with no internal/report member. **A monthly-report generation event fails both CHECKs.**
>    - The **general** audit-log helper is unauthored (`emit_audit_log` / `fn_audit_log` return nothing across the migration tree), consistent with ADR-011 Decision 9's amendment recording that slot as reserved-unauthored.
>    - ⚠ **Built verbatim, round 1's AC either silently drops D1(d) on the wave's only privileged non-JWT writer, or widens two CHECKs on a shipped append-only audit-class table** — a Decision 2 surface change, joint-review-mandatory, *"not something an AC should reach by accident"* (Sec).
>    - **Options (Architect's to draft, F/CTO's to ratify; Sec explicitly has no lean).** **(a) Extend the successor's two CHECKs** — cheapest, but widens an audit-class table's domain from *"sync"* to *"any privileged write"*, and that widening is permanent on an append-only surface. **(b) Author the reserved general helper** — largest, and it discharges a long-standing Decision 9 reservation rather than deferring it again. **(c) A report-scoped audit table** — narrow and clean, but adds a fourth audit surface with its own RLS, grants and immutability triggers. **Architect lean: (b)**, because the reservation exists precisely for this case and (a) buys cheapness by making a shipped table's domain vaguer forever. **Losing side of (b), named:** it is the largest of the three, it puts a new shared surface on V1.5's critical path, and a general helper is where per-surface discipline goes to be forgotten — (c) keeps the blast radius at one table.
>    - ⚠ **Sec withdrew its own round-1 catch criterion here** (*"assert the same-transaction audit row exists and names the resolved tenant"*), because it was written as if the surface existed. **The criterion is not testable until the home is named** — do not carry the old one forward.
>    - Whichever lands, the row carries at minimum the **trigger source** (`cron` vs `on-demand`) and **`data_as_of`** (Decision 19's shape). ⚠ PM §6 shows V1.final's N=2 gate is **unmeasurable without it**, so this is V1.5 work, not a V1.final concern.
...
> 3. A3 cross-tenant leak analysis — a foreign caller gets the **empty/unavailable shape**, i.e. fails closed *into a shape that says so*. Plus Sec **F-4**'s worker leg **with its positive control**.
>    - ⚠ **The no-`rolbypassrls`-EXECUTE leg is a STANDING catalog assertion, not a one-time check** — Sec **R2.5** calls it *"the single most valuable assertion in the file"* and *"the leg that closes F-4 at the database layer regardless of how S-2 and S-3 resolve"*: if no bypass-RLS role can execute the helper, no bypass-RLS caller can compose across tenants through it, whatever the app does. Sec verified the mechanism it rests on — the shipped pattern at `104:913-914` and `105:434-435` is `revoke execute … from public; grant execute … to authenticated;`, and `008_pfin_service_role_grants.sql` contains **no** function-level EXECUTE grant, so `service_role` acquires EXECUTE only by an explicit future grant. **The failure mode is a future migration adding a grant to make something work**, which is why the leg must be standing. ⚠ Note the asymmetry: for a SECURITY INVOKER function the EXECUTE ACL is normally the weakest of several fences; **against a `rolbypassrls` caller it is the only one**, because RLS is not in the picture.
...
>
> **AC sketch.** App endpoint under the **user's own session** (the session is the tenant binding, which is why this path does not inherit S-3). Server-derives `p_data_as_of` — never client-asserted (Lock 15; **RT-25**). Writes the `draft` row per Lock 11 INSERT-new-version; refuses to finalize until P4's complete-or-skip. Emits the same per-generation audit row as A7 item 5 with **trigger source = on-demand** — so it inherits `⟨RULING⟩` **A7-AUDIT** and cannot be specified before that home is named. `sec-joint-review` label; **QA** legs listed at P10 item 8.
>
> ⚠ **Decision 2 is the governing trigger, not merely the queue-read surface** (Sec **R2.5** item 3): this is **a new user-reachable WRITE path onto a Decision 2 audit-class table**, D2-mandatory joint review on its own — a second, independent reason it does not belong folded into a Frontend-reviewed UI issue.
>
> **⟳ Sec BACKS A10** (`sec-findings.md` @ `fe757d1` **R2.9(2)**), on the attachment point rather than on tidiness, citing ADR-064 Decision 5 **as a precedent for the reasoning and expressly not as jurisdiction over this table** (D5 is scoped to `pfin.account_trans` write surfaces; conflating them would be the false-composite class). D5 verbatim: **"The trigger is the surface, not the layer, and not the author's assessment of risk."** Folding does not change what the surface is — only which layer the issue *looks like* it belongs to. D5 also records that its rule *"is not self-enforcing"* and that the routing trigger *"needs to sit where the router and the builder actually look."* **A separately-labelled issue is that place; a fold relies on a reviewer remembering.**
>
> ⚠ **Fallback, stated so this is not binary** (Sec R2.9(2)): if F/CTO prefers PM's fold-all-five, **the fold is acceptable on two conditions — P5 itself carries `joint-review:sec`, and its AC names the Decision 2 obligation and the RT-25 server-derived-`data_as_of` obligation explicitly rather than inheriting them.** Sec: *"I would rather have the label on P5 than a nineteenth issue that gets dropped at promotion"* — which is **Architect's own named losing side for A10** and a real risk.
>
