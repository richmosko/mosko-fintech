...
**Round-2 folds.** PM's product wording (`pm-findings.md` §8, read at `origin/meta/v15-preflight-pm` @ `4c9f628`, md5 `06db14a4…`) is folded **verbatim and credited `(PM)`** into the blocks that own each string; every round-1 `⟨PM⟩` placeholder is now resolved or names the one PM item that still owns it. The canonical RT labels Sec names at `sec-findings.md` **D-3** (read at `origin/meta/v15-preflight-sec` @ `374ba8e`, md5 `a8e7090d…`) are added to the blocks they belong to. The ADR-029 / `025` **aal2** clause is stated on A1, A2 and A8 per Sec **F-9** and the `090` policy standard. A6's block is now a **close-as-discharged** block carrying Sec **D-1** and **F-6**.
...
> 8. **Status vocabulary bridge**, stated once against the transition (PM **A-8**): cron writes `draft`; completing **or explicitly skipping** authoring promotes to `final`; regeneration supersedes. Presentation mapping: *pending = draft, generated = final; superseded is a storage state only and never renders in V1* (PM A-8).
...
> 2. **Length bound: 120 characters** (PM §2 A8 — *"App B (n) 'length bounds, no executable content' is unanswered anywhere; PM proposes 120 characters, the parity example is 41"*). Enforced as a CHECK **and** in the Zod schema. `⟨PM⟩` — PM's 120 is a proposal, not a ratify; F/CTO confirms at the sitting.
...
> 9. ⚠ **Review classification: `⟨RULING⟩` Sec R-6.** The drafted *"Sec advisory (not joint-review — single-column user-scoped table with no chain)"* is **not accepted by Sec** (**F-7**): *"'No chain' is true and is not the predicate. The predicate is Lock 14 membership, and A8 is the fifth member."* **Architect concurs with Sec** — the same amendment that ratifies the five-table family says of itself that touching Lock 14 carries mandatory joint-review. F/CTO has final authority.
...
> 7. **Inline editing:** ⚠ the drafted *"NO inline edit per ADR-013 P5"* is struck as a mis-citation — ADR-013 Decision 7 governs the four **planning values**, and commentary is not one of them (Arch F-10). Whether commentary edits inline is a UX call with no architectural stake.
>
...
> 5. Filename pattern `⟨PM⟩` — PM §8 does not supply one; it is the single copy string still owed.
...
> 4. **State the snapshot consequence at the editor:** a rename applies **forward only** — historical reports keep the header in effect when they were generated (PRD §2.6.4; PM §5; Sec **M-1**). `⟨PM⟩` whether the editor says so to the user — PM §8 supplies the helper string but not a forward-only notice.
