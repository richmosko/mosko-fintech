...
>
> ⚠ **Decision 2 is the governing trigger, not merely the queue-read surface** (Sec **R2.5** item 3, adding a trigger Sec did not name in round 1): this is **a new user-reachable WRITE path onto a Decision 2 audit-class table**, which is D2-mandatory joint review on its own. That is a second, independent reason it does not belong folded into a Frontend-reviewed UI issue.
>
