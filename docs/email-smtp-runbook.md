...
6. **[BY-HAND]** Verify: submit `/forgot-password` for a real address → the recovery email lands in a **real inbox, not spam**, sender reads `<SMTP_SENDER_NAME> <SMTP_ADMIN_EMAIL>`, link host is `SITE_URL` with path `/auth/confirm?token_hash=…&type=recovery`. ⚠ A link containing `/auth/v1/verify` means the template fetch failed and GoTrue fell back to stock — stop, re-run step 5. **Signup is hard-gated off in prod (`GOTRUE_DISABLE_SIGNUP=true`) — recovery is the live verification path, not signup.**
7. **[BY-HAND]** Access-log residual: Coolify's Traefik may log request paths including the query string, and the confirmation token travels in `/auth/confirm`'s query. Accepted as a single-use/short-TTL residual (token lifetime is a production store read, not assumed) — measured live via `docker inspect coolify-proxy --format '{{json .Args}}'` (grep `--accesslog`; not measured at authoring time). **Enabling Traefik's access log, any request logger in `api/`, or routing `/auth/confirm` via `api-gw` all re-trigger Sec's grade on this ADR (ADR-074).**
