...
| `rt22-manifest-violation/lockfile-transitive/` (`package.json` + `package-lock.json`) | RT-22 dependency-manifest fence (SELF-350 A6) | **Lockfile-scan route, isolated from the manifest-scan route.** `package.json` names only `objection-lite`, a plausible NON-denylisted dependency (deliberately not `knex`/`sequelize` — either would ALSO trip the manifest leg, per DevOps's fence run against `e97a98e`, giving the lockfile route no independent watcher) — `pg` never appears there or anywhere denylisted in the manifest. `package-lock.json`'s resolved tree (lockfileVersion 3 `packages` map) carries `pg` both nested under `node_modules/objection-lite`'s own `dependencies` and as its own flat `node_modules/pg` entry, the way npm actually writes a resolved transitive dependency. Fence output for this fixture is `lockfile:pg` ONLY, with no `manifest:` token — proving the lockfile-scan path fires independently of the manifest-scan path. |
