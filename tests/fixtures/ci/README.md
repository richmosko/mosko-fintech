...
| `rt22-manifest-violation/lifecycle-script/` (`package.json` + `package-lock.json`) | RT-22 dependency-manifest fence (Sec F-11B, 2026-09-05) | **Lifecycle-script route, isolated from the dependency/lockfile routes.** `dependencies` name ONLY the two allowlisted packages (`puppeteer-core` `25.10.0`, `jsonwebtoken` `9.0.2`) with plain registry specs, and the lockfile resolves nothing denylisted — clean on every other route — but `scripts.postinstall` runs `apt-get install -y postgresql-client && npm i pg`, the exact evasion Sec measured live (M11: `npm ci --omit=dev` runs `postinstall` regardless; the Dockerfile has no `--ignore-scripts`). Fence output for this fixture is `manifest:scripts:postinstall(script-not-allowlisted)` ONLY, with no `manifest:dependencies:`/`lockfile:` token — proving the lifecycle-script path fires independently. DevOps-authored per team-lead dispatch; Sec reviewed the catch criterion in the same finding that flagged the gap. |
...
| `report-css-drift-corrupted.css` | `check-report-css-identical.sh` regenerate-and-diff fence (SELF-358 / P6, DevOps, belt-and-braces per Architect's ruling) | **Inversion reference.** The real committed `api/src/lib/generated/report.css` plus one appended corrupting CSS comment. The fence regenerates `report.css` for real via `npm run build:report-css` and diffs the fresh output against this fixture instead of the real committed file; they MUST differ so the fence exits 1. Not itself consumed by any route — the appended comment is inert CSS, but this file is never referenced from `src/**` regardless. |
