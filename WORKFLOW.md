...
**Current phase:** Phase 1 — Product Definition (PRD), in progress. Step 2 (ratification) complete (ADR-002); Step 3 (PRD section drafting) in progress — script-audit pivot complete (ADR-004), `docs/v1-parity-matrix.md` authoritative for V1 capability scope, **PRD §2.1 + §2.2 locked** (both 2026-05-14 — §2.1 per Task #8, §2.2 per Task #14, both Sec passes pass-with-comments; §1 substantively drafted, §1.2 attribute #5 strengthened at v1.6). Next thread: §2.3 (Spending and income categorization) drafting. §§2.3–§7 stubbed; §8 V1 milestone framing queued. Team-mode (`phase-1` team) per ADR-003 active.
