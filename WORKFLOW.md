...
**Current phase:** Phase 1 — Product Definition (PRD), in progress. Step 2 (ratification) complete (ADR-002); Step 3 (PRD section drafting) in progress — script-audit pivot complete (ADR-004), `docs/v1-parity-matrix.md` authoritative for V1 capability scope, **PRD §2.1 + §2.2 + §2.3 locked** (all 2026-05-14 — §2.1 per Task #8, §2.2 per Task #14, §2.3 per Task #23, all Sec passes pass-with-comments; §1 substantively drafted, §1.2 attribute #5 strengthened at v1.6; ADR-002 §1.2 V1 non-goals formally amended per ADR-005 for planning-targets static-reference rendering parity + V1 settings UI). Next thread: §2.4 (Cross-cutting stories — account onboarding, manual entry, re-auth) drafting. §§2.4–§7 stubbed; §8 V1 milestone framing queued. Team-mode (`phase-1` team) per ADR-003 active.
