...
**The general form: an anchored pattern is defeated by decoration on the text it anchors to, and the failure is silent.** `ls` aliased to `-F` appends `*` to executables, so a `\.sample$` anchor matched nothing and sample hook files were counted as active hooks. **Strip decoration — `--` prefixes, continuations, case, glob suffixes — before matching**, and when a result surprises you, **re-run it by a different route rather than more carefully by the same one.** Reading the same broken query again returns the same wrong answer with more confidence.
...
**Invariance is the tell for a broken instrument.** A check that cannot distinguish *present-in-both* from *broken-in-both* is not measuring the property. Identical answers on both sides of a comparison read as confirmation, and have repeatedly been failure instead — `$?` after a pipeline returns the *last* command's status, so `ruff check … | tail` reported `0` while ruff itself exited `1`, and the same `0` came back from both sides of the comparison it was feeding. **Where a check compares two refs or two states, vary the input and require the answer to change: a check that cannot fail has not passed.**
