...
**Current phase:** Phase 1 — Product Definition (PRD), in progress. Step 2 (ratification) complete (ADR-002); Step 3 (PRD section drafting) in progress — script-audit pivot complete (ADR-004), `docs/v1-parity-matrix.md` authoritative for V1 capability scope, **PRD §2.1 + §2.2 + §2.3 + §2.4 + §2.5 + §2.6 + §3 + §5 locked** (§2 series on 2026-05-14 / 2026-05-15 / 2026-05-17; §3 on 2026-05-17 — first non-§2 section, 4-for-4 PM-lean ratifications; **§5 on 2026-05-17 — second non-§2 section locked; 6-for-6 PM-lean ratifications (Q1 structure + Q2 format + Q3a TLH→§6 + Q3b 4-reroute bundle + bulk-closeout cadence + bulk-closeout body+ADR-007 acceptance); no Sec at-lock pass (no credential-handling surface); bulk-closeout-from-structure-proposal pattern reused successfully**; all Sec passes on §2 sections pass-with-comments; §1 substantively drafted, §1.2 attribute #5 strengthened at v1.6; ADR-002 §1.2 V1 non-goals amended per ADR-005 for planning-targets settings UI; ADR-004 Decision D input-layer amended per ADR-006 for bracket schedules + Sub-Cat tax_character enum; **ADR-002 Finding (b) amended per ADR-007 — tax-loss-harvesting recommendations reclassified from V2+ candidate to permanent non-goal (§6 home) under advisor-role axis; first amendment exercising §5/§6 axis-as-product-identity-boundary pattern**; ADR-005 settings store carries three additive fields; §3.4 is home for "V1 done" definition per F/CTO Q2 lock). **PRD §2 series + §3 + §5 complete.** Next thread per F/CTO sequencing choice (a) — continue lighter PM-led inventory sections (§6 Out-of-scope for PRD lifecycle → §7 Constraints) before closing with §4 (Sec primary author, mandatory engagement per Sec Task #54 framing). §6 will pick up TLH per ADR-007 + the existing ADR-002 §3.0 permanent non-goals. §8 V1 milestone framing queued (references §3.4 V1-done criteria). Team-mode (`phase-1` team) per ADR-003 active.
