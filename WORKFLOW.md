...
**Current phase:** Phase 1 — Product Definition (PRD), in progress. Step 2 (ratification) complete (ADR-002); Step 3 (PRD section drafting) in progress — script-audit pivot complete (ADR-004), `docs/v1-parity-matrix.md` authoritative for V1 capability scope, **PRD §2.1 + §2.2 + §2.3 + §2.4 locked** (§2.1/§2.2/§2.3 on 2026-05-14 per Tasks #8/#14/#23; §2.4 on 2026-05-15 per Task #33 preceded by Task #27 two-touch early consult; all Sec passes pass-with-comments; §1 substantively drafted, §1.2 attribute #5 strengthened at v1.6; ADR-002 §1.2 V1 non-goals amended per ADR-005 for planning-targets static-reference rendering parity + V1 settings UI; no new ADR for §2.4 — all locks fold cleanly into PRD body + routing-flags). Next thread: §2.5 (Estimated taxes — Federal + California FTB primitive form per ADR-004 Decision D) drafting. §§2.5–§7 stubbed; §4 awaits Sec primary authoring; §8 V1 milestone framing queued. Team-mode (`phase-1` team) per ADR-003 active.
