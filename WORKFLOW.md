...
**Current phase:** Phase 1 — Product Definition (PRD), in progress. Step 2 (ratification) complete (ADR-002); Step 3 (PRD section drafting) in progress — script-audit pivot complete (ADR-004), `docs/v1-parity-matrix.md` authoritative for V1 capability scope, **PRD §2.1 + §2.2 + §2.3 + §2.4 + §2.5 + §2.6 + §3 locked** (§2.1/§2.2/§2.3 on 2026-05-14; §2.4 on 2026-05-15; §2.5 + §2.6 on 2026-05-17; **§3 on 2026-05-17 — first non-§2 PRD section locked; 4-for-4 PM-lean ratifications (Q1 structure + Q2 V1-done home + Q3 tolerance + bulk-closeout body draft); no Sec at-lock pass (no credential-handling surface; Sec is §4 primary author next); bulk-closeout drafting pattern from §2.6 reused successfully**; all Sec passes on §2 sections pass-with-comments; §1 substantively drafted, §1.2 attribute #5 strengthened at v1.6; ADR-002 §1.2 V1 non-goals amended per ADR-005 for planning-targets static-reference rendering parity + V1 settings UI; ADR-004 Decision D input-layer characterization amended per ADR-006 for bracket schedules + Sub-Cat tax_character enum at V1; no new ADR for §2.6 or §3 — locks fold cleanly; ADR-005 settings store extended additively (third additive field after §2.3.2 + §2.5.2); **§3.4 is home for "V1 done" definition, §8 references §3.4 per F/CTO Q2 lock**). **PRD §2 series + §3 complete.** Next thread per F/CTO sequencing choice (a) — drive lighter PM-led inventory sections (§5 V2 deferred candidates → §6 Out-of-scope for PRD lifecycle → §7 Constraints) before closing with §4 (Sec primary author, mandatory engagement per Sec Task #54 framing). §8 V1 milestone framing queued (references §3.4 V1-done criteria). Team-mode (`phase-1` team) per ADR-003 active.
