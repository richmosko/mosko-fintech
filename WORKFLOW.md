...
**⚠⚠ The same wrong upstream aims a force-push at `main`.** `git checkout -B <branch> origin/main` leaves the upstream as `origin/main`, so a bare `git push --force-with-lease` targets **`main`**, not your branch. It happened, and it failed **only because `allow_force_pushes: false` held.** ⚠ **Branch protection covers `main` and nothing else — the same mistake aimed at a teammate's branch succeeds silently.** When a force-push is genuinely warranted, name both ends:
...
**Sweeping comment or header text: strip, join, normalise — and prove the instrument.** A search over `--` comment blocks MUST strip the comment prefix, join continuation lines, and normalise case before matching, and MUST be confirmed by a **control string known to be present, which must match**. Each clause has its own measured failure: a phrase spanning a line break; a phrase whose case had been shifted to a file's emphasis convention; and a `git show "$ref:$path"` in which zsh consumed `$ref:` as a history modifier and silently resolved the wrong object (use `${ref}:${path}`). **A `0` from an unconfirmed filter is not evidence of absence** — it is equally consistent with a filter that could never have matched anything.

**The general form: an anchored pattern is defeated by decoration on the text it anchors to, and the failure is silent.** `ls` aliased to `-F` appends `*` to executables, so a `\.sample$` anchor matched nothing and **14 sample files were counted as 14 active hooks** — a plausible number, which is why it was believed. **Strip decoration — `--` prefixes, continuations, case, glob suffixes — before matching**, and when a result surprises you, **re-run it by a different route rather than more carefully by the same one.** Reading the same broken query again returns the same wrong answer with more confidence. *This list is a sample of one day, not a complete inventory of the class.*
...
**Invariance is the tell for a broken instrument.** A check that cannot distinguish *present-in-both* from *broken-in-both* is not measuring the property. `$?` after a pipeline returns the *last* command's status, so `ruff check … | tail` reported `0` while ruff itself exited `1` — and, in DevOps's words, *"the `$?` bug returned `0` for both sides of a comparison, and two readings that agree look like confirmation rather than a broken instrument."* **Where a check compares two refs or two states, vary the input and require the answer to change: a check that cannot fail has not passed.**

**The generating move, stated by Sec after the session that produced most of this section:** *"All three came from distrusting a result that arrived too cleanly. None required expertise. All required refusing to accept a correct-looking answer."* Every rule here is a special case of that.

**A count is unreliable in BOTH directions on a naming sweep**, and the two failures look identical. One count **passed** incorrect work — a stale registry — and would have **flagged correct work as failure**, because a restored mention was legitimately re-added. ⚠ **`grep -c` counts LINES, not occurrences**: one identifier measured **12 occurrences on 9 lines**. **The use-vs-name classification is what makes a sweep verifiable; the number never was.**

**A sweep over an HTML artifact has three independent retrieval failures, each with a confirmed instance:** **token form** (one word-form saw 12 hits where the any-form pattern saw 72), **case** (three occurrences capitalised at sentence start), and **tag-splitting** (a phrase interrupted by `<code>…</code>` returns 0). ⚠ **And the inverse — do NOT tag-strip when the target is an HTML comment**: stripping deletes it wholesale.

**The discriminator's known blind spots are emphasis-loss, substitution, synonymy, and line-orientation.** Five sweeps under-counted on one branch and **every one had been verified by whoever scoped it.** The rule that follows: **state the predicate AND its window**, and **a clean sweep reports the predicate's reach, never the claim's absence.**

**A partial correction is worse than none** — it removes the uniformity that made the staleness survivable, so the remaining instances now read as deliberate. **Correcting one instance obliges enumerating its siblings** — ⚠ and **the sibling set is defined by the CLAIM, not by the string**: enumerating a dropped column name missed three further sites that used the superseded *view* name.

**A test that fails when you implement a requirement is evidence about the requirement, not an obstacle in front of it.** Backend implemented a proposed fence exactly as specified, saw one test fail — the one pinning ratified behaviour — and **reverted the fence rather than editing the test.** That is how a false premise gets caught by the person implementing it.

**A self-correction is a reason to re-measure, not a reason to discard the observation.** A relay that self-corrected mid-answer was recorded as unverified for a full session; when finally re-queried, **the original observation was true.** Neither trusting nor dropping it was right — measuring again was.
...
**⚠ Guard shape — a guard must survive inattention, not require attention.** **A guard that depends on the next person reading carefully is decoration. A guard that survives `grep -A1`, a trim, a partial-block copy and a context boundary without anyone's attention is a control.**

Concretely, and this is the general shape for **any** marker, not one convention: **a marker repeats on every line of what it marks, rather than heading the block once.** The redundancy is the entire point. **A marker that any ordinary operation can separate from what it marks is a conditional waiting to be re-armed by transcription.** Inseparable beats nearby.

Prefer guards that hold **by construction** over guards that hold by discipline. If the guard's correctness depends on someone noticing it, it will hold until the first person who does not.

**⚠ Marker scope — scope by hazard, not by provenance.** **Mark any string that would itself match the sweep the marker exists to defeat.** Provenance — whether the file once asserted the string, merely considered it, or quoted it from elsewhere — is the wrong test, because the sweep does not know the provenance and neither does the author running it.

**But this is still not "any false-looking string", and the boundary is an asymmetry in what a sweep can do:**

- A **claim the file once made** is *resurrectable*. The file genuinely had that state, so a sweep "correcting" it produces **a plausible-looking reversion to real history** — the falsehood returns intact and the diff reads as a cleanup. **Mark it.**
- An **illustrative wrong phrasing** — one written only to show what to avoid, never asserted — has **no prior state to restore it to**. Touching it yields a visibly novel edit, not a reversion. **Do not mark it.**

**If such a string ever needs protection it gets a DIFFERENT marker, never a widened one.** A marker meaning two things is one a sweep author cannot act on, and widening destroys the single distinction that makes it actionable.
...
**A measured ref is pinned** ([ADR-052](DECISIONS.md#adr-052) — the alternatives, their rejection rationale and the worked example live there; this is the operative rule). **A ref is pinned the moment anyone other than its author measures it and reports the measurement.** From that point: **new commits only — no `--amend`, no rebase, no force-push.** The author keeps working and keeps fixing; they *stack* rather than *rewrite*.

**What this protects is a measurement someone else can no longer reproduce — not an approval.** That distinction is the entire rule, and it is what fixes where the freeze arms: not at first commit, which throttles the author while nothing yet exists to invalidate; and **not at first sign-off, which is the tempting answer and is wrong** — a measured, reported, never-signed-off commit was destroyed during the review that produced this rule, and a sign-off rule would have permitted it. **At first external measurement** is both earlier and cheaper than either alternative. **The constraint is on rewriting history, not on changing the work.** An amend orphans the measured ref; a new commit leaves it reachable. Nothing is blocked.

Orphaned commits survive as unreachable objects until `gc`. **Cite a destroyed ref as destroyed** (`<sha>`, unreachable, recoverable) rather than silently re-anchoring to its replacement — a verification that named a ref which then vanished is a fact about the review, and the record should show it. *(Named rather than numbered on purpose — this was "condition 10" in the review that produced it, and a bare ordinal carried into a new document is a false composite waiting to happen.)*
...
**⚠ A silent delivery gap looks exactly like an agent going quiet, and the two have opposite responses.** On 2026-08-11 an agent replied to a handoff repeatedly with **none of its replies reaching the lead**; an item was re-dispatched after it had already been answered, and rulings were issued against a ref `HEAD` had moved past. **Log this class as a DELIVERY GAP, not as an agent that stalled** — the wrong label teaches the next session a wrong model of its own team. Practically: when a teammate seems silent, a diagnostic ping ("did you receive X?") beats a corrective re-poke, and when a measurement disagrees with someone who has just measured the same thing, **check whether the ref they verdicted against is still the head** before forming a view. The likely explanation is a crossing, not a disagreement.

**Cite by an anchor that resolves in the artifact named.** Labels like `2.1.2.c` are **Linear AC sub-letters**, not PRD sections — they appear **zero** times in the PRD. Written with a `§` prefix they read as PRD anchors and **pass every spot-check of their parts**, which is why this propagated into a ledger and had to be swept out. **Cite Linear issues by ID and title; reserve `§` for anchors that resolve in the document you are naming.**
...
⚠ **`CHANGELOG.md` is FROZEN as of 2026-08-12 — not maintained, and no skill writes to it.** It is retained, not deleted, because `MILESTONES.md` and `BACKLOG.md` cite its `v1.NN` versions as anchors and deleting it would orphan them. Nothing reads it programmatically, no CI job touches it, and **it went 19 versions stale without anyone noticing** — the same signature as the pre-commit set that sat inert for six weeks: an artifact whose absence produces no symptom, because nothing depends on it. **A write-only artifact stops costing anything the moment you stop writing to it**, so the maintenance halts now and the delete stays a cheap decision later, once it is clear whether it is ever missed. **Completed work is recorded in the PR body, which GitHub keeps.** ⚠ **Do not backfill, reformat, or "catch up" the file** — and if you meet a procedure with an obviously missing CHANGELOG step, that step was removed on purpose; do not restore it.
...
**Gates (per-PR, F/CTO-ratified 2026-08-22; light-loop tier added 2026-09-02 per [ADR-066](DECISIONS.md#adr-066); expected-red header retirement added 2026-09-02 as convention):**
...
- **The light-loop tier — a SCOPED loop for a bounded class of issue** ([ADR-066](DECISIONS.md#adr-066)). An issue runs the light loop when **all three** hold: **(a)** it adds no DB surface — no migration, no schema / RLS policy / grant / trigger / function change; **(b)** it changes no money path — no financial calculation and no figure a user reads as money, *rendering included*; and **(c)** Sec is not mandatory per the ratified joint-review map — **read that map live from [ADR-011](DECISIONS.md#adr-011) and the `spawn-sec-joint-review` trigger list; this is a pointer, not a copy.** The predicate is a conjunction with no discretion: *"mostly light"* is not a state, and failing one bullet means the full loop rather than a negotiation. **What scopes down:** the walk covers **the changed arc only** (the paths the diff can reach, still driven for real); **no fresh full-suite scratch chain run where CI already covers it** (where CI does not cover it, the run is not optional); and **delta-depth review** — the diff at its own depth, without the surrounding-module sweep a DB or money surface earns. **What does NOT scope down, enumerated because a tier that quietly drops a gate is the failure mode:** the walk still happens (narrowed, never skipped) · combined-tree green before PR · QA test-pairing for anything extending a verification surface · F/CTO ratify at PR · walk-before-Sec ordering ([ADR-063](DECISIONS.md#adr-063)) wherever Sec is in play at all. **Who calls it: team-lead, at dispatch.** ⚠ **The tier is a property of the issue's CURRENT scope, not a label applied once** — it **RE-EVALUATES** on any scope change, and specifically when *a migration appears · a money-path figure changes · any surface on the ratified map is touched · an F/CTO ruling adds a deliverable*. **On trigger the tier lapses immediately and the full loop applies to the WHOLE ARC, not only to the part that was added** — the addition is usually what makes the earlier work reachable in a new way, and that earlier work was reviewed under an assumption the addition just falsified. Anyone may call the lapse; team-lead confirms it. **SELF-258 is the worked example:** dispatched light and qualifying honestly, it stopped qualifying the moment migration `099` appeared, and the lapsed gates then caught a join-shaped fail-open and a falsified CONTRACT invariant — neither visible to a green suite. **The tier changes no gate's content; it changes scope of application only.**
- **An "expected-red" header RETIRES in the closing commit of the pair that lands its counterpart.** Where one half of a paired artifact lands first and carries a header note that a check is expected to fail until the other half arrives, **removing that note is part of the commit that lands the other half** — not a follow-up, not a cleanup pass. **Why it is a gate rather than tidiness:** the note is a live warning to whoever reads the file next, and the moment the counterpart lands it becomes a false one that still reads as authoritative — a reader meets it and either doubts a green check or stops investigating a red one. It is also the cheapest possible fix at exactly one moment and progressively harder afterwards, because the person who knows the pair is closed is the person closing it. Flagged twice by Sec, and the stale-header class was measured **three times on the SELF-258 branch alone by Sec's freeze review (PR #591)**. ⚠ **The check is on the CLOSING commit, so it cannot be satisfied by the opening one:** the author landing the first half should expect to return to the note, and the reviewer of the closing commit should look for its removal in that diff specifically.
